...
public class JaxbToJsonTest {
  ...
  public void bug61264AttributeDispositionCONTENThandling() throws Exception {
    ...
    // Bug 61264 is about issues with code that used JAXB which replaced code in
    // GetCalendarItemSummaries
    // which started with an element created using:
    //     calItemElem = lc.createElement(isAppointment ? MailConstants.E_APPOINTMENT :
  ...
  /**
   * XmlValue should map to an attribute with name "_content" At present, classes that want this
   * feature need the annotation @JsonProperty("_content"), otherwise, the name "value" is used.
   * "chunk": [ { "disp": "disposition 1", "_content": "text 1\nIn the sun" }, { "disp":
   * "disPosition 2", "_content": "text 2" }],
   */
  @Test
  public void xmlValueAnnotation() throws Exception {
    ...
    String text2 = "text 2";
    // ---------------------------------  For Comparison - Element handling where the JAXB has an
    // @XmlValue
    Element legacyElem = JSONElement.mFactory.createElement(MailConstants.DIFF_DOCUMENT_RESPONSE);
    legacyElem
        .addNonUniqueElement(MailConstants.E_CHUNK)
        .addAttribute(MailConstants.A_DISP, dispos1)
        .setText(text1);
    legacyElem
        .addNonUniqueElement(MailConstants.E_CHUNK)
        .addAttribute(MailConstants.A_DISP, dispos2)
        .setText(text2);
    logDebug("DiffDocumentResponse JSONElement ---> prettyPrint\n%1$s", legacyElem.prettyPrint());
    // --------------------------------- @XmlValue handling test - need @JsonProperty("_content")
    // annotation
    ...
    logDebug(
        "DiffDocumentResponse JSONElement from JAXB ---> prettyPrint\n%1$s",
        jsonJaxbElem.prettyPrint());
    ...
    Assert.assertEquals(
        "1st chunk disposition", dispos1, chunk1.getAttribute(MailConstants.A_DISP));
    Assert.assertEquals("1st chunk value", text1, chunk1.getText());
    Assert.assertEquals(
        "2nd chunk disposition", dispos2, chunk2.getAttribute(MailConstants.A_DISP));
    Assert.assertEquals("2nd chunk value", text2, chunk2.getText());
    DiffDocumentResponse roundtripped =
        JaxbUtil.elementToJaxb(jsonJaxbElem, DiffDocumentResponse.class);
  ...
  public void keyValuePairMbxSearch() throws Exception {
    ...
    Assert.assertEquals(
        "KeyValuePair notificationMessage key", "notificationMessage", kvp4.getKey());
  ...
  public void bug65572BooleanAndXmlElements() throws Exception {
    ...
    Assert.assertEquals(
        "roundtripped header test stringComparison", "contains", rtHdrTest.getStringComparison());
  ...
  public void xmlEnumValuesInAttrAndElem() throws Exception {
    ...
    // Want '' not 'UNKNOWN'
    Assert.assertEquals(
        "fold2 value", ViewEnum.UNKNOWN.toString(), jsonJaxbElem.getElement("fold2").getText());
    Assert.assertEquals(
        "roundtripped fold1", ViewEnum.VIRTUAL_CONVERSATION, roundtripped.getFold1());
  ...
  public void xmlEnumValuesInAttributes() throws Exception {
    ...
    // Want '' not 'UNKNOWN'
    Assert.assertEquals(
        "fold2 value", ViewEnum.UNKNOWN.toString(), jsonJaxbElem.getAttribute("fold2"));
    Assert.assertEquals(
        "roundtripped fold1", ViewEnum.VIRTUAL_CONVERSATION, roundtripped.getFold1());
    Assert.assertEquals("roundtripped fold2", ViewEnum.UNKNOWN, roundtripped.getFold2());
  }
  ...
  public void stringAttrAndElem() throws Exception {
    ...
    Assert.assertEquals(
        "JSONElement elem1", elem1Val, jsonJaxbElem.getElement("element1").getText());
  ...
  public void elemsInDiffNamespace() throws Exception {
    ...
    Assert.assertEquals(
        "JSONElement attr1", attr1Val, saseJsonJaxbElem.getAttribute("attribute-1"));
    Assert.assertEquals(
        "JSONElement elem1", elem1Val, saseJsonJaxbElem.getElement("element1").getText());
  ...
  public void stringElemAndZimbraJsonlongAttr() throws Exception {
    ...
    Assert.assertEquals(
        "JSONElement authToken", authToken, jsonJaxbElem.getElement("authToken").getText());
  ...
  public void wrapperAbsentIfEmpty() throws Exception {
    ...
    Assert.assertNotNull(
        "XML roundtripped getNumbers (when 2 numbers in list)", roundtrippedX.getNumbers());
    Assert.assertEquals(
        "JSON roundtripped size of Numbers (2 numbers)", 2, roundtripped.getNumbers().size());
    ...
    Assert.assertEquals(
        "XML roundtripped size of Numbers (2 numbers)", 2, roundtrippedX.getNumbers().size());
    Assert.assertEquals(
  ...
  public void mixedHandlingWithJaxbAndNoText() throws Exception {
    ...
    Assert.assertEquals(
        "roundtripped [Mixed has element] num elems", 1, roundtripped.getElems().size());
    StringAttribIntValue rtByRef = (StringAttribIntValue) roundtripped.getElems().get(0);
  ...
  public void mixedHandlingJustText() throws Exception {
    ...
    MixedTester roundtrippedX = JaxbUtil.elementToJaxb(xmlElem, MixedTester.class);
    Assert.assertEquals(
        "roundtrippedX [Mixed has just text] num elems", 1, roundtrippedX.getElems().size());
    Assert.assertEquals(
    ...
    MixedTester roundtripped = JaxbUtil.elementToJaxb(jsonJaxbElem, MixedTester.class);
    Assert.assertEquals(
        "roundtripped [Mixed has just text] num elems", 1, roundtripped.getElems().size());
    Assert.assertEquals(
        "roundtripped [Mixed has just text] str", textStr, (String) roundtripped.getElems().get(0));
  }
  ...
  public void mixedAndAnyElementHandlingJustElement() throws Exception {
    ...
    Assert.assertEquals(
        "roundtrippedX [Mixed w3c element] num elems", 1, roundtrippedX.getElems().size());
    org.w3c.dom.Element w3ce = (org.w3c.dom.Element) roundtrippedX.getElems().get(0);
    Assert.assertEquals(
        "roundtrippedX [Mixed w3c element] elem name", "alien", w3ce.getLocalName());
    Element jsonJaxbElem = JacksonUtil.jaxbToJSONElement(jaxb);
    logDebug(
    ...
    Assert.assertEquals(
        "roundtripped [Mixed w3c element] num elems", 1, roundtripped.getElems().size());
    ...
    Assert.assertEquals(
        "roundtripped [Mixed w3c element] elem namespace", "urn:foreign", rtElem.getNamespaceURI());
  }
  ...
  public void systemRetentionPolicyResponse() throws Exception {
    ...
    logDebug("JSONElement from JAXB ---> prettyPrint\n%1$s", jsonJaxbElem.prettyPrint());
    Assert.assertEquals(
        "json string from jaxb", jsonGetSystemRetentionPolicyResponse, jsonJaxbElem.prettyPrint());
