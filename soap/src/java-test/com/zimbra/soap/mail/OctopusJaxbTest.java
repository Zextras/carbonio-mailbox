...
public class OctopusJaxbTest {
  ...
  /**
   * Validating that method used to create response in CheckDeviceStatus should work.
   *
   * @throws Exception
   */
  @Test
  ...
  private static ActivityInfo toActivityInfo(
      String account,
      String op,
      long timestamp,
      String itemId,
    ...
    if (version > 0) activity.setVersion(version);
    if (account != null) activity.setEmail(account);
  ...
  private static void checkActivityInfo(ActivityInfo ai, String tag) {
    ...
    Assert.assertEquals(
        tag + "arg1 value prefix", "value", nvArgs.get(0).getValue().substring(0, 5));
    Assert.assertEquals(tag + "arg2 name prefix", "key", nvArgs.get(1).getName().substring(0, 3));
    Assert.assertEquals(
        tag + "arg2 value prefix", "value", nvArgs.get(1).getValue().substring(0, 5));
  }
  ...
  public void activityInfoForGetActivityStreamXml() throws Exception {
    ...
    Element activityElem =
        JaxbUtil.addChildElementFromJaxb(
            notify, MailConstants.E_A, MailConstants.NAMESPACE_STR, ai);
