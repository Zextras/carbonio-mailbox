...
public final class GetFolderTest {
  ...
  /**
   * Motivated by Bug 55153 failure in ZGrant.java line 134: mGranteeType =
   * GranteeType.fromString(grant.getGranteeType().toString());
   */
  @Test
  public void unmarshallGetFolderResponseContainingGrant() throws Exception {
    GetFolderResponse result =
        (GetFolderResponse)
    ...
    for (Folder child : top.getSubfolders()) {
      ...
      if (acl != null) {
        ...
        if (myGrants.size() > 0) {
          ...
          GrantGranteeType mGranteeType =
              GrantGranteeType.fromString(first.getGranteeType().toString());
  ...
  public void unmarshallSearchFolderEmptyTypes() throws Exception {
    ...
    for (Folder folder : resp.getFolder().getSubfolders()) {
      if ("searchfolder-with-types".equals(folder.getName())) {
        Assert.assertEquals(
            EnumSet.of(ItemType.CONVERSATION, ItemType.DOCUMENT),
