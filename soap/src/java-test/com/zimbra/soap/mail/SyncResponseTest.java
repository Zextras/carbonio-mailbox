...
public final class SyncResponseTest {
  ...
  public void unmarshallSyncResponse() throws Exception {
    ...
    SyncResponse resp =
        (SyncResponse) unmarshaller.unmarshal(getClass().getResourceAsStream("SyncResponse.xml"));
        ...
        if (expect.getClass() == item.getClass()) {
          if (expectedMsg != null) {
            ...
            if (expectedSyncFolder.getId().equals(folder.getId())) {
              ...
              for (Iterator<IdsAttr> expectIdsIterator = expectedItems.iterator();
                  expectIdsIterator.hasNext(); ) {
    ...
    for (IdsAttr deleteIds : deletesByType) {
      if (deleteIds instanceof ConvIdsAttr && deleteIds.getIds().equals("1455,999")) {
        foundConvs = true;
      } else if (deleteIds instanceof MsgIdsAttr
          && deleteIds.getIds().equals("578,1448,1443,1451,1441,1454")) {
        foundMsg = true;
      }
    }
