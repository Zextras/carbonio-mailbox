...
public class TritonBlobStoreManagerTest extends AbstractStoreManagerTest {
  ...
  public void sis() throws Exception {
    ...
    Assert.assertTrue(
        "StoreManager is content addressable", sm instanceof ContentAddressableStoreManager);
    Assert.assertTrue(
        "StoreManager supports SIS check", sm.supports(StoreFeature.SINGLE_INSTANCE_SERVER_CREATE));
    ...
    Assert.assertTrue(
        "blob content = mime content", TestUtil.bytesEqual(bytes, blob.getInputStream()));
    ...
    Assert.assertTrue(
        ...
        TestUtil.bytesEqual(bytes, mblob.getLocalBlob().getInputStream()));
    ...
    Assert.assertTrue(
        "blob content = mime content", TestUtil.bytesEqual(bytes, sisBlob.getInputStream()));
