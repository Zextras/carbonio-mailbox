...
public class BlobInputStreamTest {
  ...
  public void deleteHalfway() throws Exception {
    ...
    Assert.assertEquals(
        "can read 10 bytes from full substream", CONTENT.length, copy.read(new byte[100]));
    try (InputStream badStartIs = bis.newStream(-1, CONTENT.length)) {
      ...
      Assert.assertTrue(
          "Blob name included in exception message",
          iae.getMessage().contains(file.getAbsolutePath()));
    }
    ...
    Assert.assertEquals(
        "can read 6 bytes from substream after delete", 6, substream.read(new byte[100]));
