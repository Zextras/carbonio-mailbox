...
public final class RFC822AddressTokenStreamTest {
  ...
  public void multi() throws Exception {
    TokenStream stream =
        new RFC822AddressTokenStream(
            "\"User One\" <user.1@zimbra.com>, \"User Two\" <user.2@zimbra.com>, \"User Three\""
                + " <user.3@zimbra.com>");
    Assert.assertEquals(
        Arrays.asList(
            "user",
  ...
  public void topPrivateDomain() throws Exception {
    ...
    Assert.assertEquals(
        Arrays.asList(
            "support@zimbra.com", "support", "@zimbra.com", "zimbra.com", "zimbra", "@zimbra"),
        ZimbraAnalyzerTest.toTokens(stream));

    stream = new RFC822AddressTokenStream("support@zimbra.vmware.co.jp");
    Assert.assertEquals(
        Arrays.asList(
            "support@zimbra.vmware.co.jp",
            "support",
            "@zimbra.vmware.co.jp",
            "zimbra.vmware.co.jp",
            "vmware",
    ...
    Assert.assertEquals(
        Arrays.asList("test@co.jp", "test", "@co.jp", "co.jp"),
        ZimbraAnalyzerTest.toTokens(stream));
  }
  ...
  public void limit() throws Exception {
    TokenStream stream =
        new RFC822AddressTokenStream("<" + Strings.repeat("x.", 600) + "x@zimbra.com>");
  ...
  public void japanese() throws Exception {
    TokenStream stream =
        new RFC822AddressTokenStream("=?utf-8?B?5qOu44CA5qyh6YOO?= <jiro.mori@zimbra.com>");
    Assert.assertEquals(
        Arrays.asList(
            "\u68ee",
            "\u6b21\u90ce",
            "jiro.mori@zimbra.com",
            "jiro.mori",
            "jiro",
            "mori",
            "@zimbra.com",
