...
public final class QueryParserTest {
  ...
  public void text() throws Exception {
    ...
    Assert.assertEquals(
        "(Q(l.content:x) || Q(l.content:y)) && Q(IN:Inbox)", Query.toString(parser.parse(src)));
  ...
  public void date() throws Exception {
    String src = "date:-4d";
    Assert.assertEquals(
        "Q(DATE:DATE," + getDate(-4) + "-" + getDate(-3) + ")", Query.toString(parser.parse(src)));
    ...
    Assert.assertEquals(
        "(Q(l.content:) || Q(l.content:b)) && Q(DATE:BEFORE,196912312359-200901010000) && "
            + "-Q(subject:quoted,string)",
    ...
    Assert.assertEquals(
        "Q(DATE:DATE,"
            ...
            + "-"
            + getDate(0)
            + ") && (Q(DATE:DATE,200101010000-200101020000) &&"
            + " Q(DATE:DATE,200202020000-200202030000))",
        Query.toString(parser.parse(src)));
    ...
    src = "date:\"+1d\"";
    Assert.assertEquals(
        "Q(DATE:DATE," + getDate(1) + "-" + getDate(2) + ")", Query.toString(parser.parse(src)));

    src = "date:+2w";
    Assert.assertEquals(
        "Q(DATE:DATE," + getWeek(2) + "-" + getWeek(3) + ")", Query.toString(parser.parse(src)));

    src = "not date:(1/1/2004 or 2/1/2004)";
    Assert.assertEquals(
        "-(Q(DATE:DATE,200401010000-200401020000) || Q(DATE:DATE,200402010000-200402020000))",
        Query.toString(parser.parse(src)));
  }
  ...
  /**
   * Validate that date queries parse to the proper ranges. The only caveat here is that a query
   * like {@code date:>foo} turns into the range {@code (foo+1, true, -1, false)} instead of the
   * more obvious one {@code (foo, false, -1, false)} -- this is a quirk of the query parsing code.
   * Both are correct.
   */
  @Test
  ...
  public void builtIn() throws Exception {
    ...
    Assert.assertEquals(
        "Q(TAG:\\Unread,UNREAD) && Q(UNDER:REMOTE)", Query.toString(parser.parse(src)));
  }
  ...
  public void subject() throws Exception {
    ...
    Assert.assertEquals(
        "Q(subject:foo,bar) && Q(l.content:baz,gub)", Query.toString(parser.parse(src)));

    src = "subject:this_is_my_subject subject:\"this is_my_subject\"";
    Assert.assertEquals(
        "Q(subject:my,subject) && Q(subject:my,subject)", Query.toString(parser.parse(src)));
  }
  ...
  public void has() throws Exception {
    ...
    Assert.assertEquals(
        "Q(attachment:any) && Q(has:phone) && Q(has:url)", Query.toString(parser.parse(src)));
  }
  ...
  public void filename() throws Exception {
    ...
    Assert.assertEquals(
        "Q(filename:foo) && (Q(filename:foo) && Q(filename:foo,bar) && Q(filename:gub))",
        Query.toString(parser.parse(src)));
  }
  ...
  public void typeImages() throws Exception {
    ...
    Assert.assertEquals(
        "Q(type:image) && Q(type:image/jpeg) && Q(type:image/gif) && Q(type:image/bmp)",
        Query.toString(parser.parse(src)));
  }
  ...
  public void tag() throws Exception {
    ...
    Assert.assertEquals(
        "(Q(TAG:\\Unread,READ) && Q(TAG:\\Unread,UNREAD))", Query.toString(parser.parse(src)));

    src = "is:(flagged unflagged)";
    Assert.assertEquals(
        "(Q(TAG:\\Flagged,FLAGGED) && Q(TAG:\\Flagged,UNFLAGGED))",
        Query.toString(parser.parse(src)));

    src = "is:(\"sent\" received)";
    Assert.assertEquals(
        "(Q(TAG:\\Sent,SENT) && Q(TAG:\\Sent,RECEIVED))", Query.toString(parser.parse(src)));

    src = "is:(replied unreplied)";
    Assert.assertEquals(
        "(Q(TAG:\\Answered,REPLIED) && Q(TAG:\\Answered,UNREPLIED))",
        Query.toString(parser.parse(src)));

    src = "is:(forwarded unforwarded)";
    Assert.assertEquals(
        "(Q(TAG:\\Forwarded,FORWARDED) && Q(TAG:\\Forwarded,UNFORWARDED))",
        Query.toString(parser.parse(src)));
  }
  ...
  public void size() throws Exception {
    ...
    Assert.assertEquals(
        "(Q(SIZE:=1) && Q(SIZE:=20) && Q(SIZE:=300) && Q(SIZE:=1024) && Q(SIZE:=10240) &&"
            + " Q(SIZE:=102400) && Q(SIZE:=35651584))",
        Query.toString(parser.parse(src)));
    ...
    Assert.assertEquals(
        "(Q(SIZE:>1) && Q(SIZE:>20) && Q(SIZE:>300) && Q(SIZE:>102400) && Q(SIZE:>35651584))",
        Query.toString(parser.parse(src)));
  ...
  public void metadata() throws Exception {
    ...
    Assert.assertEquals(
        "Q(AUTHOR:foo) && (Q(AUTHOR:foo) && Q(AUTHOR:foo,bar) && Q(AUTHOR:gub))",
        Query.toString(parser.parse(src)));

    src = "title:foo title:(\"foo\" \"foo bar\" gub)";
    Assert.assertEquals(
        "Q(TITLE:foo) && (Q(TITLE:foo) && Q(TITLE:foo,bar) && Q(TITLE:gub))",
        Query.toString(parser.parse(src)));

    src = "keywords:foo keywords:(\"foo\" \"foo bar\" gub)";
    Assert.assertEquals(
        "Q(KEYWORDS:foo) && (Q(KEYWORDS:foo) && Q(KEYWORDS:foo,bar) && Q(KEYWORDS:gub))",
        Query.toString(parser.parse(src)));

    src = "company:foo company:(\"foo\" \"foo bar\" gub)";
    Assert.assertEquals(
        "Q(COMPANY:foo) && (Q(COMPANY:foo) && Q(COMPANY:foo,bar) && Q(COMPANY:gub))",
        Query.toString(parser.parse(src)));
  }
  ...
  public void textLexicalState() throws Exception {
    ...
    Assert.assertEquals(
        "Q(from:and) || Q(from:or) || -Q(from:not)", Query.toString(parser.parse(src)));
  }
  ...
  public void contactContent() throws Exception {
    ...
    String src = "zimbra";
    Assert.assertEquals(
        "(Q(CONTACT:zimbra) || Q(l.content:zimbra))", Query.toString(parser.parse(src)));
  ...
  public void quoted() throws Exception {
    ...
    Assert.assertEquals(
        "(Q(CONTACT:zimbra,quoted,test) || Q(l.content:zimbra,quoted,test))",
  ...
  public void quick() throws Exception {
    ...
    Assert.assertEquals(
        "Q(l.content:all,hands,meeting[*])", Query.toString(parser.parse("all hands meeting")));
    Assert.assertEquals(
        "Q(l.content:all,hands,meeting[*])", Query.toString(parser.parse("all hands meeting*")));
  }
}
