...
public final class TextQueryTest {
  ...
  public void sortByHasAttach() throws Exception {
    ...
    Message msgWithAttach =
        mbox.addMessage(
            null,
            MailboxTestUtil.generateMessageWithAttachment("test subject has attach"),
            dopt,
            null);
    ...
    Assert.assertEquals(
        "correct hit ascending has attachments", msgWithAttach.getId(), msgWithAttachId);
    ...
    Assert.assertEquals(
        "correct hit descending has attachments", msgWithAttach.getId(), msgWithAttachId);
  }
  ...
  public void sortByIsFlagged() throws Exception {
    ...
    dopt =
        new DeliveryOptions()
            ...
            .setFlags(Flag.BITMASK_UNREAD | Flag.BITMASK_FLAGGED);
    Message msgWithFlag =
        mbox.addMessage(null, MailboxTestUtil.generateMessage("test subject flag"), dopt, null);
  ...
  public void sortByPriority() throws Exception {
    ...
    Message msg =
        mbox.addMessage(null, MailboxTestUtil.generateMessage("test subject"), dopt, null);
    Message msgWithHighPri =
        mbox.addMessage(
            null,
            MailboxTestUtil.generateHighPriorityMessage("test subject is HI-PRI"),
            dopt,
            null);
    Message msgWithLowPri =
        mbox.addMessage(
            null,
            MailboxTestUtil.generateLowPriorityMessage("test subject is LOW-PRI"),
            dopt,
            null);
  ...
  static SearchResponse doSearch(SearchRequest request, Account acct) throws Exception {
    Element response =
        new Search()
            .handle(
                JaxbUtil.jaxbToElement(request, Element.XMLElement.mFactory),
