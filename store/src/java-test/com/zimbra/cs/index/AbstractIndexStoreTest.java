...
public abstract class AbstractIndexStoreTest {
  ...
  /**
   * Override this for any Index Store specific cleanup. Note that for Mock Provisioning, deleting
   * an account does not currently cleanup the index.
   */
  ...
  public void termQuery() throws Exception {
    ...
    result =
        searcher.search(
            new TermQuery(new Term(LuceneFields.L_CONTACT_DATA, "test@zimbra.com")), 100);
    ...
    Assert.assertEquals(
        String.valueOf(contact.getId()), getBlobIdForResultDoc(searcher, result, 0));
  ...
  public void filteredTermQuery() throws Exception {
    ...
    result =
        searcher.search(
            new TermQuery(new Term(LuceneFields.L_CONTACT_DATA, "zimbra.com")), filter, 100);
    Assert.assertNotNull("searcher.search result object - searching for zimbra.com", result);
    ZimbraLog.test.debug(
        "Result for search for 'zimbra.com', filtering by IDs\n%s", result.toString());
  ...
  public void sortedFilteredTermQuery() throws Exception {
    ...
    result =
        srchr.search(
            new TermQuery(new Term(LuceneFields.L_CONTACT_DATA, "zimbra.com")), filter, 100, sort);
    Assert.assertNotNull("searcher.search result object - searching for zimbra.com", result);
    ZimbraLog.test.debug(
        "Result for search for 'zimbra.com', filtering by IDs 2,4 & 5\n%s", result.toString());
    Assert.assertEquals("Number of hits", 2, result.getTotalHits());
    Assert.assertEquals(
        "Match Blob ID 1", String.valueOf(con2.getId()), getBlobIdForResultDoc(srchr, result, 0));
    Assert.assertEquals(
        "Match Blob ID 2", String.valueOf(con4.getId()), getBlobIdForResultDoc(srchr, result, 1));
    ...
    Assert.assertNotNull("searcher.search result object - searching for zimbra.com", result);
    ZimbraLog.test.debug(
        "Result for search for 'zimbra.com' sorted reverse, filter by IDs\n%s", result.toString());
    Assert.assertEquals("Number of hits", 2, result.getTotalHits());
    Assert.assertEquals(
        "Match Blob ID 1", String.valueOf(con4.getId()), getBlobIdForResultDoc(srchr, result, 0));
    Assert.assertEquals(
        "Match Blob ID 2", String.valueOf(con2.getId()), getBlobIdForResultDoc(srchr, result, 1));
    srchr.close();
  }
  ...
  public void booleanQuery() throws Exception {
    ...
    Mailbox mbox =
        MailboxManager.getInstance().getMailboxByAccountId(MockProvisioning.DEFAULT_ACCOUNT_ID);
    Contact contact =
        createContact(mbox, "First", "Last", "f.last@zimbra.com", "Software Development Engineer");
    ...
    ZimbraLog.test.debug(
        "Result for search [hits=%d]:%s", result.getTotalHits(), result.toString());
  ...
  public void multiPhraseQuery() throws Exception {
    ...
    createContact(mbox, "Non", "Match", "nOn.MaTchiNg@zimbra.com");
    Contact contact1 =
        createContact(mbox, "Paul", "AA", "aa@example.net", "Software Development Engineer");
    createContact(mbox, "Jane", "BB", "bb@example.net", "Software Planning Engineer");
    Contact contact2 =
        createContact(mbox, "Peter", "CC", "cc@example.net", "Software Dev Engineer");
    createContact(mbox, "Avril", "DD", "dd@example.net", "Software Architectural Engineer");
    Contact contact3 =
        createContact(mbox, "Leo", "EE", "ee@example.net", "Software Developer Engineer");
    Contact contact4 =
        createContact(mbox, "Wow", "DD", "dd@example.net", "Softly Development Engineer");
    createContact(mbox, "Given", "Surname", "GiV.SurN@zimbra.com");
    mbox.index.indexDeferredItems(); // Make sure all indexing has been done
    ...
    // Lower case required for each term for Lucene
    Term[] firstWords = {
      new Term(LuceneFields.L_CONTENT, "softly"), new Term(LuceneFields.L_CONTENT, "software")
    };
    pquery.add(firstWords);
    Term[] secondWords = {
      new Term(LuceneFields.L_CONTENT, "dev"),
  ...
  public void prefixQuery() throws Exception {
    ...
    ZimbraIndexSearcher searcher = index.openSearcher();
    ZimbraTopDocs result =
        searcher.search(new PrefixQuery(new Term(LuceneFields.L_CONTACT_DATA, "ab")), 100);
    ...
    ZimbraLog.test.debug(
        "Contact1ID=%s Contact2ID=%s match1id=%s match2id=%s",
    ...
    if (contact1Id.equals(match1Id)) {
      ...
      Assert.fail(
          String.format(
              "Contact 1 ID [%s] doesn't match either [%s] or [%s]",
  ...
  public void termRangeQuery() throws Exception {
    ...
    TermRangeQuery query =
        new TermRangeQuery(
            LuceneFields.L_FIELD, "email:aba@zimbra.com", "email:abz@zimbra.com", false, true);
    ZimbraTopDocs result = searcher.search(query, 100);
    Assert.assertNotNull("searcher.search result object", result);
  ...
  public void deleteDocument() throws Exception {
    ...
    ZimbraTopDocs result =
        searcher.search(new TermQuery(new Term(LuceneFields.L_CONTACT_DATA, "@zimbra.com")), 100);
    Assert.assertNotNull("searcher.search result object - searching for '@zimbra.com'", result);
  ...
  private void checkNextTerm(TermFieldEnumeration fields, Term term) {
    Assert.assertTrue(
        "fields.hasMoreElements() value when expecting:" + term.toString(),
        fields.hasMoreElements());
    BrowseTerm browseTerm = fields.nextElement();
    Assert.assertNotNull(
        "fields.nextElement() value when expecting:" + term.toString(), browseTerm);
    ZimbraLog.test.debug(
        "Expecting %s=%s value is %s docFreq=%d",
  ...
  private void checkNextTermFieldType(TermFieldEnumeration fields, String field) {
    Assert.assertTrue(
        "fields.hasMoreElements() value when expecting:" + field, fields.hasMoreElements());
    BrowseTerm browseTerm = fields.nextElement();
    Assert.assertNotNull("fields.nextElement() value when expecting:" + field, browseTerm);
    ZimbraLog.test.debug(
        "Expecting %s=?anyvalue? value is %s docFreq=%d",
        field, browseTerm.getText(), browseTerm.getFreq());
  }

  private void checkAtEnd(TermFieldEnumeration fields, String field) {
    Assert.assertFalse(
        "fields.hasMoreElements() at end of list for field:" + field, fields.hasMoreElements());
    try {
      fields.nextElement();
      Assert.fail(
          "fields.nextElement() at end of list for field:" + field + " contact data succeeded");
    } catch (NoSuchElementException ex) {
    }
  }

  /**
   * The result of getTermsForField can be good for seeing the effects of {@code ZimbraAnalyzer} on
   * how fields get tokenized. TODO: Add tests for different types of tokenizers.
   *
   * @throws Exception
   */
  @Test
  public void termEnum() throws Exception {
    ...
    try (TermFieldEnumeration fields =
        searcher.getIndexReader().getTermsForField(LuceneFields.L_CONTACT_DATA, "")) {
    ...
    try (TermFieldEnumeration fields =
        searcher.getIndexReader().getTermsForField(LuceneFields.L_CONTENT, "")) {
      // l.content values:
    ...
    try (TermFieldEnumeration fields =
        searcher.getIndexReader().getTermsForField(LuceneFields.L_FIELD, "")) {
    ...
    try (TermFieldEnumeration fields =
        searcher.getIndexReader().getTermsForField(LuceneFields.L_PARTNAME, "")) {
      checkNextTerm(fields, new Term(LuceneFields.L_PARTNAME, "CONTACT"));
      checkAtEnd(fields, LuceneFields.L_PARTNAME);
    }
    try (TermFieldEnumeration fields =
        searcher.getIndexReader().getTermsForField(LuceneFields.L_H_TO, "")) {
    ...
    try (TermFieldEnumeration fields =
        searcher.getIndexReader().getTermsForField(LuceneFields.L_H_TO, "tess")) {
      checkNextTerm(fields, new Term(LuceneFields.L_H_TO, "test1"));
      checkNextTerm(fields, new Term(LuceneFields.L_H_TO, "test1@zimbra.com"));
      checkNextTerm(fields, new Term(LuceneFields.L_H_TO, "test2"));
      checkNextTerm(fields, new Term(LuceneFields.L_H_TO, "test2@zimbra.com"));
    ...
    try (TermFieldEnumeration fields =
        searcher.getIndexReader().getTermsForField(LuceneFields.L_SORT_DATE, "")) {
    ...
    try (TermFieldEnumeration fields =
        searcher.getIndexReader().getTermsForField(LuceneFields.L_MAILBOX_BLOB_ID, "")) {
      checkNextTermFieldType(fields, LuceneFields.L_MAILBOX_BLOB_ID);
      checkNextTermFieldType(fields, LuceneFields.L_MAILBOX_BLOB_ID);
      // TODO:  ElasticSearch has more.  Investigate?  Believe it relates to fact that is a number
      // field
  ...
  private Contact createContact(Mailbox mbox, String email) throws ServiceException {
    ...
    return mbox.createContact(
        null,
        new ParsedContact(Collections.singletonMap(ContactConstants.A_email, email)),
        folder.getId(),
        null);
  }
  ...
  private Contact createContact(
      Mailbox mbox, String firstName, String lastName, String email, String jobTitle)
      throws ServiceException {
    Folder folder = mbox.getFolderById(null, Mailbox.ID_FOLDER_CONTACTS);
    Map<String, Object> fields;
    fields =
  ...
  private static String getBlobIdForResultDoc(
      ZimbraIndexSearcher searcher, ZimbraTopDocs result, int index) throws IOException {
    return searcher
        .doc(result.getScoreDoc(index).getDocumentID())
        .get(LuceneFields.L_MAILBOX_BLOB_ID);
  }
}
