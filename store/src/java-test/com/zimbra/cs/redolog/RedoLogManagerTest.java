...
public class RedoLogManagerTest {
  ...
  public void internalState() throws Exception {
    ...
    Assert.assertFalse("crashRecovery == true", redoLogManager.getInCrashRecovery());
  }
  ...
  public void rolloverIncrementsLogSequence() throws Exception {
    ...
    Assert.assertNull("No rollover occured, so previous log file should be NULL.", previousFile);
    ...
    Assert.assertEquals(
        ...
        currentSequence + 1,
        redoLogManager.getCurrentLogSequence());
    Assert.assertTrue(previousFile.getName().contains("seq" + currentSequence + ".log"));
  }
}
