...
public class CreateMessageTest {
  ...
  public void startSetsTimestamp() {
    Assert.assertEquals("receivedDate != -1 before start", -1, op.mReceivedDate);
    op.start(7);
  ...
  public void serializeDeserialize() throws Exception {
    ...
    Assert.assertEquals(
        ...
        "",
        CharStreams.toString(new InputStreamReader(op.getAdditionalDataStream(), Charsets.UTF_8)));
  }
  ...
  public void redo() throws Exception {
    ...
    Message msg = mbox.getMessageById(op.getOperationContext(), mbox.getLastItemId());
    Assert.assertEquals("subject != test", "test", msg.getSubject());
    Assert.assertEquals(
        "sender != bob@example.com", "Bob Evans <bob@example.com>", msg.getSender());
