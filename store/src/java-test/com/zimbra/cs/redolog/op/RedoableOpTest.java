...
public class RedoableOpTest extends EasyMockSupport {
  ...
  public void setUp() {
    ...
    op =
        createMockBuilder(RedoableOp.class)
  ...
  public void startLogCommit() {
    ...
    op.commit();
    Assert.assertNull("Commit clears byte array.", op.mSerializedByteArrayVector);
    verifyAll();
  }
  ...
  public void serialize() throws Exception {
    ...
    EasyMock.expect(op.getTransactionId()).andReturn(new TransactionId(1, 2));
    op.serializeData(EasyMock.anyObject(RedoLogOutput.class));
    replayAll();
    InputStream out = op.getInputStream();
    Assert.assertNotNull("getInputStream sets up internal vector.", op.mSerializedByteArrayVector);
    Assert.assertEquals("available bytes != 46", 46, out.available());
    byte[] bytes = new byte[RedoableOp.REDO_MAGIC.length()];
    out.read(bytes);
    Assert.assertEquals(
        "REDO_MAGIC missing in serialize.", RedoableOp.REDO_MAGIC, new String(bytes));
    verifyAll();
  }
