...
public class FileLogReaderTest {
  ...
  public void setUp() throws Exception {
    ...
    RedoLogManager mockRedoLogManager = EasyMock.createNiceMock(RedoLogManager.class);
    logReader = new FileLogReader(logfile);
    logWriter = new FileLogWriter(mockRedoLogManager, logfile, 0 /* no fsync thread */);
  }

  private void writeOp(TransactionId id) throws IOException {
    logWriter.open();
    RedoableOp op =
        EasyMock.createMockBuilder(CopyItem.class)
            .withConstructor()
  ...
  public void openReadClose() throws Exception {
    ...
    logReader.open();
    Assert.assertEquals(
        "Read file to unexpected position", FileHeader.HEADER_LEN, logReader.position());
    RedoableOp op = logReader.getNextOp();
    Assert.assertEquals(FileHeader.HEADER_LEN, logReader.getLastOpStartOffset());
    Assert.assertEquals(
        "mismateched transactionid", op.getTransactionId(), new TransactionId(7, 3));
  ...
  public void skipsJunkInFile() throws Exception {
    ...
    Assert.assertEquals("TransactionId mismatch", op.getTransactionId(), new TransactionId(7, 3));

    op = logReader.getNextOp();
    Assert.assertEquals("TransactionId mismatch", op.getTransactionId(), new TransactionId(8, 4));
  ...
  public void readBeforeOpen() throws Exception {
    assertThrows(IOException.class, () -> logReader.getNextOp());
  }
  ...
  public void junkFileFails() throws Exception {
    ...
    try {
      ...
      Assert.assertTrue(
          "Cause should contain 'missing magic bytes' " + "Got: " + e.getCause().getMessage(),
