...
public final class DbSearchTest {
  ...
  public void sortByAttachment() throws Exception {
    ...
    List<DbSearch.Result> result =
        new DbSearch(mbox)
            .search(
                conn,
                new DbSearchConstraints.Leaf(),
                SortBy.ATTACHMENT_ASC,
                0,
                100,
  ...
  public void subjectCursor() throws Exception {
    ...
    constraints.cursorRange =
        new DbSearchConstraints.CursorRange(
            "SUBJECT0000000102", true, null, false, SortBy.SUBJ_ASC);
    List<DbSearch.Result> result =
        new DbSearch(mbox)
            .search(conn, constraints, SortBy.SUBJ_ASC, 0, 100, DbSearch.FetchMode.ID);
    Assert.assertEquals(3, result.size());
  ...
  public void mdate() throws Exception {
    DbUtil.executeUpdate(
        conn,
        "INSERT INTO mboxgroup1.mail_item (mailbox_id, id, type, flags, date, change_date, size,"
            + " tags, mod_metadata, mod_content) VALUES(?, ?, ?, 0, ?, ?, 0, 0, 0, 0)",
        mbox.getId(),
        101,
        MailItem.Type.MESSAGE.toByte(),
        100,
    ...
    List<DbSearch.Result> result =
        new DbSearch(mbox)
            .search(conn, constraints, SortBy.DATE_ASC, 0, 100, DbSearch.FetchMode.ID);
  ...
  public void tag() throws Exception {
    ...
    DbUtil.executeUpdate(
        conn,
        "INSERT INTO mboxgroup1.tagged_item (mailbox_id, item_id, tag_id) " + "VALUES(?, ?, ?)",
        mbox.getId(),
        101,
        Flag.ID_UNREAD);
    DbUtil.executeUpdate(
    ...
    List<DbSearch.Result> result =
        new DbSearch(mbox).search(conn, constraints, SortBy.NONE, 0, 100, DbSearch.FetchMode.ID);
    Assert.assertEquals(2, result.size());
    Assert.assertEquals(101, result.get(0).getId());
    Assert.assertEquals(103, result.get(1).getId());
  ...
  public void tagInDumpster() throws Exception {
    DbUtil.executeUpdate(
        conn,
        "INSERT INTO mboxgroup1.mail_item_dumpster (mailbox_id, id, type, flags, unread, tag_names,"
            + " date, size, mod_metadata, mod_content) VALUES(?, ?, ?, ?, ?, ?, ?, ?, 0, 0)",
        mbox.getId(),
        101,
        MailItem.Type.MESSAGE.toByte(),
        0,
    ...
    constraints.addTag(mbox.getTagByName(null, "\\Unread"), true);
    List<DbSearch.Result> result =
        new DbSearch(mbox, true)
            .search(conn, constraints, SortBy.NONE, 0, 100, DbSearch.FetchMode.ID);
    Assert.assertEquals(2, result.size());
    Assert.assertEquals(101, result.get(0).getId());
    Assert.assertEquals(103, result.get(1).getId());
  ...
  public void caseInsensitiveSort() throws Exception {
    ...
    List<DbSearch.Result> result =
        new DbSearch(mbox)
            .search(conn, constraints, SortBy.NAME_ASC, 0, 100, DbSearch.FetchMode.ID);
    Assert.assertEquals(3, result.size());
    Assert.assertEquals(102, result.get(0).getId());
    Assert.assertEquals(103, result.get(1).getId());
    Assert.assertEquals(101, result.get(2).getId());
    ...
    result =
        new DbSearch(mbox)
            .search(conn, constraints, SortBy.NAME_DESC, 0, 100, DbSearch.FetchMode.ID);
    Assert.assertEquals(3, result.size());
    Assert.assertEquals(101, result.get(0).getId());
    Assert.assertEquals(103, result.get(1).getId());
