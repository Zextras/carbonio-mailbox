...
public class MigrateAttributesTest {
  ...
  private static Map<String, AttributeConverter> converters =
  ...
  public void testMigrationInfo() throws Exception {
    ...
    try {
      ...
      AttributeMigration migration =
          new AttributeMigration("testAttributeMigration", attrsToMigrate, source, null);
  ...
  public void testAttributeMigration() throws Exception {
    ...
    // DummyMigrationCallback will store attributes in InMemoryEphemeralStore, and track deletions
    // in deletedAttrs map
    MigrationCallback callback = new DummyMigrationCallback(destination);
    AttributeMigration.setCallback(callback);
    AttributeMigration migration =
    ...
    EphemeralResult result =
        destination.get(new EphemeralKey(Provisioning.A_zimbraAuthTokens, "1234"), location);
    ...
    result =
        destination.get(new EphemeralKey(Provisioning.A_zimbraCsrfTokenData, "crumb1"), location);
    assertEquals("data1", result.getValue());
    result =
        destination.get(new EphemeralKey(Provisioning.A_zimbraCsrfTokenData, "crumb2"), location);
  ...
  private void verifyAuthTokenEphemeralInput(
      EphemeralInput input, String token, String serverVersion, Long expiration) {
  ...
  private void verifyCsrfTokenEphemeralInput(
      EphemeralInput input, String crumb, String data, Long expiration) {
    EphemeralKey key = input.getEphemeralKey();
  ...
  public static class DummyMigrationCallback implements AttributeMigration.MigrationCallback {
    ...
    public void setEphemeralData(
        EphemeralInput input, EphemeralLocation location, String origKey, Object origValue)
