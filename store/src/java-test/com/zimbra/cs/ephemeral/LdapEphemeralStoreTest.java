...
/**
 * Tests the LdapEphemeralStore backend by verifying the LDAP attribute map that would be sent to
 * Provisioning.modifyAttrs() by the ZimbraLdapHelper.
 *
 * @author iraykin
 */
public class LdapEphemeralStoreTest {
  ...
  /**
   * helper class that stores data in an in-memory ephemeral store, while keeping track of the
   * attribute modification map that would otherwise be used to modify LDAP values with
