...
public class PermCacheManagerTest {
      ...
      while (true) {
          ...
          for (int j = 0; j < grantees.length; j++) {
            for (int k = 0; k < rights.size(); k++) {
              ...
              for (CachedPermission cachedPerm : CachedPermission.values()) {
                if (cachedPerm == CachedPermission.NOT_CACHED) continue;
        ...
        synchronized (lock) {
          if (numIters % 2 == 0) {
            synchronized (lock) {
              System.out.println(
                  "Thread " + id + " iter = " + numIters + " invalidating entire cache");
              pcm.invalidateCache();
            }
          }
        }
  ...
  private List<Right> getAllCacheableRights() throws Exception {
    ...
    */
    ...
    for (Right r : RightManager.getInstance().getAllUserRights().values()) {
      if (r.isCacheable()) cacheableRights.add(r);
    }
  ...
  public void testCachedPerms() throws Exception {
    ...
    for (int rightIdx = 0; rightIdx < cacheableRights.size(); rightIdx++) {
      ...
      for (CachedPermission expectedPerm : CachedPermission.values()) {
        if (expectedPerm == CachedPermission.NOT_CACHED) continue;
        ...
        for (int otherRightIdx = 0; otherRightIdx < cacheableRights.size(); otherRightIdx++) {
          if (otherRightIdx == rightIdx) continue;
          ...
          if (otherRightIdx < rightIdx)
            expectedPermForOtherRights =
                CachedPermission.DENIED; // last cached perm for the right in the test
          else expectedPermForOtherRights = CachedPermission.NOT_CACHED;
  ...
  public void testMaxAge() throws Exception {
    ...
    // wait for TTL
    long waitFor =
        acl_cache_target_maxage * Constants.MILLIS_PER_MINUTE
            + 1000; // plus one second for the cusion
