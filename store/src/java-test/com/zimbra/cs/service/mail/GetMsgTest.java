...
public class GetMsgTest {
  ...
  public void testHandle() throws Exception {
    ...
    ParsedDateTime e =
        ParsedDateTime.fromUTCTime(System.currentTimeMillis() + (30 * 60 * 1000) + utc, ical);
    invite.setDtStart(s);
    invite.setDtEnd(e);
    ...
    MsgSpec msgSpec =
        new MsgSpec(acct1.getId() + ":" + inviteData.calItemId + "-" + inviteData.invId);
    msgSpec.setWantHtml(true);
    msgSpec.setNeedCanExpand(true);
    GetMsgRequest getMsgReq = new GetMsgRequest(msgSpec);
    Element response =
        new GetMsg()
            .handle(JaxbUtil.jaxbToElement(getMsgReq), ServiceTestUtil.getRequestContext(acct1));
    Element organizer =
        response.getElement("m").getElement("inv").getElement("comp").getElement("or");
    ...
    response =
        new CreateMountpoint()
            .handle(JaxbUtil.jaxbToElement(createMpReq), ServiceTestUtil.getRequestContext(acct2));
    ...
    response =
        new GetMsg()
            .handle(
                JaxbUtil.jaxbToElement(getMsgReq), ServiceTestUtil.getRequestContext(acct2, acct1));
