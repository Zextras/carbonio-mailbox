...
public final class ParseMimeMessageTest {
  ...
  public void customMimeHeader() throws Exception {
    ...
    try {
      ...
      Assert.assertEquals(
          "invalid request: header 'X-Zimbra-Test' not allowed", expected.getMessage());
    }
    ...
    mm =
        ParseMimeMessage.parseMimeMsgSoap(
            zsc, octxt, null, el, null, new ParseMimeMessage.MimeMessageData());
    Assert.assertEquals(
        "custom, =?utf-8?B?44Kr44K544K/44Og?=", mm.getHeader("X-Zimbra-Test", ", "));
  }
  ...
  public void attachPdfDocument() throws Exception {
    ...
    Document doc =
        mbox.createDocument(
            ...
            "testdoc",
            MimeConstants.CT_APPLICATION_PDF,
            "author",
            "description",
    ...
    Assert.assertEquals(
        MimeConstants.CT_APPLICATION_PDF, new ContentType(part.getContentType()).getContentType());
  }
  ...
  public void attachZimbraDocument() throws Exception {
    ...
    MimeBodyPart part = (MimeBodyPart) mmp.getBodyPart(1);
    Assert.assertEquals(
        MimeConstants.CT_TEXT_HTML, new ContentType(part.getContentType()).getContentType());
  }
  ...
  public void staleReference() throws Exception {
    ...
    Assert.assertEquals(
        "attached part content", "test2", firstLine((MimeBodyPart) multi.getBodyPart(1)));
  }
}
