...
public final class VCardTest {
  ...
  public void multiX() throws ServiceException {
    ...
    Assert.assertEquals(
        String.format("Number of unknown properties in %s", unknowns), 2, unknowns.size());
  ...
  public void multiX2() throws ServiceException {
    ...
    ListMultimap<String, VCardParamsAndValue> unknowns = Contact.decodeUnknownVCardProps(xprops);
    Assert.assertEquals(
        String.format("Number of unknown properties in %s", unknowns), 1, unknowns.size());
    List<VCardParamsAndValue> xfoos = unknowns.get("X-FOO");
  ...
  public void smallBusyMacAttach() throws ServiceException {
    ...
    Assert.assertEquals(
        String.format("Number of %s", "X-BUSYMAC-ATTACH"), 1, busymacAttaches.size());
    VCardParamsAndValue attach = busymacAttaches.get(0);
    Assert.assertTrue(
        String.format(
            "Value fo X-BUSYMAC-ATTACH property %s starts as expected", attach.getValue()),
        attach.getValue().startsWith("AAABAAEAEBAAAAEAIABoBAAAF"));
    Assert.assertEquals(
        String.format("X-BUSYMAC-ATTACH params=%s number", attach.getParams()),
        2,
        attach.getParams().size());
  }
