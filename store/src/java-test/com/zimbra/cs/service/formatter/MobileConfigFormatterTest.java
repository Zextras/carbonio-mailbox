...
public class MobileConfigFormatterTest {
  ...
  public void testGetDictForCaldav() throws Exception {
    ...
    Node dict =
        MobileConfigFormatter.getDictForCaldavAndCarddav(
            document, EMAIL_PART, user, server, ConfigType.CALDAV, domain);
    ...
    String expected =
        "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>\n"
            + "<!DOCTYPE parent PUBLIC \"-//Apple//DTD PLIST 1.0//EN\""
            + " \"http://www.apple.com/DTDs/PropertyList-1.0.dtd\">\n"
            + "<parent>\n"
            + "    <dict>\n"
  ...
  public void testGetDictForCarddav() throws Exception {
    ...
    Node dict =
        MobileConfigFormatter.getDictForCaldavAndCarddav(
            document, EMAIL_PART, user, server, ConfigType.CARDDAV, domain);
    fakeParent.appendChild(dict);
    document.appendChild(fakeParent);
  ...
  public void testGetDictForImap() throws Exception {
    ...
    Node dict =
        MobileConfigFormatter.getDictForImap(
            document, EMAIL_PART, user, server, ConfigType.IMAP, domain);
    fakeParent.appendChild(dict);
    document.appendChild(fakeParent);
