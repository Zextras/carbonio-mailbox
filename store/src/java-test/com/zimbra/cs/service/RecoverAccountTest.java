...
public class RecoverAccountTest {
  ...
  public void testGetRecoveryEmail() throws Exception {
    ...
    Assert.assertEquals(
        PrefPasswordRecoveryAddressStatus.verified, acct1.getPrefPasswordRecoveryAddressStatus());
  ...
  public void testSendRecoveryCode() throws Exception {
    ...
    Account recoveryAccount =
        Provisioning.getInstance().get(Key.AccountBy.name, "testRecovery@zimbra.com");
  ...
  public void testResendRecoveryCodePositiveAndNegativeScenarios() throws Exception {
    ...
    try {
      new RecoverAccount()
          .handle(req, ServiceTestUtil.getRequestContext(acct1)); // resend count = 0
    } catch (ServiceException se) {
      Assert.fail("ServiceException should not be thrown.");
    }
    ...
    try {
      new RecoverAccount()
          .handle(req, ServiceTestUtil.getRequestContext(acct1)); // resend count = 1
      new RecoverAccount()
          .handle(req, ServiceTestUtil.getRequestContext(acct1)); // resend count = 2
      new RecoverAccount()
          .handle(req, ServiceTestUtil.getRequestContext(acct1)); // resend count = 3
    } catch (ServiceException se) {
      Assert.fail("ServiceException should not be thrown.");
    }
    try {
      new RecoverAccount()
          .handle(
              req,
              ServiceTestUtil.getRequestContext(acct1)); // resend count = 4, and it should fail
    ...
    try {
      ...
      Assert.assertEquals(
          "service exception: Password reset feature is suspended.", e.getMessage());
    }
