...
public class EnvelopeTest {
  ...
  public void testMailFromBackslash() {
    ...
    LmtpEnvelope env = new LmtpEnvelope();
    LmtpAddress sender =
        new LmtpAddress("<\"ti\\\\m\"@example.com>", new String[] {"BODY", "SIZE"}, null);
    LmtpAddress recipient = new LmtpAddress("<xyz@zimbra.com>", null, null);
    env.setSender(sender);
    env.addLocalRecipient(recipient);
  ...
  public void testVariable(String filterScript) {
    ...
    LmtpEnvelope env = new LmtpEnvelope();
    LmtpAddress sender =
        new LmtpAddress("<envelope_from@example.com>", new String[] {"BODY", "SIZE"}, null);
    LmtpAddress recipient = new LmtpAddress("<testEnv@zimbra.com>", null, null);
    env.setSender(sender);
    env.addLocalRecipient(recipient);
  ...
  public void testCompareEmptyStringWithAsciiNumeric() {
    String filterScript =
        "require [\"envelope\", \"comparator-i;ascii-numeric\"];\n"
  ...
  public void testTo_Alias() {
    ...
    try {
      Provisioning prov = Provisioning.getInstance();
      Account account =
          prov.createAccount("original1@zimbra.com", "secret", new HashMap<String, Object>());
      RuleManager.clearCachedRules(account);
      Mailbox mbox = MailboxManager.getInstance().getMailboxByAccount(account);
