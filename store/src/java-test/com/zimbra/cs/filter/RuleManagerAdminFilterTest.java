...
public final class RuleManagerAdminFilterTest {
  String scriptAdminBefore =
      "require [\"tag\", \"log\"];\n" + "if true {\n" + "  tag \"admin-defined-before\";\n" + "}";

  String scriptAdminBeforeStop =
      "require [\"tag\", \"log\"];\n"
          + "if true {\n"
          + "  tag \"admin-defined-before\";\n"
          + "  stop;\n"
  ...
  /**
   * Checking backward compatibility: when only the user-defined sieve rule is set, the sieve filter
  ...
  public void fileintoAtAdminBefore() throws Exception {
    ...
    RuleManager.applyRulesToIncomingMessage(
        new OperationContext(mbox),
        mbox,
        new ParsedMessage(rawTest.getBytes(), false),
        0,
        account.getName(),
        new DeliveryContext(),
        Mailbox.ID_FOLDER_INBOX,
    ...
    Integer item =
        mbox.getItemIds(
                null, mbox.getFolderByName(null, Mailbox.ID_FOLDER_USER_ROOT, "foo").getId())
  ...
  */
  ...
  public void requireText() throws Exception {
    ...
    RuleManager.applyRulesToIncomingMessage(
        new OperationContext(mbox),
        mbox,
        new ParsedMessage("Subject: require abc def\n".getBytes(), false),
        0,
        account.getName(),
        new DeliveryContext(),
        Mailbox.ID_FOLDER_INBOX,
