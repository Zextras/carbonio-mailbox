  ...
  public void proofpoint() throws Exception {
    List<ItemId> ids =
        RuleManager.applyRulesToIncomingMessage(
            ...
            new ParsedMessage(
                ("To: list@zimbra.com\n"
                     + "X-Proofpoint-Spam-Details: rule=tag_notspam policy=tag score=0 spamscore=0"
                     + " ipscore=0 suspectscore=49 phishscore=0 bulkscore=100 adultscore=0"
                     + " classifier=spam adjust=0 reason=mlx engine=6.0.2-1012030000"
                     + " definitions=main-1108230088\n"
                     + "Subject: bulk")
                    .getBytes(),
                false),
            0,
