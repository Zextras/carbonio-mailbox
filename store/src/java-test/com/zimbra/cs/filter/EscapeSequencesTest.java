...
public final class EscapeSequencesTest {
  ...
  public void testEnvelopeEscape1() {
    doTestEnvelopeEscapePattern(
        "require \"envelope\";\n"
            + "if envelope :all :comparator \"i;ascii-casemap\" :matches \"from\""
            + " \"user\\*@example.com\" {\n"
            + "discard;\n"
            + "}");
  }
  ...
  public void testHeaderEscape5() {
    doTestHeaderEscapePattern(
        "if header :comparator \"i;ascii-casemap\" :matches \"X-Header-0BackSlash\""
            + " \"test\\Sample\" {tag \"list\";}");
  }
  ...
  public void testHeaderEscape6() {
    doTestHeaderEscapePattern(
        "if header :comparator \"i;ascii-casemap\" :matches \"X-Header-1BackSlash\""
            + " \"test\\\\Sample\" {tag \"list\";}");
  }
  ...
  public void testHeaderEscape8() {
    doTestHeaderEscapePattern(
        "if header :comparator \"i;ascii-casemap\" :matches \"X-Header-2BackSlash\""
            + " \"test\\\\\\\\Sample\" {tag \"list\";}");
  }
  ...
  public void testHeaderEscape9() {
    doTestHeaderEscapePattern(
        "if header :comparator \"i;ascii-casemap\" :matches \"X-Header-DoubleQuote\""
            + " \"test\\\"Sample\" {tag \"list\";}");
  }
  ...
  public void doTestHeaderEscapePattern(String filterScript) {
    ...
    LmtpAddress sender =
        new LmtpAddress("<user123@zimbra.com>", new String[] {"BODY", "SIZE"}, null);
  ...
  public void doTestEnvelopeEscapePattern(String filterScript) {
    ...
    LmtpEnvelope env = new LmtpEnvelope();
    LmtpAddress sender =
        new LmtpAddress("<user123@example.com>", new String[] {"BODY", "SIZE"}, null);
    LmtpAddress recipient = new LmtpAddress("<test1@zimbra.com>", null, null);
    env.setSender(sender);
    env.addLocalRecipient(recipient);
