...
public class SetVariableTest {
    ...
    try {
      ...
      List<ItemId> ids =
          RuleManager.applyRulesToIncomingMessage(
              ...
              mbox,
              new ParsedMessage(raw.getBytes(), false),
              0,
              account.getName(),
              new DeliveryContext(),
  ...
  public void testReplacement() {
    ...
    testCases.put(
        "${fo\\\\o}",
        "${fo\\\\o}"); /* First it is converted to ${fo\o}, which is an illegal identifier ==> left verbatim. */
    ...
    testCases.put(
        "${a_b} ${def} ${c_d}hao!",
        "\u304a\u3057\u3089\u305b  Chao!"); // 1st valid variable, 2nd undefined, 3rd valid variable
    ...
    try {
      ...
      try {
        ...
        if (e instanceof SyntaxException) {
          ...
          assertTrue(
              se.getMessage().indexOf("Atleast 2 argument are needed. Found Arguments: [[hello]]")
                  > -1);
        }
      }
  ...
  public void testVariablesCombo() {
    try {
      ...
      Message msg = mbox.getMessageById(null, ids.get(0).getId());
      Assert.assertEquals(
          "Hello \u304a\u3057\u3089\u305b!!", ArrayUtil.getFirstElement(msg.getTags()));
    } catch (Exception e) {
      fail("No exception should be thrown");
    }
  }
  ...
  public void testStringTest() {
    try {
      ...
      RuleManager.clearCachedRules(account);
      filterScript =
          "require [\"variables\", \"comparator-i;ascii-numeric\"];\n"
              + "set :lower :upperfirst \"name\" \"Joe\";\n"
              + "if string :is :comparator \"i;ascii-numeric\" \"${name}\" [ \"Joe\", \"Hello\","
              + " \"User\" ]{\n"
              + "  tag \"sales-1\";\n"
              + "}if string :comparator \"i;ascii-numeric\" :is \"${name}\" [ \"Joe\", \"Hello\","
              + " \"User\" ]{\n"
              + "  tag \"sales-2\";\n"
  ...
  public void testSetMatchVarAndUseInHeaderSingleOccurrence() {
    try {
      ...
      filterScript =
          "require [\"variables\"];\n"
              + "if header :matches [\"To\", \"Cc\"]"
              + " [\"coyote@??M?.Example.com\",\"wile@**.com\"]{\n"
              + "  log \"Match 1 ${1}\";\n"
              + "  tag \"${3}\";\n"
              + "}\n";
      account.setMailSieveScript(filterScript);
    ...
    try {
      ...
      filterScript =
          "require [\"variables\"];\n"
              ...
              + " :comparator \"i;ascii-casemap\" :matches \"Subject\" \"test C-91"
              + " replac?header\";\n"
      ...
      Message msg = mbox.getMessageById(null, ids.get(0).getId());
      Assert.assertEquals(
          "[new]0: test C-91 replaceheader, 1: e",
          ArrayUtil.getFirstElement(msg.getMimeMessage().getHeader("X-New-Header")));
    } catch (Exception e) {
      fail("No exception should be thrown");
    }
  }
    ...
    try {
      ...
      filterScript =
          "require [\"variables\", \"tag\"];\n"
              + "set \"from_address_One\" \"user1@ykomiyam.local\";set \"from_address_Two\""
              + " \"user2@ykomiyam.local\";set \"from_header_name\" \"From\";set \"to_header_name\""
              + " \"To\";if address :comparator \"i;ascii-casemap\" :is \"From\""
              + " [\"${from_address_One}\",\"${from_address_Two}\"] {  tag \"KeyListTag\";\n"
              + "}if address :comparator \"i;ascii-casemap\" :is"
              + " [\"${from_header_name}\",\"${to_header_name}\"] \"user2@ykomiyam.local\" {  tag"
              + " \"HeaderListTag\";\n"
              + "}";
  ...
  public void testVariables_Envelope_KeyList() {
    ...
    String filterScript =
        "require [\"envelope\", \"variables\", \"tag\"];\n"
            + "set \"from_address\" \"tim@example.com\";"
            + "set \"to_address\" \"test1@zimbra.com\";"
  ...
  public void testVariables_AddHeader_FieldName_Value() {
    try {
      ...
      RuleManager.applyRulesToIncomingMessage(
          ...
          true);
      Integer itemId =
          mbox1.getItemIds(null, Mailbox.ID_FOLDER_INBOX).getIds(MailItem.Type.MESSAGE).get(0);
      int index = 0;
  ...
  public void testVariables_DeleteHeader_FieldName() {
    try {
      ...
      for (Enumeration<Header> enumeration = message.getMimeMessage().getAllHeaders();
  ...
  public void testVariables_ReplaceHeader_FieldName() {
    try {
      ...
      String filterScript =
          "require [\"editheader\"];\n"
              + "set \"header_name\" \"Subject\";set \"header_value\" \"example\";set \"new_value\""
              + " \"my subject\";replaceheader :newvalue \"${new_value}\" :contains"
  ...
  public void testSetMatchVarAndFileInto() {
    try {
      ...
      filterScript =
          "require [\"fileinto\", \"log\", \"variables\"];\n"
              + "set \"sub\" \"test\";\n"
  ...
  public void testSetMatchVarWithEnvelope() {
    ...
    try {
      ...
      filterScript =
          "require [\"log\", \"variables\", \"envelope\" ];\n"
  ...
  public void testSetMatchVarOutOfRange() {
    try {
      ...
      LmtpEnvelope env = new LmtpEnvelope();
      LmtpAddress sender =
          new LmtpAddress("<coyote@ACME.Example.COM>", new String[] {"BODY", "SIZE"}, null);
      LmtpAddress recipient = new LmtpAddress("<test@zimbra.com>", null, null);
      env.setSender(sender);
      env.addLocalRecipient(recipient);
      ...
      RuleManager.applyRulesToIncomingMessage(
          ...
          true);
      Integer itemId =
          mbox.getItemIds(null, Mailbox.ID_FOLDER_INBOX).getIds(MailItem.Type.MESSAGE).get(0);
  ...
  /**
   * To verify that the variable ${name} is evaluated only once. Before fix: when the match pattern
   * *${dollar}{sample}* was defined, first, the ${dollar} part was replaced to the $, then it was
   * treated as a parameter name ${sample}, and replaced to the test_text. As a result, the Subject
   * was mistakenly compared to the pattern *test text*. After fix: the Subject is compared to the
   * *${sample}*.
   */
  @Test
  ...
  /**
   * When the match pattern ${dollar} was defined as '$' and used in the header/address/envelope
   * test, this '$' should be treated as a string, not a part of the wild-card.
   */
  @Test
    ...
    try {
      ...
      filterScript =
          "require [\"variables\", \"envelope\"];\n"
              ...
              + "  tag \"header-contains\";\n"
              + "}\n"
              + "if address :all :matches :comparator \"i;ascii-casemap\" \"To\""
              + " \"${dollar}${val}@zimbra.com\" {\n"
              + "  tag \"address-matches\";\n"
              + "}\n"
              + "if address :all :is :comparator \"i;ascii-casemap\" \"To\""
              + " \"${dollar}${val}@zimbra.com\" {\n"
              + "  tag \"address-is\";\n"
              + "}\n"
              + "if address :all :contains :comparator \"i;ascii-casemap\" \"To\""
              + " \"${dollar}${val}\" {\n"
              + "  tag \"address-contains\";\n"
              + "}if envelope :all :matches :comparator \"i;ascii-casemap\" \"From\""
              + " \"${dollar}${val}@zimbra.com\" {\n"
              + "  tag \"envelope-matches\";\n"
              + "}\n"
              + "if envelope :all :is :comparator \"i;ascii-casemap\" \"From\""
              + " \"${dollar}${val}@zimbra.com\" {\n"
              + "  tag \"envelope-is\";\n"
              + "}\n"
              + "if envelope :all :contains :comparator \"i;ascii-casemap\" \"From\""
              + " \"${dollar}${val}\" {\n"
              + "  tag \"envelope-contains\";\n"
              + "}if not header :matches :comparator \"i;ascii-casemap\" \"Subject\""
              + " \"${dollar}${val}\" {\n"
              ...
              + "  tag \"header-not-is\";\n"
              + "}\n"
              + "if not header :contains :comparator \"i;ascii-casemap\" \"Subject\""
              + " \"${dollar}${val}\" {\n"
              + "  tag \"header-not-contains\";\n"
              + "}\n"
              + "if not address :all :matches :comparator \"i;ascii-casemap\" \"To\""
              + " \"${dollar}${val}@zimbra.com\" {\n"
              + "  tag \"address-not-matches\";\n"
              + "}\n"
              + "if not address :all :is :comparator \"i;ascii-casemap\" \"To\""
              + " \"${dollar}${val}@zimbra.com\" {\n"
              + "  tag \"address-not-is\";\n"
              + "}\n"
              + "if not address :all :contains :comparator \"i;ascii-casemap\" \"To\""
              + " \"${dollar}${val}\" {\n"
              + "  tag \"address-not-contains\";\n"
              + "}if not envelope :all :matches :comparator \"i;ascii-casemap\" \"From\""
              + " \"${dollar}${val}@zimbra.com\" {\n"
              + "  tag \"envelope-not-matches\";\n"
              + "}\n"
              + "if not envelope :all :is :comparator \"i;ascii-casemap\" \"From\""
              + " \"${dollar}${val}@zimbra.com\" {\n"
              + "  tag \"envelope-not-is\";\n"
              + "}\n"
              + "if not envelope :all :contains :comparator \"i;ascii-casemap\" \"From\""
              + " \"${dollar}${val}\" {\n"
      ...
      LmtpEnvelope env = new LmtpEnvelope();
      LmtpAddress sender =
          new LmtpAddress("<$xyz@zimbra.com>", new String[] {"BODY", "SIZE"}, null);
      LmtpAddress recipient = new LmtpAddress("<test@zimbra.com>", null, null);
      env.setSender(sender);
      env.addLocalRecipient(recipient);
    ...
    try {
      filterScript =
          "set \"dollar\" \"$\";\n"
              ...
              + "if string :matches :comparator \"i;ascii-casemap\" \"abc${dollar}{sample}\""
              + " \"*${sample}\" {\n"
              + "  addheader :last \"X-New-Header-1\" \"${1}\";\n"
              + "  addheader :last \"X-New-Header-2\" \"${2}\";\n"
              + "}\n"
              + "if string :contains :comparator \"i;ascii-casemap\" \"abc${dollar}{sample}\""
              + " \"${sample}\" {\n"
              + "  addheader :last \"X-New-Header-3\" \"contains\";\n"
              + "}\n"
              + "if string :is :comparator \"i;ascii-casemap\" \"abc${dollar}{sample}\""
              + " \"abc${sample}\" {\n"
              + "  addheader :last \"X-New-Header-4\" \"is\";\n"
              + "}\n"
              + "# set \"${dollar}{sample}\" in source\n"
              + "if string :matches :comparator \"i;ascii-casemap\" \"${dollar}{sample}\""
              + " \"*${sample}\" {\n"
              + "  addheader :last \"X-New-Header-5\" \"${1}\";\n"
              + "  addheader :last \"X-New-Header-6\" \"${2}\";\n"
              + "}\n"
              + "if string :contains :comparator \"i;ascii-casemap\" \"${dollar}{sample}\""
              + " \"${sample}\" {\n"
              ...
              + "}if string :is :comparator \"i;ascii-casemap\" \"${dollar}{sample}\""
              + " \"${dollar}{sample}\" {\n"
              ...
              + " Text\" {  addheader :last \"X-New-Header-18\" \"string test value (case"
              + " insensitive) eq\";\n"
              + "}"
              ...
              + "}if string :value \"eq\" :comparator \"i;ascii-casemap\" \"Test Text\""
              + " \"${sample}\" {  addheader :last \"X-New-Header-25\" \"string test value (case"
              + " insensitive) eq\";\n"
              + "}"
              ...
              + "}if string :value \"eq\" :comparator \"i;ascii-casemap\" \"${sample}\" \"Test"
              + " Text\" {  addheader :last \"X-New-Header-32\" \"string test value (case"
              + " insensitive) eq\";\n"
              + "}"
              ...
              + "}if string :value \"eq\" :comparator \"i;ascii-casemap\" \"Test Text\""
              + " \"${sample}\" {  addheader :last \"X-New-Header-39\" \"string test value (case"
              + " insensitive) eq\";\n"
              + "}"
              // :count operator
              ...
              + " [\"${sample}\",\"${hello}\"] \"2\" {  addheader :last \"X-New-Header-42\""
              + " \"string test count eq two\";\n"
              + "}if string :count \"eq\" :comparator \"i;ascii-numeric\""
              + " [\"${sample}\",\"${unknown}\"] [\"3\",\"2\",\"1\"] {  addheader :last"
              + " \"X-New-Header-43\" \"string test count eq one or two\";\n"
              + "}"
              // Default comparator
              ...
              + "if string :value \"eq\" :comparator \"i;ascii-numeric\" \"${sample}\""
              + " \"${undefined}\" {  addheader :last \"X-New-Header-46\" \"string test value"
              + " numeric eq positive infinity\";\n"
              + "}if string :count \"lt\" \"${number}\" \"\" {  addheader :last \"X-New-Header-47\""
              + " \"string test count numeric lt positive infinity\";\n"
              + "}";
      Account account = Provisioning.getInstance().getAccount(MockProvisioning.DEFAULT_ACCOUNT_ID);
      Mailbox mbox = MailboxManager.getInstance().getMailboxByAccount(account);
      RuleManager.clearCachedRules(account);
  ...
  public void testVarIndexWithLeadingZeroes() {
    try {
      ...
      filterScript =
          "require \"variables\";\n"
              + "if header :matches :comparator \"i;ascii-casemap\" \"Subject\" \"*C*a*c*ple*oo"
              + " *ge*yo 123 *56*89 sie*e*t\" { tag \"${001}\";}";
  ...
  public void testNegativeVarIndex() {
    try {
      ...
      filterScript =
          "require \"variables\";if header :matches :comparator \"i;ascii-casemap\" \"Subject\""
              + " \"*C*a*c*ple*oo *ge*yo 123 *56*89 sie*e*t\" { tag \"${-1}\";}";
      ...
      Message msg = mbox.getMessageById(null, ids.get(0).getId());
      String tag = msg.getTags()[0];
      // ${-1} is not valid variable index. So, no tag will be set because of a Sieve syntax
      // Exception
      fail("Should not reach here");
  ...
  public void testOutofRangeVarIndex() {
    try {
      ...
      filterScript =
          "require \"variables\";if header :matches :comparator \"i;ascii-casemap\" \"Subject\""
              + " \"*C*a*c*ple*oo *ge*yo 123 *56*89 sie*e*t\" { tag \"${10}\";}";
      ...
      Message msg = mbox.getMessageById(null, ids.get(0).getId());
      String tag = msg.getTags()[0];
      // ${10} is not valid variable index. So, no tag will be set because of a Sieve syntax
      // Exception
      fail("Should not reach here");
  ...
  public void testOutofRangeVarIndexWithLeadingZeroes() {
    try {
      ...
      filterScript =
          "require \"variables\";if header :matches :comparator \"i;ascii-casemap\" \"Subject\""
              + " \"*C*a*c*ple*oo *ge*yo 123 *56*89 sie*e*t\" { tag \"${0010}\";}";
      ...
      Message msg = mbox.getMessageById(null, ids.get(0).getId());
      String tag = msg.getTags()[0];
      // ${0010} is not valid variable index. So, no tag will be set because of a Sieve syntax
      // Exception
      fail("Should not reach here");
  ...
  public void testStringNumericNegativeTest() {
    try {
      ...
      filterScript =
          "require [\"variables\", \"tag\", \"comparator-i;ascii-numeric\"];\n"
              + "set \"negative\" \"-123\";\n"
