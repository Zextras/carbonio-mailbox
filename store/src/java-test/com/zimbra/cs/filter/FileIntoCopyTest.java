...
public class FileIntoCopyTest {
  ...
  public void testCopyFileIntoPattern1Test() {
    try {
      ...
      item =
          mbox.getItemIds(
                  null, mbox.getFolderByName(null, Mailbox.ID_FOLDER_USER_ROOT, "bar").getId())
              .getIds(MailItem.Type.MESSAGE)
              .get(0);
    ...
    try {
      ...
      Message msg =
          mbox.addMessage(
              ...
              new DeliveryContext());
      boolean filtered =
          RuleManager.applyRulesToExistingMessage(
              new OperationContext(mbox), mbox, msg.getId(), RuleManager.parse(filterScript));
