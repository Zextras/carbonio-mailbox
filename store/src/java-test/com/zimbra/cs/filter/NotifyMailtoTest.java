...
public class NotifyMailtoTest {
  ...
  String filterScript_NoMessage =
      "require [\"enotify\", \"variables\"];\n"
          + "set \"subject\" \"じゅげむ　じゅげむ　ごこうのすりきれ　かいじゃりすいぎょの　すいぎょうまつ　うんらいまつ　ふうらいまつ　くうねるところにすむところ"
          + "　やぶらこうじのぶらこうじ　ぱいぽ　ぱいぽ　ぱいぽのしゅーりんがん　しゅーりんがんのぐーりんだい　ぐーりんだいのぽんぽこぴーの　ぽんぽこなーの"
          + "　ちょうきゅうめいのちょうすけ\";\n"
          + "set \"contents\" text:\r\n"
          + "新しいメールが届きました。\n"
          + "You've got a mail.\n"
  ...
  String filterScript_MsgIDDate =
      "require [\"enotify\"];\n"
          + "notify :message \"NotifyMessage\"   :from \"test1@zimbra.com\"  "
  ...
  String filterScriptWithSpaceInHeaderName =
      "require [\"enotify\", \"variables\"];\n"
          ...
          + "    :from \"test1@zimbra.com\"\n"
          + "    :importance \"3\"\n"
          + "    \"mailto:test2@zimbra.com?to=test3@zimbra.com&Importance=High&X-Priority"
          + " =1&From=notifyfrom@example.com&body=${contents}\";  keep;\n"
          + "}\n";

  /**
   * Tests 'notify' filter rule: - Set :message (Subject field), :from (From field) and mechanism
   * (mailto:...) - Body of the notification message contains non-ascii characters - Additional
   * header fields are specified via mechanism (maito:) parameter
   */
  @Test
  public void test() {
    String sampleMsg =
    ...
    try {
      ...
      Integer item =
          mbox2.getItemIds(null, Mailbox.ID_FOLDER_INBOX).getIds(MailItem.Type.MESSAGE).get(0);
      ...
      for (Enumeration<Header> e = notifyMsg.getMimeMessage().getAllHeaders();
          e.hasMoreElements(); ) {
  ...
  public void testEmptyMessageBody() {
    ...
    try {
      ...
      LmtpEnvelope env = new LmtpEnvelope();
      LmtpAddress sender =
          new LmtpAddress("<xyz@example.com>", new String[] {"BODY", "SIZE"}, null);
  ...
  public void testNoMessage() {
    ...
    try {
      ...
      notifyMsg = mbox3.getMessageById(null, item);
      Assert.assertEquals(
          "じゅげむ　じゅげむ　ごこうのすりきれ　かいじゃりすいぎょの　すいぎょうまつ　うんらいまつ　ふうらいまつ　くうねるところにすむところ　やぶらこうじのぶらこうじ　ぱいぽ　ぱいぽ"
              + "　ぱいぽのしゅーりんがん　しゅーりんがんのぐーりんだい　ぐーりんだいのぽんぽこぴーの　ぽんぽこなーの　ちょうきゅうめいのちょうすけ",
  ...
  /**
   * == (triggering message) == LMTP; MAIL FROM: &lt;xyz@example.com&gt; LMTP; RCPT TO:
   * &lt;test1@zimbra.com&gt; LMTP; DATA From: xyz@example.com to: test1@zimbra.com
   *
   * <p>== (generated notification message) == SMTP; MAIL FROM: &lt;test1@zimbra.com&gt; SMTP; RCPT
   * TO: &lt;test2@zimbra.com&gt; SMTP; DATA From: test1@zimbra.com To: test2@zimbra.com Subject:
   * おしらせ
   *
   * <p>新しいメールが届きました。 You've got a mail. Chao!
   */
  ...
  public void testLoopDetected() {
    String sampleMsg =
        "Auto-Submitted: auto-notified; owner-email=\"test2@zimbra.com\"\n"
            + "from: xyz@example.com\n"
            + "Subject: [acme-users] [fwd] version 1.0 is out\n"
            + "to: foo@example.com, baz@example.com\n"
            + "cc: qux@example.com\n";
  ...
  /**
   * Verify the "header fields of the notification message that are normally related to an
   * individual new message (such as "Message-ID" and "Date") are generated for the notification
   * message in the normal manner, and MUST NOT be copied from the triggering message" (RFC
   * 5436-2.7)
   */
  @Test
  ...
  /**
   * Tests the 'valid_notify_method' test: verify that the given parameters do not have any syntax
   * error.
   */
  @Test
  ...
  /**
   * Tests the notify_method_capability ("online"/"yes") The mailto method returns true for only
   * "maybe"; return false for other values.
   */
  @Test
  ...
  public void testNotifyMethodCapability_Relational() {
    ...
    try {
      ...
      Message msg = mbox1.getMessageById(null, ids.get(0).getId());
      Assert.assertEquals(
          "notify_method_capability_eq_1", ArrayUtil.getFirstElement(msg.getTags()));
    } catch (Exception e) {
      fail("No exception should be thrown");
    }
  }
  ...
  public void testNotify_variable() {
    String filterScript =
        "require [\"enotify\", \"tag\", \"variables\", \"envelope\"];\n"
            ...
            + "if header   :matches  \"X-Header-With-Control-Chars\" \"*\" {set \"xheader\""
            + " \"${1}\";}\n"
    ...
    String sampleMsg =
        "Auto-Submitted: \"no\"\n"
            ...
            + "to: foo@example.com, baz@example.com\n"
            + "cc: qux@example.com\n"
            + "X-Header-With-Control-Chars:"
            + " =?utf-8?B?dGVzdCBIVAkgVlQLIEVUWAMgQkVMByBCUwggbnVsbAAgYWZ0ZXIgbnVsbA0K?=\n";

    String expectedNotifyMsg =
        "Hello test1@zimbra.com,\r\n"
            + "A new massage has arrived.\r\n"
            + "Sent: Tue, 11 Oct 2016 12:01:37  0900\r\n"
            + "From: xyz@example.com\r\n"
            + "Subject: [acme-users] [fwd] version 1.0 is out\r\n"
            + "X-Header-With-Control-Chars: test HT\t VT\u000b ETX\u0003 BEL\u0007 BS\u0008"
            + " null\u0000 after null";
  ...
  public void testBackslashEscapeSequence() {
    ...
    try {
      Account acct1 = Provisioning.getInstance().get(Key.AccountBy.name, "test1@zimbra.com");
      Account acct2 =
          Provisioning.getInstance().get(Key.AccountBy.name, "\"tes\\\\t2\"@zimbra.com");
