...
public class MimeTest {
  ...
  public void multipartReportRfc822Headers() throws Exception {
    ...
    for (MPartInfo child : children) {
      Assert.assertTrue(
          "Expected: " + child.getContentType(), types.remove(child.getContentType()));
    }
  ...
  public void fixBase64LineWrapping() throws Exception {
    ...
    Assert.assertTrue(
        "Text Part Content-Disposition header should be preserved",
        mpiText.mDisposition.equals("inline"));
    Assert.assertTrue(
        "HTML Part Content-Disposition header should be preserved",
