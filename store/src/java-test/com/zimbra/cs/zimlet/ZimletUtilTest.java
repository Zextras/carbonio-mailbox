...
public class ZimletUtilTest {
  ...
  public void testZimletRootDir() {
    ...
    try {
      assertEquals(
          LC.zimlet_directory.value() + "/org_my_zimlet",
          ZimletUtil.getZimletRootDir("org_my_zimlet").getPath());
    } catch (ZimletException e) {
    ...
    try {
      assertEquals(
          LC.zimlet_directory.value() + "/myzimlet",
          ZimletUtil.getZimletRootDir("myzimlet").getPath());
    } catch (ZimletException e) {
      fail("Should not throw ZimletException for good zimlet name");
    }
    ...
    try {
      assertEquals(
          LC.zimlet_directory.value() + "/my.zimlet",
          ZimletUtil.getZimletRootDir("my.zimlet").getPath());
    } catch (ZimletException e) {
      fail("Should not throw ZimletException for good zimlet name");
    }
  }
