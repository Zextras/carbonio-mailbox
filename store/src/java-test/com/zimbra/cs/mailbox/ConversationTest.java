...
public class ConversationTest {
  ...
  public void delete() throws Exception {
    ...
    DeliveryOptions dopt =
        new DeliveryOptions().setFolderId(Mailbox.ID_FOLDER_TRASH).setConversationId(-msgId);
    mbox.addMessage(null, MailboxTestUtil.generateMessage("Re: test subject"), dopt, null);
    Message msg3 =
        mbox.addMessage(null, MailboxTestUtil.generateMessage("Fwd: test subject"), dopt, null);
    ...
    Assert.assertEquals(
        "1 message remaining in conv (cache)", 1, mbox.getConversationById(null, convId).getSize());

    // clear the cache and make sure the counts are correct in the DB as well
    mbox.purge(MailItem.Type.CONVERSATION);
    Assert.assertEquals(
        "1 message remaining in conv (DB)", 1, mbox.getConversationById(null, convId).getSize());
  ...
  public void expiry() throws Exception {
    Account account =
        Provisioning.getInstance().get(AccountBy.id, MockProvisioning.DEFAULT_ACCOUNT_ID);
    ...
    DeliveryOptions dopt =
        new DeliveryOptions().setFolderId(Mailbox.ID_FOLDER_TRASH).setConversationId(-msgId);
    mbox.addMessage(
        null, MailboxTestUtil.generateMessage("Re: test subject").setReceivedDate(old), dopt, null);
    Message msg3 =
        mbox.addMessage(
            null,
            MailboxTestUtil.generateMessage("Fwd: test subject").setReceivedDate(old),
            dopt,
            null);
    ...
    Assert.assertEquals(
        "empty Trash folder", 0, mbox.getFolderById(null, Mailbox.ID_FOLDER_TRASH).getSize());
    Assert.assertEquals(
        "1 message remaining in conv (cache)", 1, mbox.getConversationById(null, convId).getSize());
