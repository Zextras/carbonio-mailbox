...
public final class FolderTest {
  ...
  public void defaultFolderFlags() throws Exception {
    ...
    try {
      ...
      account.setDefaultFolderFlags(
          null); // don't leave account in modified state since other tests (such as create) assume
      // no default flags
    }
  }
  ...
  public void deleteParent() throws Exception {
    Mailbox mbox =
        MailboxManager.getInstance().getMailboxByAccountId(MockProvisioning.DEFAULT_ACCOUNT_ID);
    Folder parent =
        mbox.createFolder(null, "/" + "deleteParent - parent", new Folder.FolderOptions());
    int parentId = parent.getId();
    Folder child =
        mbox.createFolder(null, "deleteParent - child", parent.getId(), new Folder.FolderOptions());
  ...
  public void manySubfolders() throws Exception {
    ...
    for (int i = 1; i <= NUM_LEVELS; i++) {
      Folder folder =
          mbox.createFolder(null, "manySubfolders " + i, parentId, new Folder.FolderOptions());
      if (i == 1) {
        top = folder;
      }
  ...
  /**
   * Deletes a folder that contains messages in a conversation. Confirms that the conversation size
   * was correctly decremented.
   */
  @Test
  ...
  public void create() throws Exception {
    ...
    Assert.assertEquals(
        "correct attributes", Folder.FOLDER_DONT_TRACK_COUNTS, folder.getAttributes());
