...
public final class ContactAutoCompleteTest {
  ...
  public void rankingTestContactWithSameEmailDifferentDisplayName() throws Exception {
    ...
    ContactRankings.increment(
        mbox.getAccountId(), Collections.singleton(new InternetAddress("testauto@zimbra.com")));
    ContactRankings.increment(
        mbox.getAccountId(), Collections.singleton(new InternetAddress("testauto@zimbra.com")));
  ...
  public void autocompleteTestNonExistingContact() throws Exception {
    ...
    Mailbox mbox =
        MailboxManager.getInstance().getMailboxByAccountId(MockProvisioning.DEFAULT_ACCOUNT_ID);
    ContactRankings.increment(
        mbox.getAccountId(), Collections.singleton(new InternetAddress("noex@zimbra.com")));
    ContactRankings.increment(
        mbox.getAccountId(), Collections.singleton(new InternetAddress("noex@zimbra.com")));
    ContactAutoComplete autocomplete =
        new ContactAutoComplete(mbox.getAccount(), new OperationContext(mbox));
