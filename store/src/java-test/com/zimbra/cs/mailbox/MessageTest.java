...
public final class MessageTest {
  ...
  public void indexRawMimeMessage() throws Exception {
    Account account =
        Provisioning.getInstance().getAccountById(MockProvisioning.DEFAULT_ACCOUNT_ID);
  ...
  public void getSortRecipients() throws Exception {
    ...
    Message msg2 =
        mbox.addMessage(
            null,
            new ParsedMessage(
                "From: from2@zimbra.com\r\nTo: to2 <to2@zimbra.com>".getBytes(), false),
            opt,
            null);
    Message msg3 =
        mbox.addMessage(
            null,
            new ParsedMessage(
                "From: from3@zimbra.com\r\nTo: to3-1 <to3-1@zimbra.com>, to3-2 <to3-2@zimbra.com>"
                    .getBytes(),
                false),
            opt,
            null);
  ...
  public void moveOutOfSpam() throws Exception {
    ...
    Message msg =
        mbox.addMessage(
            null,
            new ParsedMessage("From: spammer@zimbra.com\r\nTo: test@zimbra.com".getBytes(), false),
            opt,
            null);
    ...
    ZimbraQueryResults result =
        mbox.index.search(SoapProtocol.Soap12, new OperationContext(mbox), params);
    Assert.assertFalse(result.hasNext());

    mbox.move(
        new OperationContext(mbox), msg.getId(), MailItem.Type.MESSAGE, Mailbox.ID_FOLDER_INBOX);
    MailboxTestUtil.index(mbox);
  ...
  public void post() throws Exception {
    ...
    Assert.assertTrue(
        "POST flag set", (msg.getFlagBitmask() & Flag.FlagInfo.POST.toBitmask()) != 0);
