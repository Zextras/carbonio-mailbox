...
public final class ThreaderTest {
  ...
  public void unrelated() throws Exception {
    ...
    threadMessage(
        "unrelated", MailThreadingAlgorithm.none, pm, mbox, Collections.<Integer>emptyList());
    threadMessage(
        "unrelated", MailThreadingAlgorithm.subject, pm, mbox, Collections.<Integer>emptyList());
    threadMessage(
        "unrelated", MailThreadingAlgorithm.references, pm, mbox, Collections.<Integer>emptyList());
    threadMessage(
        "unrelated", MailThreadingAlgorithm.subjrefs, pm, mbox, Collections.<Integer>emptyList());
    threadMessage(
        "unrelated", MailThreadingAlgorithm.strict, pm, mbox, Collections.<Integer>emptyList());
    ...
    threadMessage(
        "unrelated reply", MailThreadingAlgorithm.none, pm, mbox, Collections.<Integer>emptyList());
    threadMessage(
        "unrelated reply",
        MailThreadingAlgorithm.subject,
        pm,
        mbox,
    ...
    threadMessage(
        "unrelated reply",
        MailThreadingAlgorithm.subjrefs,
        pm,
        mbox,
        Collections.<Integer>emptyList());
    threadMessage(
        "unrelated reply",
        MailThreadingAlgorithm.strict,
        pm,
        mbox,
        Collections.<Integer>emptyList());
  }
  ...
  public void followup() throws Exception {
    ...
    threadMessage(
        "followup", MailThreadingAlgorithm.none, pm, mbox, Collections.<Integer>emptyList());
    ...
    threadMessage(
        "followup [irt]", MailThreadingAlgorithm.none, pm, mbox, Collections.<Integer>emptyList());
    ...
    threadMessage(
        "followup [refs]", MailThreadingAlgorithm.none, pm, mbox, Collections.<Integer>emptyList());
  ...
  public void changedSubject() throws Exception {
    ...
    threadMessage(
        "changed subject", MailThreadingAlgorithm.none, pm, mbox, Collections.<Integer>emptyList());
    threadMessage(
        "changed subject",
        MailThreadingAlgorithm.subject,
        pm,
  ...
  public void crossedThread() throws Exception {
    ...
    threadMessage(
        "crossed threads", MailThreadingAlgorithm.none, pm, mbox, Collections.<Integer>emptyList());
  ...
  public void outlook() throws Exception {
    ...
    mm.setHeader(
        "Thread-Index",
        Threader.ThreadIndex.addChild(Threader.ThreadIndex.parseHeader(ROOT_THREAD_INDEX)));
    ParsedMessage pm = new ParsedMessage(mm, false);

    threadMessage(
        "outlook", MailThreadingAlgorithm.none, pm, mbox, Collections.<Integer>emptyList());
    ...
    threadMessage(
        "outlook", MailThreadingAlgorithm.strict, pm, mbox, Collections.<Integer>emptyList());
  }
  ...
  private void checkConversations(Mailbox mbox, int msgidA, int msgidB, boolean match)
      throws ServiceException {
    Message msgA = mbox.getMessageById(null, msgidA);
    Message msgB = mbox.getMessageById(null, msgidB);
    if (match) {
      Assert.assertTrue(
          "in same conversation", msgA.getConversationId() == msgB.getConversationId());
    } else {
      Assert.assertFalse(
          "in different conversations", msgA.getConversationId() == msgB.getConversationId());
    }
  }
  ...
  public void redelivery() throws Exception {
    ...
    int msgid2 =
        mbox.addMessage(
                null, new ParsedMessage(mm, false), MailboxTest.STANDARD_DELIVERY_OPTIONS, null)
            .getId();
  ...
  public void bogusThreadIndexHeader() throws Exception {
    ...
    threadMessage(
        "same subject bogus thread index",
        MailThreadingAlgorithm.none,
        pm,
        mbox,
        Collections.<Integer>emptyList());
    threadMessage(
        "same subject bogus thread index", MailThreadingAlgorithm.subject, pm, mbox, match);
    threadMessage(
        "same subject bogus thread index",
        MailThreadingAlgorithm.references,
        pm,
