...
public final class MailboxTest {
  ...
  public void testRecentMessageCount() throws Exception {
    ...
    Assert.assertEquals(
        "recent message count should be 0 after reset", 0, mbox.getRecentMessageCount());
    mbox.addMessage(
        null, new ParsedMessage("From: test1-2@sub1.zimbra.com".getBytes(), false), dopt, null);
    mbox.addMessage(
        null, new ParsedMessage("From: test1-3@sub1.zimbra.com".getBytes(), false), dopt, null);
    ...
    mbox.resetRecentMessageCount(new OperationContext(acct1));
    Assert.assertEquals(
        "recent message count should be 0 after the second reset", 0, mbox.getRecentMessageCount());
  }
  ...
  public void threadDraft() throws Exception {
    ...
    Assert.assertEquals(
        "threaded explicitly", parent.getConversationId(), draft.getConversationId());
    ...
    Assert.assertEquals(
        "threaded implicitly [saveDraft]", parent.getConversationId(), draft.getConversationId());
    ...
    parent = mbox.getMessageById(null, rootId);
    Assert.assertEquals(
        "threaded implicitly [resaved]", parent.getConversationId(), draft.getConversationId());
    ...
    parent = mbox.getMessageById(null, rootId);
    Assert.assertEquals(
        "threaded implicitly [addMessage]", parent.getConversationId(), draft.getConversationId());
  ...
  public void trimTombstones() throws Exception {
    ...
    Assert.assertTrue(
        "deleted item still in tombstones", mbox.getTombstones(changeId2).contains(msgId));
    ...
    try {
      ...
      Assert.assertEquals(
          "sync token predates purged tombstone", e.getCode(), MailServiceException.MUST_RESYNC);
    }
    Assert.assertTrue(
        "sync token matches last purged tombstone", mbox.getTombstones(changeId3).isEmpty());
  }
  ...
  public void notifications() throws Exception {
    ...
    try {
      ...
      Assert.assertEquals(
          "1 deleted folder, 1 deleted message, 1 deleted vconv", 3, ml.getPms().deleted.size());
  ...
  public void tombstones() throws Exception {
    ...
    Message msg =
        mbox.addMessage(
            null, MailboxTestUtil.generateMessage("test subject"), STANDARD_DELIVERY_OPTIONS, null);
    ...
    mbox.move(
        null, new int[] {doc3.getId(), msg.getId()}, MailItem.Type.UNKNOWN, folder.getId(), null);
    ...
    for (Map.Entry<MailItem.Type, List<TypedIdList.ItemInfo>> row : tombstones) {
      Assert.assertTrue(
          ...
          EnumSet.of(MailItem.Type.FOLDER, MailItem.Type.MESSAGE, MailItem.Type.DOCUMENT)
              .contains(row.getKey()));
