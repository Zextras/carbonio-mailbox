...
public class ImapCommandThrottleTest {
  ...
  public void select() {
    ...
    SelectCommand select3 =
        new SelectCommand(new ImapPath(pathName + "foo", creds), (byte) 123, makeQri());
  ...
  public void examine() {
    ...
    ExamineCommand examine =
        new ExamineCommand(new ImapPath(pathName, creds), (byte) 123, makeQri());
    ...
    ExamineCommand examine2 =
        new ExamineCommand(new ImapPath(pathName, creds), (byte) 123, makeQri());
    Assert.assertTrue("diff obj same fields", examine.isDuplicate(examine2));

    ExamineCommand examine3 =
        new ExamineCommand(new ImapPath(pathName + "foo", creds), (byte) 123, makeQri());
    Assert.assertFalse("different path", examine.isDuplicate(examine3));

    ExamineCommand examine4 =
        new ExamineCommand(new ImapPath(pathName, creds), (byte) 101, makeQri());
  ...
  public void fetch() {
    ...
    Assert.assertTrue(
        "same parts; different order - should be a duplicate", fetch.isDuplicate(fetch7));
  ...
  public void append() throws Exception {
    ...
    List<Part> parts = new ArrayList<Part>();
    AppendMessage appendMsg2 =
        new AppendMessage(appendMsg.getPersistentFlagNames(), appendMsg.getDate(), parts);
  ...
  public void list() {
    ...
    Set<String> mailboxNames =
        new HashSet<String>(Arrays.asList(new String[] {"mbox1", "mbox2", "mbox3"}));
    byte selectOptions = (byte) 24;
    byte returnOptions = (byte) 38;
    byte status = (byte) 67;
    AbstractListCommand list =
        new ListCommand(refName, mailboxNames, selectOptions, returnOptions, status);

    Assert.assertTrue("same obj", list.isDuplicate(list));

    AbstractListCommand list2 =
        new ListCommand(refName, mailboxNames, selectOptions, returnOptions, status);
