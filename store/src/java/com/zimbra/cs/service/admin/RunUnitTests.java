...
public class RunUnitTests extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    for (Iterator<Element> iter = request.elementIterator(AdminConstants.E_TEST);
        iter.hasNext(); ) {
      if (testNames == null) testNames = new ArrayList<String>();
      Element e = iter.next();
    ...
    TestResults results;
    if (testNames == null) results = ZimbraSuite.runTestSuite();
    else results = ZimbraSuite.runUserTests(testNames);
