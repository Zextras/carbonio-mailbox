...
public class TestLog {
  ...
  /**
   * Confirms that account loggers are added for all categories when the category name is set to
   * "all" (bug 29715).
   */
  @Test
  public void allCategories() throws Exception {
    ...
    assertEquals(
        String.format("Number of loggers for acct=%s after acct creation", USER_1),
        0,
        prov.getAccountLoggers(account, null).size());
  ...
  private void assertLoggerExists(
      Iterable<AccountLogger> loggers, Account account, String category, Level level) {
    for (AccountLogger logger : loggers) {
      if (logger.getAccountName().equals(account.getName())
          && logger.getCategory().equals(category)) {
        assertEquals(
            String.format("Log Level for account=%s category=%s", account.getName(), category),
            level,
            logger.getLevel());
        return;
      }
