...
/**
 * Tests out-of-office notification. All tests must be run inside the server, because the cleanup
 * code needs to delete notification rows from the database table.
 */
public class TestOutOfOffice {
  ...
  public void setUp() throws Exception {
    ...
    TestUtil.sendMessage(
        TestUtil.getZMailbox(SENDER_NAME), RECIPIENT_NAME, "message to init recipient's mailbox");
    TestUtil.sendMessage(
        TestUtil.getZMailbox(RECIPIENT_NAME), SENDER_NAME, "message to init sender's mailbox");
  ...
  public void testRowExists() throws Exception {
    ...
    Assert.assertFalse(
        "1 day",
        DbOutOfOffice.alreadySent(mConn, mMbox, RECIPIENT1_ADDRESS, 1 * Constants.MILLIS_PER_DAY));
    Assert.assertFalse(
        "4 days",
        DbOutOfOffice.alreadySent(mConn, mMbox, RECIPIENT1_ADDRESS, 4 * Constants.MILLIS_PER_DAY));
    Assert.assertFalse(
        "5 days",
        DbOutOfOffice.alreadySent(mConn, mMbox, RECIPIENT1_ADDRESS, 5 * Constants.MILLIS_PER_DAY));
    Assert.assertTrue(
        "6 days",
        DbOutOfOffice.alreadySent(mConn, mMbox, RECIPIENT1_ADDRESS, 6 * Constants.MILLIS_PER_DAY));
    Assert.assertTrue(
        "100 days",
        DbOutOfOffice.alreadySent(
            mConn, mMbox, RECIPIENT1_ADDRESS, 100 * Constants.MILLIS_PER_DAY));
  }
  ...
  public void testPrune() throws Exception {
    ...
    Assert.assertFalse(
        "recipient2",
        DbOutOfOffice.alreadySent(mConn, mMbox, RECIPIENT2_ADDRESS, 7 * Constants.MILLIS_PER_DAY));
  }

  /** Confirms that out-of-office notifications use the user's address preference (bug 40869). */
  @Test
  ...
  /**
   * Tests fix for bug 26818 (OOO message does not work when "Don't keep a local copy of messages"
   * is checked)
   *
   * @throws Exception
   */
