...
public class TestFolderACLCache {
  ...
  public void setUp() throws Exception {
    ...
    if (sub2 == null) sub2 = TestUtil.createFolder(ownerMbx, sub2Path);
  ...
  private void doTest(
      ...
      short needRightsForCanAccessTest,
      boolean expectedCanAccess)
      throws ServiceException {

    OperationContext octxt;
    if (authedAcct == null) octxt = null;
    else octxt = new OperationContext(authedAcct);
    ...
    if (actual == -1) actual = ACL.stringToRights(ACL.rightsToString(actual));
    if (expected == -1) expected = ACL.stringToRights(ACL.rightsToString(expected));
    ...
    ZimbraLog.test.debug(
        "Test %s:authedAcct='%s' targetFolderId='%s' %s\n"
            ...
            + "    canAccess:            %s (expected: %s)",
        testName,
        authedAcct,
        targetFolderId,
        (!good ? "***FAILED***" : ""),
        formatRights(effectivePermissions),
        formatRights(expectedEffectivePermissions),
        canAccess,
        expectedCanAccess);
  }
  ...
  public void publicUser() throws Exception {
    ...
    doTest(
        GuestAccount.ANONYMOUS_ACCT, OWNER_ACCT_ID, SUB3_FID, ACL.RIGHT_READ, ACL.RIGHT_READ, true);
    doTest(
        GuestAccount.ANONYMOUS_ACCT,
        OWNER_ACCT_ID,
        SUB3_FID,
        ACL.RIGHT_READ,
        ACL.RIGHT_WRITE,
