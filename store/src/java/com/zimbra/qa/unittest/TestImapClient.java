...
public class TestImapClient {
  ...
  public static void beforeClass() throws Exception {
    ...
    sp.soapSetURI(
        LC.zimbra_admin_service_scheme.value()
            + homeServer.getServiceHostname()
            + ":"
            + homeServer.getAdminPort()
  ...
  public void testSSLLogin() throws Exception {
    ...
    TestUtil.assumeTrue(
        "No server supporting SSL server to connect to",
        (homeServer.isImapServerEnabled() && homeServer.isImapSSLServerEnabled())
            || (homeServer.isRemoteImapServerEnabled()
                && homeServer.isRemoteImapSSLServerEnabled()));
  ...
  public void testIdle() throws Exception {
    ...
    assertEquals(
        String.format(
            "got wrong number of messages. mb.getExists(): %d. exists.get(): %d",
            mb.getExists(), exists.get()),
        mb.getExists(),
        exists.get());
  }
  ...
  public void testAppend() throws Exception {
    ...
    AppendResult res =
        connection.append(
            "INBOX", Flags.fromSpec("fs"), date, new Literal(Ascii.getBytes(MESSAGE)));
    assertNotNull(res);
  ...
  public void testDelete() throws Exception {
    ...
    mb = connection.select("INBOX");
    assertEquals(
        "expecting 'exists' to still be " + (mb.getExists() - 1), exists, mb.getExists() - 1);
    connection.expunge();
    mb = connection.select("INBOX");
    assertEquals(
        "expecting 'exists' to go back to previous value after EXPUNGE", exists, mb.getExists());
  }
  ...
  public void testFetch() throws Exception {
    ...
    connection.uidFetch(
        "1:*",
        "(FLAGS INTERNALDATE RFC822.SIZE ENVELOPE BODY BODY.PEEK[])",
        new ResponseHandler() {
          @Override
