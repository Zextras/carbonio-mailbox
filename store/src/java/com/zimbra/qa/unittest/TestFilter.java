...
public final class TestFilter {
  ...
  private static String MOUNTPOINT_SUBFOLDER_PATH =
      "/" + MOUNTPOINT_FOLDER_NAME + "/" + MOUNTPOINT_SUBFOLDER_NAME;
  ...
  public void setUp() throws Exception {
    ...
    TestUtil.createMountpoint(
        remoteMbox, "/" + MOUNTPOINT_FOLDER_NAME, mMbox, MOUNTPOINT_FOLDER_NAME);
  ...
  /**
   * Confirms that outgoing filters are applied as expected when a message is sent via
   * SendMsgRequest.
   */
  @Test
  ...
  /**
   * Confirms that outgoing filters are applied as expected when a sent message is added via
   * AddMsgRequest.
   */
  public void testOutgoingFiltersWithAddMsg() throws Exception {
    ...
    mMbox.addMessage(
        "" + Mailbox.ID_FOLDER_SENT, "s", null, System.currentTimeMillis(), content, false, true);
    ...
    mMbox.addMessage(
        "" + Mailbox.ID_FOLDER_SENT, "s", null, System.currentTimeMillis(), content, false, false);
  ...
  /**
   * Confirms that a message with a base64-encoded subject can be filtered correctly (bug 11219).
   */
  ...
  /** Confirms that all actions are performed when a message matches multiple filter rules. */
  ...
  /**
   * Confirms that spam filtering takes a higher precedence than custom user filtering (bug 23886).
   */
  public void testSpam() throws Exception {
    ...
    message =
        config.getAttr(Provisioning.A_zimbraSpamHeader)
            + ": "
    ...
    TestUtil.setAccountAttr(
        USER_NAME, Provisioning.A_zimbraSpamApplyUserFilters, LdapConstants.LDAP_TRUE);
    TestUtil.addMessageLmtp(recipients, sender, message);
    ...
    TestUtil.setAccountAttr(
        USER_NAME, Provisioning.A_zimbraSpamApplyUserFilters, LdapConstants.LDAP_FALSE);
    TestUtil.addMessageLmtp(recipients, sender, message);
    TestUtil.waitForMessage(mbox, "in:junk subject:testSpam");
  }

  /**
   * Verifies the fix to bug 5455. Tests sending a message that matches two filter rules, each of
   * which has a tag action and a flag action.
   */
  ...
  public void testMountpoint() throws Exception {
    ...
    TestUtil.waitForMessage(
        mbox, "in:\"/" + MOUNTPOINT_FOLDER_NAME + "\" subject:\"" + subject + "\"");
    ...
    assertEquals(
        0,
        TestUtil.search(mbox, "in:\"/" + MOUNTPOINT_FOLDER_NAME + "\" subject:\"" + subject + "\"")
            .size());
  }
  ...
  public void testMountpointSubfolder() throws Exception {
    ...
    TestUtil.waitForMessage(
        mbox, "in:\"" + MOUNTPOINT_SUBFOLDER_PATH + "\" subject:\"" + subject + "\"");
    assertEquals(0, TestUtil.search(mbox, "in:inbox subject:\"" + subject + "\"").size());
    ...
    // Send another message, confirm that it gets filed into inbox and not in the mountpoint
    // subfolder.
  ...
  public void testMimeHeader() throws Exception {
    ...
    String content =
        new MessageBuilder()
            .withSubject(subject)
            .withToRecipient(USER_NAME)
            .withAttachment(attachmentData, "attachment.html", "text/html")
            .create();
  ...
  public void testToOrCc() throws Exception {
    ...
    String content =
        new MessageBuilder()
            .withSubject(subject)
            .withToRecipient(USER_NAME)
            .withCcRecipient("cc@checkthis.com")
            .create();
    TestUtil.addMessageLmtp(new String[] {USER_NAME}, USER_NAME, content);
    ZMessage msg = TestUtil.getMessage(mMbox, "in:inbox subject:\"" + subject + "\"");
    Set<String> tagIds = new HashSet<String>();
    ...
    content =
        new MessageBuilder()
            .withSubject(subject)
            .withToRecipient("to@checkthis.com")
            .withCcRecipient(USER_NAME)
            .create();
    TestUtil.addMessageLmtp(new String[] {USER_NAME}, USER_NAME, content);
  ...
  public void testCaseSensitiveComparison() throws Exception {
    ...
    ZMessage msg = TestUtil.getMessage(mMbox, "in:inbox subject:\"" + subject + "\"");
    assertTrue(
        "message should not have been tagged",
        msg.getTagIds() == null || msg.getTagIds().isEmpty());
    ...
    // Deliver message having lower case body content substring and make sure that tag1 was not
    // applied
  ...
  public void testCurrentDayOfWeekTest() throws Exception {
    ...
    conditions.add(
        new ZFilterCondition.ZCurrentDayOfWeekCondition(
            SimpleOp.IS, dayYesterday + "," + dayTomorrow));
    actions.add(new ZTagAction(mTag1.getName()));
    rules.add(
        new ZFilterRule(
            "testCurrentDayOfWeekTest day not today", true, false, conditions, actions));
    saveIncomingRules(mMbox, new ZFilterRules(rules));
    ...
    conditions.add(
        new ZFilterCondition.ZCurrentDayOfWeekCondition(SimpleOp.IS, Integer.toString(dayToday)));
  ...
  public void testAddressTest() throws Exception {
    ...
    conditions.add(
        new ZFilterCondition.ZHeaderCondition("From", HeaderOp.IS, "john.doe@example.com"));
    ...
    String mime =
        new MessageBuilder()
            .withFrom("John Doe <john.doe@example.com>")
            .withSubject(subject)
            .create();
    ...
    conditions.add(
        new ZFilterCondition.ZAddressCondition(
  ...
  public void testReplyAction() throws Exception {
    ...
    assertTrue(
        bodyContent.contains(TestUtil.getAddress(REMOTE_USER_NAME)) && bodyContent.contains(body));

    // bug 65369
    assertTrue(
        !mimeStructure.getContentType().contains("charset")
   ...
   *
   * <p>Note: please set 'zimbraSieveNotifyActionRFCCompliant' to FALSE.
   */
  public void testNotifyAction() throws Exception {
    ...
    actions.add(
        new ZFilterAction.ZNotifyAction(
            TestUtil.getAddress(REMOTE_USER_NAME),
            "${SUBJECT}",
  ...
  public void testNotifyActionCopyAllOrigHeaders() throws Exception {
    ...
    rules.add(
        new ZFilterRule("testNotifyActionCopyAllOrigHeaders", true, false, conditions, actions));
    saveIncomingRules(mMbox, new ZFilterRules(rules));
  ...
  public void testMultipleMultilineText() throws Exception {
    ...
    assertEquals(
        ((ZFilterAction.ZNotifyAction) rules.get(0).getActions().get(0)).getBodyTemplate(),
        notifyMsg1);
    assertEquals(
        ((ZFilterAction.ZNotifyAction) rules.get(0).getActions().get(1)).getBodyTemplate(),
        notifyMsg2);
    assertEquals("Rule 2 should have 1 action", rules.get(1).getActions().size(), 1);
    assertEquals(
        ((ZFilterAction.ZReplyAction) rules.get(1).getActions().get(0)).getBodyTemplate(),
        replyMsg);
  }
  ...
  public void testMarkRead() throws Exception {
    ...
    assertTrue(
        "Unexpected flags: " + flags,
        flags == null || flags.indexOf(ZMessage.Flag.UNREAD.getFlagChar()) < 0);
  }

  /**
   * Confirms that the header test works with headers that are folded. See section 2.2.3 of RFC 2822
   * and bug 14942.
   */
  @Test
  ...
  public void testInvite() throws Exception {
    ...
    condition =
        new ZInviteCondition(
            true, Arrays.asList(ZInviteCondition.METHOD_REQUEST, ZInviteCondition.METHOD_CANCEL));
    ...
    TestUtil.sendInviteReply(
        mbox, msg.getId(), organizer.getName(), subject, ZMailbox.ReplyVerb.ACCEPT);
  ...
  public void testManyAsterisks() throws Exception {
    ...
    ZFilterCondition condition =
        new ZHeaderCondition("from", HeaderOp.MATCHES, "*****address@yahoo.com");
  ...
  public void testDateFiltering() throws Exception {
    ...
    conditions.add(
        new ZDateCondition(
            DateOp.BEFORE, new Date(System.currentTimeMillis() + Constants.MILLIS_PER_DAY)));
    ...
    conditions.add(
        new ZDateCondition(
            DateOp.AFTER, new Date(System.currentTimeMillis() - Constants.MILLIS_PER_DAY)));
  ...
  /**
   * Confirms that the body test looks for text in the main message body and attached message
   * bodies, but not in attachments.
   */
  @Test
  public void testBodyContains() throws Exception {
    assertTrue(
        "To run this test copy data/unittest/TestFilter-testBodyContains.msg to"
            + " /opt/zextras/unittest",
        Files.exists(Paths.get("/opt/zextras/unittest/TestFilter-testBodyContains.msg")));
    ...
    doBodyContainsTest(
        "This is the text version of the main body of the message. This is the second line.", true);
  }
  ...
  public void testSpecialCharInBody() throws Exception {
    assertTrue(
        "To run this test copy data/unittest/TestFilter-testSpecialCharInBody.msg to"
            + " /opt/zextras/unittest",
        Files.exists(Paths.get("/opt/zextras/unittest/TestFilter-testSpecialCharInBody.msg")));
    List<ZFilterCondition> conditions = new ArrayList<ZFilterCondition>();
    List<ZFilterAction> actions = new ArrayList<ZFilterAction>();
    List<ZFilterRule> rules = new ArrayList<ZFilterRule>();
    ...
    // Add a message and test the flagged state.
    String address = TestUtil.getAddress(USER_NAME);
    // TestFilter-testSpecialCharInBody.msg's body contains base64 encoded content (containing
    // "Andr\u00e9")
    String msgContent =
        new String(
            ByteUtil.getContent(
                new File("/opt/zextras/unittest/TestFilter-testSpecialCharInBody.msg")));
    TestUtil.addMessageLmtp(new String[] {address}, address, msgContent);
  ...
  /**
   * Tests the redirect filter action and confirms that the X-ZimbraForwarded header is set on the
   * redirected message.
   */
  @Test
  ...
  public void testAttachment() throws Exception {
    ...
    String msgContent =
        new MessageBuilder()
            .withSubject(subject)
            .withAttachment("attach this", "attach.txt", "text/plain")
            .create();
    TestUtil.addMessageLmtp(new String[] {address}, address, msgContent);
    ZMessage msg = TestUtil.getMessage(mMbox, "in:inbox subject:\"" + subject + "\"");
  ...
  /** Confirms that we handle the negative test flag properly with multiple tests (bug 46007). */
  @Test
  public void testPositiveAndNegative() throws Exception {
    assertTrue(
        "To run this test copy data/unittest/bug46007.sieve to /opt/zextras/unittest",
        Files.exists(Paths.get("/opt/zextras/unittest/bug46007.sieve")));
    String script =
        new String(ByteUtil.getContent(new File("/opt/zextras/unittest/bug46007.sieve")));
  ...
  /**
   * Saves the given incoming filter rules. Then gets them from the server and confirms that the
   * element tree matches.
   */
  ...
  /**
   * Saves the given outgoing filter rules. Then gets them from the server and confirms that the
   * element tree matches.
   */
