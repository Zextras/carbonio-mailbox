...
/**
 * Test the IMAP server provided by the IMAP daemon, doing the necessary configuration to make it
 * work.
 *
 * <p>Note: Currently bypasses Proxy, the tests connect directly to the IMAP daemon's port
 *
...
public class TestImapViaImapDaemon extends SharedImapTests {
  ...
  public void setUp() throws Exception {
    getLocalServer();
    TestUtil.assumeTrue(
        "remoteImapServerEnabled false for this server", imapServer.isRemoteImapServerEnabled());
  ...
  private void enableImapAndCheckCachedValue(Account acct) throws Exception {
    ...
    tryConnect(
        false, "should not be able to log in since imapd has old value of zimbraImapEnabled=FALSE");
  }

  private void disableImapAndCheckCachedValue(Account acct) throws Exception {
    acct.setImapEnabled(false);
    tryConnect(
        true, "should be able to log in since imapd has old value of zimbraImapEnabled=TRUE");
  }
  ...
  public void testClearDaemonCache() throws Exception {
    tryConnect(
        true, "should be able to log in initially"); // loads the account into the imapd cache
    ...
    tryConnect(
        false, "should not be able to log in after flushing LDAP cache by account name and ID");
  ...
  public void testReloadLocalConfig() throws Exception {
    ...
    connection.login(PASS);
    try {
      // sanity check: even though the LC value is changed, imapd should still be using the old
      // value
  ...
  public void imapdStatFiles() throws Exception {
    ...
    assertTrue(
        String.format("%s file is unexpectedly empty", testFile.getCanonicalPath()),
        testFile.length() > 0);
  ...
  public void flushNonImapCacheTypes() throws Exception {
    ...
    ImapConnection adminConn = getAdminConnection();
    Set<CacheEntryType> allTypes =
        new HashSet<CacheEntryType>(Arrays.asList(CacheEntryType.values()));
