...
public class TestCookieReuse {
  ...
  public void tearDown() throws Exception {
    ...
    Provisioning.getInstance()
        .getLocalServer()
        .setLowestSupportedAuthVersion(currentSupportedAuthVersion);
  }
  ...
  public void testAutoEndSession() throws ServiceException, IOException, HttpException {
    ...
    int statusCode = response.getStatusLine().getStatusCode();
    Assert.assertEquals(
        "This request should not succeed. Getting status code " + statusCode,
  ...
  /**
   * Verify that we canNOT RE-use the cookie taken from a legitimate HTTP session for a SOAP request
   * after ending the original session
   */
  @Test
  public void testInvalidSearchRequest() throws ServiceException, IOException, HttpException {
    ...
    Assert.assertFalse(
        "this search request should return some conversations", searchHits.isEmpty());
    ...
    try {
      ...
      Assert.assertEquals(
          "Should be getting 'auth required' exception",
          ServiceException.AUTH_EXPIRED,
          ex.getCode());
    }
  }
  ...
  public void testWebLogOut()
    ...
    Assert.assertEquals(
        "Log out request should succeed. Getting status code " + statusCode,
        HttpStatus.SC_OK,
        statusCode);
  ...
  public void testReuseAdminCookieWithoutCsrf() throws Exception {
    ...
    String getServerConfigURL =
        "https://localhost:" + port + "/service/collectconfig/?host=" + host;
    HttpClientBuilder eve = ZimbraHttpConnectionManager.getInternalHttpConnMgr().newHttpClient();
  ...
  public void testReuseUserCookieWithoutCsrf() throws Exception {
    ...
    BasicCookieStore state =
        HttpClientUtil.newHttpState(new ZAuthToken(at.getEncoded()), uri.getHost(), false);
    eve.setDefaultCookieStore(state);
    ...
    int statusCode = httpResp.getStatusLine().getStatusCode();
    Assert.assertEquals(
        "This request should succeed. Getting status code "
            + statusCode
            + " Response: "
  ...
  /**
   * Verify that we CANNOT make an admin POST request by reusing a valid csrf-enabled cookie without
   * a csrf token
   */
  @Test
  public void testUnauthorizedAdminPostWithCsrf() throws Exception {
    ...
    try {
      ...
      Assert.assertEquals(
          "should be catching AUTH EXPIRED here", ServiceException.AUTH_REQUIRED, e.getCode());
      return;
    }
  ...
  /**
   * Verify that we CANNOT make an admin POST request with a non-CSRF-enabled auth token if the auth
   * token has an associated CSRF token
   */
  @Test
  ...
  /**
   * version of SOAP transport that uses HTTP cookies instead of SOAP message header for
   * transporting Auth Token
   *
   * @author gsolovyev
   */
  private class HttpCookieSoapTransport extends SoapHttpTransport {
    ...
    @Override
    protected final Element generateSoapMessage(
        Element document,
        boolean raw,
        boolean noSession,
      ...
      if (generateContextHeader()) {
        ...
        if (noSession) {
          ...
          SoapUtil.addSessionToCtxt(
              context, getAuthToken() == null ? null : getSessionId(), getMaxNotifySeq());
        }
