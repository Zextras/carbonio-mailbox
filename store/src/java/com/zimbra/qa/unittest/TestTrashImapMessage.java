...
public class TestTrashImapMessage {
  ...
  public void setUp() throws Exception {
    if (!TestUtil.accountExists(USER_NAME)) TestUtil.createAccount(USER_NAME);

    mbox = TestUtil.getZMailbox(USER_NAME);
    if (!TestUtil.accountExists(IMAP_ACCOUNT_NAME_1)) TestUtil.createAccount(IMAP_ACCOUNT_NAME_1);
  ...
  private String addMessage(ZMailbox mbox, String subject, String body) throws Exception {
    String id =
        mbox.addMessage(
            "2", null, null, System.currentTimeMillis(), getMessage(subject, body), true);
    Thread.sleep(1000);
    return id;
  }
