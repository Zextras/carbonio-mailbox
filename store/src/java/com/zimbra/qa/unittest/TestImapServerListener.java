...
public class TestImapServerListener {
  ...
  public static void beforeClass() throws Exception {
    ...
    sp.soapSetURI(
        LC.zimbra_admin_service_scheme.value()
            + localServer.getServiceHostname()
            + ":"
            + localServer.getAdminPort()
  ...
  public void setUp() throws Exception {
    ...
    if (remoteServer != null) {
      localServer.setReverseProxyUpstreamImapServers(
          new String[] {remoteServer.getServiceHostname()});
      ...
      remoteServer.setReverseProxyUpstreamImapServers(
          new String[] {localServer.getServiceHostname()});
  ...
  public void testNotifyInbox() throws Exception {
    ...
    TestUtil.addMessage(
        mboxStore,
        "TestImapServerListener - testNotify - init message",
    ...
    QueryWaitSetResponse resp =
        TestUtil.waitForSessions(1, 1, 6000, remoteListener.getWSId(), remoteServer);
    ...
    TestUtil.addMessageLmtp(
        ...
        "randomUserTestImapServerListener@yahoo.com");
    TestUtil.waitForMessages(
        mboxStore, String.format("in:inbox is:unread \"%s\"", subject), 1, 1000);
    try {
      session.doneSignal.await(
          (LC.zimbra_waitset_nodata_sleep_time.intValue() / 1000 + 2), TimeUnit.SECONDS);
    } catch (Exception e) {
  ...
  public void testNotifyNewFolder() throws Exception {
    ...
    TestUtil.addMessageLmtp(
        ...
        "randomUserTestImapServerListener@yahoo.com");
    ZMessage msg =
        TestUtil.waitForMessage(mboxStore, String.format("in:inbox subject:\"%s\"", subject));
    mboxStore.moveMessage(msg.getId(), folder.getId());
    TestUtil.waitForMessage(
        mboxStore, String.format("in:%s subject:\"%s\"", folder.getName(), subject));
    try {
      session.doneSignal.await(
          (LC.zimbra_waitset_nodata_sleep_time.intValue() / 1000 + 2), TimeUnit.SECONDS);
    } catch (Exception e) {
  ...
  public void testNotifyReadUnread() throws Exception {
    ...
    assertTrue(
        "Expected session to be triggered after delivering the test message",
        session.wasTriggered());
    ...
    msg =
        TestUtil.waitForMessage(
            mboxStore, String.format("in:inbox is:read subject:\"%s\"", subject));
    ...
    assertTrue(
        "Expected session to be triggered after marking test message READ", session.wasTriggered());
    ...
    msg =
        TestUtil.waitForMessage(
            mboxStore, String.format("in:inbox is:unread subject:\"%s\"", subject));
    assertTrue("New message should have UNREAD flag", msg.isUnread());
    try {
      session.doneSignal.await(
          (LC.zimbra_waitset_nodata_sleep_time.intValue() / 1000 + 2), TimeUnit.SECONDS);
    ...
    assertTrue(
        "Expected session to be triggered after marking test message UREAD",
        session.wasTriggered());
    remoteListener.removeListener(session);
  }
  ...
  public void testNotifyTagUntag() throws Exception {
    ...
    msg =
        TestUtil.waitForMessage(
            mboxStore, String.format("in:inbox tag:testNotifyTagUntag subject:\"%s\"", subject));
    assertTrue(
        "Test message should have testNotifyTagUntag tag after tagging",
        msg.getTagIds().contains(tag.getId()));
    try {
      session.doneSignal.await(
          (LC.zimbra_waitset_nodata_sleep_time.intValue() / 1000 + 2), TimeUnit.SECONDS);
    } catch (Exception e) {
    ...
    assertTrue(
        "Expected session to be triggered after tagging test message", session.wasTriggered());
    ...
    msg =
        TestUtil.waitForMessage(
            mboxStore, String.format("in:inbox -tag:testNotifyTagUntag subject:\"%s\"", subject));
    assertFalse(
        "Test message should NOT have testNotifyTagUntag tag after untagging",
        msg.getTagIds().contains(tag.getId()));
    try {
      session.doneSignal.await(
          (LC.zimbra_waitset_nodata_sleep_time.intValue() / 1000 + 2), TimeUnit.SECONDS);
    ...
    assertTrue(
        "Expected session to be triggered after untagging test message", session.wasTriggered());
    remoteListener.removeListener(session);
  }
  ...
  public void testNotifyRenameTag() throws Exception {
    ...
    msg =
        TestUtil.waitForMessage(
            mboxStore, String.format("in:inbox tag:testNotifyTag subject:\"%s\"", subject));
    assertTrue(
        "Test message should have testNotifyTag tag after tagging",
        msg.getTagIds().contains(tag.getId()));
    try {
      session.doneSignal.await(
          (LC.zimbra_waitset_nodata_sleep_time.intValue() / 1000 + 2), TimeUnit.SECONDS);
    ...
    msg =
        TestUtil.waitForMessage(
            mboxStore, String.format("in:inbox tag:testNotifyRenamedTag subject:\"%s\"", subject));
    assertTrue(
        "Test message should have testNotifyRenamedTag tag after renaming the tag",
        msg.getTagIds().contains(tag.getId()));
    try {
      session.doneSignal.await(
          (LC.zimbra_waitset_nodata_sleep_time.intValue() / 1000 + 2), TimeUnit.SECONDS);
  ...
  @Disabled("this is failing until WaitSetResponse can process deletes")
  ...
  public void testNotifyWrongFolder() throws Exception {
    ...
    ZMailbox mboxStore = TestUtil.getZMailbox(REMOTE_USER_NAME);
    ZFolder folder =
        TestUtil.createFolder(mboxStore, "/TestImapServerListener-testNotifyNewFolder");
    ImapServerListener remoteListener = ImapServerListenerPool.getInstance().get(mboxStore);
    RemoteImapMailboxStore imapStore = new RemoteImapMailboxStore(mboxStore);
    ImapCredentials creds = new ImapCredentials(remoteAccount);
    ImapPath path = new ImapPath("INBOX", creds);
    ...
    QueryWaitSetResponse resp =
        TestUtil.waitForSessions(1, 1, 6000, remoteListener.getWSId(), remoteServer);
    TestUtil.addMessage(
        mboxStore, "TestImapServerListener - testNotifyWrongFolder", folder.getId());
    try {
      session.doneSignal.await(
          (LC.zimbra_waitset_nodata_sleep_time.intValue() / 1000 + 2), TimeUnit.SECONDS);
    } catch (Exception e) {
  ...
  public void testRegisterUnregister() throws ServiceException, IOException, HttpException {
    ...
    assertFalse(
        "Expecting ImapServerListener::isListeningOn to return false before calling addListener",
        remoteListener.isListeningOn(session.getTargetAccountId(), session.getFolderId()));
    assertNull(
        "Should not have a waitset before registering first listener", remoteListener.getWSId());
    remoteListener.addListener(session);
    assertTrue(
        "Expecting ImapServerListener::isListeningOn to return true after calling addListener",
        remoteListener.isListeningOn(session.getTargetAccountId(), session.getFolderId()));
    assertNotNull(
        "Should have a waitset after registering first listener", remoteListener.getWSId());
    ...
    QueryWaitSetResponse resp =
        JaxbUtil.elementToJaxb(transport.invoke(JaxbUtil.jaxbToElement(req)));
    assertNotNull(resp);
    ...
    remoteListener.removeListener(session);
    assertFalse(
        "Expecting ImapServerListener::isListeningOn to return false after calling removeListener",
        remoteListener.isListeningOn(session.getTargetAccountId(), session.getFolderId()));
  ...
  public void testGetListeners() throws Exception {
    ...
    MockImapListener session = new MockImapListener(imapStore, i4folder, handler);
    assertNotNull("ImapListener instance should not be null", session);
    Set<ImapRemoteSession> sessions =
        remoteListener.getListeners(remoteAccount.getId(), i4folder.getId());
  ...
  public void testShutdown() throws Exception {
    ...
    remoteListener.shutdown();
    assertNull(
        "Should not have a waitset after shutting down ImapServerListener",
        remoteListener.getWSId());
  }
  ...
  public void testRemoveFolderInterest() throws Exception {
    ...
    assertEquals(
        "expected to find 1 session after adding a listener for INBOX", 1, sessions.size());
    ...
    ImapRemoteSession draftsSession =
        (ImapRemoteSession) imapStore.createListener(i4folder, handler);
    ...
    assertNotNull(sessions);
    assertEquals(
        "expected to find 1 session after adding a listener for DRAFTS", 1, sessions.size());
    s = sessions.get(0);
    folders = s.getWaitSetSession().getFolderInterestsAsSet();
    assertNotNull("folder interests cannot be NULL", folders);
    assertEquals("should have two folder interests", 2, folders.size());
    assertTrue(
        "folder interests should contain DRAFTS", folders.contains(Mailbox.ID_FOLDER_DRAFTS));
    assertTrue("folder interests should contain INBOX", folders.contains(Mailbox.ID_FOLDER_INBOX));
    ...
    assertNotNull(sessions);
    assertEquals(
        "expected to find 1 session after removing listener for inbox", 1, sessions.size());
    s = sessions.get(0);
    folders = s.getWaitSetSession().getFolderInterestsAsSet();
    assertNotNull("folder interests cannot be NULL", folders);
    assertEquals("should have one folder interest after removing a listener", 1, folders.size());
    assertTrue(
        "folder interests should contain DRAFTS after removing INBOX listener",
        folders.contains(Mailbox.ID_FOLDER_DRAFTS));
  }
  ...
  public void testDestroyWaitset() throws Exception {
    ...
    TestUtil.addMessage(
        mboxStore,
        "TestImapServerListener - testDestroyWaitset - init message",
        Integer.toString(Mailbox.ID_FOLDER_INBOX));
    ImapServerListener remoteListener = ImapServerListenerPool.getInstance().get(mboxStore);
    RemoteImapMailboxStore imapStore = new RemoteImapMailboxStore(mboxStore);
    ImapCredentials creds = new ImapCredentials(remoteAccount);
    ...
    AdminDestroyWaitSetResponse destroyResp =
        JaxbUtil.elementToJaxb(transport.invoke(JaxbUtil.jaxbToElement(destroyReq)));
    ...
    while (maxWait > 0) {
      if (remoteListener.getWSId() != null
          && !waitSetId.equalsIgnoreCase(remoteListener.getWSId())) {
        break;
      } else {
        maxWait -= 500;
        Thread.sleep(500);
    ...
    Assert.assertFalse(
        "ImapServerListener should have created a new waitset",
        waitSetId.equalsIgnoreCase(remoteListener.getWSId()));
    ...
    TestUtil.addMessageLmtp(
        subject,
        TestUtil.getAddress(REMOTE_USER_NAME),
        TestUtil.getAddress("TestImapServerListener-testDestroyWaitset"));
    TestUtil.waitForMessages(
        mboxStore, String.format("in:inbox is:unread \"%s\"", subject), 1, 1000);
    try {
      session.doneSignal.await(
    ...
    assertFalse(
        "ImapServerListener should have created a new waitset",
        remoteListener.getWSId().equalsIgnoreCase(waitSetId));
  }
  ...
  public void testDeleteOneOutOfTwoAccounts() throws Exception {
    ...
    TestUtil.addMessage(
        mboxStore,
        "TestImapServerListener - testNotify - init message 1",
        Integer.toString(Mailbox.ID_FOLDER_INBOX));
    ImapServerListener remoteListener = ImapServerListenerPool.getInstance().get(mboxStore);
    RemoteImapMailboxStore imapStore = new RemoteImapMailboxStore(mboxStore);
    ImapCredentials creds = new ImapCredentials(remoteAccount);
    ...
    ZimbraLog.test.debug(
        "Created 2d account " + REMOTE_USER_NAME2 + " with ID " + remoteAccount2.getId());
    TestUtil.addMessage(
        mboxStore2,
        "TestImapServerListener - testNotify - init message 2",
        Integer.toString(Mailbox.ID_FOLDER_INBOX));
    ImapServerListener remoteListener2 = ImapServerListenerPool.getInstance().get(mboxStore2);
    assertEquals(
        "Should be getting the same ImapServerListener for both accounts on the same server",
        remoteListener2,
        remoteListener);
    ...
    String waitSetId = remoteListener.getWSId();
    assertTrue(
        "Both listener references should be pointing to the same listener with the same waitset",
        waitSetId.equalsIgnoreCase(remoteListener2.getWSId()));
    ...
    TestUtil.addMessageLmtp(
        subject,
        TestUtil.getAddress(REMOTE_USER_NAME),
        TestUtil.getAddress("TestImapServerListener-testDeleteOneOutOfTwoAccounts"));
    TestUtil.waitForMessages(
        mboxStore, String.format("in:inbox is:unread \"%s\"", subject), 1, 1000);

    // the 2d account's session should be killed
    try {
      session2.killSignal.await(
          (LC.zimbra_waitset_nodata_sleep_time.intValue() / 1000 + 2), TimeUnit.SECONDS);
    } catch (Exception e) {
      Assert.fail("Wait interrupted.");
    }
    ...
    assertFalse(
        "Expected the 1st account's session to NOT be unregistered", session.wasUnregistered());
    assertNotNull(
        "ImapServerListener should NOT have deleted the waitset", remoteListener.getWSId());
    assertTrue(
        "Waitset ID should have remained the same",
        waitSetId.equalsIgnoreCase(remoteListener.getWSId()));
  }
  ...
  class MockImapListener extends ImapRemoteSession {
    ...
    MockImapListener(ImapMailboxStore store, ImapFolder i4folder, ImapHandler handler)
