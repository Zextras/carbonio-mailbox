...
public class TestMailItem {
  ...
  public void testListItemIds() throws Exception {
    ...
    TestUtil.addMessage(
        ...
        testInfo.getMethodName() + "draft 1",
        System.currentTimeMillis());
    TestUtil.addMessage(
        mbox,
        Mailbox.ID_FOLDER_DRAFTS,
        testInfo.getMethodName() + "draft 2",
        System.currentTimeMillis());

    // Get item count per folder/type
    String sql =
        "SELECT folder_id, type, count(*) AS item_count "
            + "FROM "
    ...
    while (results.next()) {
      ...
      ZimbraLog.test.debug(
          "Confirming that folder '%s' has %s items of type %s", folderId, count, type);
      ...
      Assert.assertEquals(
          String.format("Item count does not match for Folder with ID='%s'", folderId),
          count,
          ids.size());
