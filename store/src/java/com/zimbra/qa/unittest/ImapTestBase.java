...
public abstract class ImapTestBase {
  ...
  /**
   * Only need to do this once for each class - the corresponding restore needs to be done after
   * every test though.
   */
  @BeforeAll
  public static void saveImapConfigSettings()
    ...
    ZimbraLog.test.debug(
        "Saved ImapConfigSettings %s=%s %s=%s %s=%s",
        LC.imap_always_use_remote_store.key(),
        saved_imap_always_use_remote_store,
        Provisioning.A_zimbraImapServerEnabled,
        saved_imap_server_enabled,
        Provisioning.A_zimbraImapSSLServerEnabled,
        saved_imap_ssl_server_enabled);
  ...
  public void restoreImapConfigSettings()
    ...
    if (imapServer != null) {
      ZimbraLog.test.debug(
          "Restoring ImapConfigSettings %s=%s %s=%s %s=%s",
          LC.imap_always_use_remote_store.key(),
          saved_imap_always_use_remote_store,
          Provisioning.A_zimbraImapServerEnabled,
          saved_imap_server_enabled,
    ...
    TestUtil.setLCValue(
        LC.imap_always_use_remote_store, String.valueOf(saved_imap_always_use_remote_store));
  ...
  protected void doSelectShouldFail(ImapConnection conn, String folderName) throws IOException {
    ...
    try {
      ...
      fail(
          String.format(
              "'SELECT %s' succeeded - should have failed mbInfo=%s", folderName, mbInfo));
  ...
  protected MailboxInfo doSelectShouldSucceed(ImapConnection conn, String folderName)
      throws IOException {
    checkConnection(conn);
    MailboxInfo mbInfo = null;
    try {
  ...
  protected MailboxInfo doExamineShouldSucceed(ImapConnection conn, String folderName)
      throws IOException {
    checkConnection(conn);
    MailboxInfo mbInfo = null;
    try {
  ...
  protected static class StatusExecutor {
    ...
    protected StatusExecutor setExists(long expected) {
      expectedExists = expected;
      return this;
    }
    ;

    protected StatusExecutor setRecent(long expected) {
      expectedRecent = expected;
      return this;
    }
    ;

    protected StatusExecutor setUnseen(long expected) {
      expectedUnseen = expected;
      return this;
    }
    ;

    protected MailboxInfo execShouldSucceed(String folderName, Object... params)
        throws IOException {
      String pr = Joiner.on(",").join(params);
      try {
        mbInfo = conn.status(folderName, params);
        assertNotNull(
            String.format("return MailboxInfo for 'STATUS %s (%s)'", folderName, pr), mbInfo);
        if (expectedExists != null) {
          assertEquals(
              String.format("Count of EXISTS for 'STATUS %s (%s)'", folderName, pr),
              expectedExists.longValue(),
              mbInfo.getExists());
        }
        if (expectedRecent != null) {
          assertEquals(
              String.format("Count of RECENT for 'STATUS %s (%s)'", folderName, pr),
              expectedRecent.longValue(),
              mbInfo.getRecent());
        }
        if (expectedUnseen != null) {
          assertEquals(
              String.format("Count of UNSEEN for 'STATUS %s (%s)'", folderName, pr),
              expectedUnseen.longValue(),
              mbInfo.getUnseen());
        }
      } catch (CommandFailedException cfe) {
  ...
  protected Map<Long, MessageData> doFetchShouldSucceed(
      ImapConnection conn, String range, String what, List<String> expectedSubjects)
      throws IOException {
    checkConnection(conn);
    try {
      Map<Long, MessageData> mdMap = conn.fetch(range, what);
      assertNotNull(
          String.format("map returned by 'FETCH %s %s' should not be null", range, what), mdMap);
      assertEquals(
          String.format("Size of map returned by 'FETCH %s %s'", range, what),
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            String.format(
                "Envelope for MessageData for %s item in results of 'FETCH %s %s' should not be"
                    + " null",
                cnt, range, what),
            env);
        assertEquals(
            String.format("Subject for %s item in results of 'FETCH %s %s'", cnt, range, what),
            expectedSubjects.get(cnt),
            env.getSubject());
        cnt++;
      }
      return mdMap;
  ...
  protected void doRenameShouldFail(String origFolderName, String newFolderName)
      throws IOException {
    try {
      connection.rename(origFolderName, newFolderName);
      fail(
          String.format(
              "2nd attempt to RENAME %s %s succeeded when it shouldn't have'",
  ...
  protected void doRenameShouldSucceed(String origFolderName, String newFolderName)
      throws IOException {
    try {
      connection.rename(origFolderName, newFolderName);
    } catch (CommandFailedException cfe) {
      fail(
          String.format(
              "attempt to 'RENAME %s %s' failed - %s",
              origFolderName, newFolderName, cfe.getError()));
    }
  }
  ...
  protected void doListShouldFail(ImapConnection conn, String ref, String mailbox, String expected)
    ...
    try {
      ...
      assertTrue(
          String.format(
              "'LIST \"%s\" \"%s\"' error should contain '%s' was '%s'",
              ref, mailbox, expected, err),
          err.contains(expected));
    }
  }
   ...
   * where the mailboxes are in the Other Users' Namespace (i.e. start with "/home/"), otherwise,
   * they should not have the '/' prefix. The "/" in that case comes from the Namespace prefix and
   * NOT from the mailbox name.
   *
   * <p>For another way of looking at it, it is worth comparing NAMESPACE and LIST command output
   * from https://tools.ietf.org/html/rfc2342 - IMAP4 Namespace: C: A001 NAMESPACE S: * NAMESPACE
   * (("" "/")) (("#Users/" "/")) NIL S: A001 OK NAMESPACE command completed C: A002 LIST ""
   * "#Users/Mike/%" S: * LIST () "/" "#Users/Mike/INBOX" S: * LIST () "/" "#Users/Mike/Foo" S: A002
   * OK LIST command completed.
   *
   * <p>with the Zimbra equivalent:
   *
   * <p>C: ZIMBRA01 NAMESPACE S: * NAMESPACE (("" "/")) (("/home/" "/")) NIL S: ZIMBRA01 OK
   * NAMESPACE completed C: ZIMBRA02 LIST "" "/home/other-user/*" S: * LIST (\HasChildren) "/"
   * "/home/other-user/INBOX/shared" S: * LIST (\HasNoChildren) "/"
   * "/home/other-user/INBOX/shared/subFolder" S: ZIMBRA02 OK LIST completed
   */
  protected List<ListData> doLSubShouldSucceed(
      ImapConnection conn,
      String ref,
      String mailbox,
      List<String> expectedMboxNames,
      String testDesc)
      throws IOException {
    checkConnection(conn);
  ...
  protected List<ListData> doListShouldSucceed(
      ImapConnection conn,
      String ref,
      String mailbox,
      List<String> expectedMboxNames,
  ...
  public void checkListDataList(
      String cmdDesc, String testDesc, List<ListData> listResult, List<String> expectedMboxNames)
      throws IOException {
    assertNotNull(
        String.format("%s:list result from %s should not be null", testDesc, cmdDesc), listResult);
    ...
    if (!missingMboxNames.isEmpty() || !extraMboxNames.isEmpty()) {
      ...
      fail(
          String.format(
              "%s:'%s' returned unexpected mailbox names %s\nList Result:%s",
              testDesc, cmdDesc, sb, listResult));
    }
    // Doubt if can get here but just in case
    assertEquals(
        String.format(
            "%s:Number of entries in list returned for %s\n%s", testDesc, cmdDesc, listResult),
        expectedMboxNames.size(),
        listResult.size());
  }
  ...
  protected MessageData fetchMessage(ImapConnection conn, long uid) throws IOException {
    ...
    assertNotNull(
        String.format(
            "`UID FETCH %s (FLAGS BODY.PEEK[])` returned no data - assume message not found", uid),
        md);
    assertEquals(
        String.format("`UID FETCH %s (FLAGS BODY.PEEK[])` returned wrong UID", uid),
        uid,
        md.getUid());
    return md;
  }
  ...
  protected AppendResult doAppend(
      ImapConnection conn,
      String folderName,
      String subject,
      String body,
  ...
  protected AppendResult doAppend(
      ImapConnection conn, String folderName, int size, Flags flags, boolean fetchResult)
      throws IOException {
    checkConnection(conn);
    assertTrue("expecting UIDPLUS capability", conn.hasCapability("UIDPLUS"));
    Date date = new Date(System.currentTimeMillis());
  ...
  protected void checkRegex(
      String regexPatt, String target, Boolean expected, int maxAccesses, Boolean timeoutOk) {
    try {
      Pattern patt = Pattern.compile(regexPatt);
      AccessBoundedRegex re = new AccessBoundedRegex(patt, maxAccesses);
      assertEquals(
          String.format("matching '%s' against pattern '%s'", target, patt),
          expected,
          Boolean.valueOf(re.matches(target)));
