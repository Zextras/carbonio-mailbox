...
/**
 * @author gsolovyev CollectLDAPConfigServlet should be accessible only by global admins:
 *     https://bugzilla.zimbra.com/show_bug.cgi?id=93800
 */
public class TestCollectConfigServletsAccess extends TestCase {
  ...
  public void setUp() throws Exception {
    ...
    Provisioning.getInstance()
        .grantRight(
            ...
            GranteeBy.name,
            TestUtil.getAddress(TEST_ADMIN_NAME),
            null,
  ...
  public void testConfigDelegatedAdmin() throws Exception {
    ...
    URI servletURI = new URI(getConfigServletUrl());
    BasicCookieStore initialState = HttpClientUtil.newHttpState(at, servletURI.getHost(), true);
    HttpClientBuilder restClientBuilder =
  ...
  /**
   * Verify that an HTTP client canNOT access servlet at /service/collectldapconfig/ without an auth
   * token
   *
   * @throws Exception
   */
  ...
  /**
   * Verify that an HTTP client canNOT access servlet at /service/collectconfig/ without an auth
   * token
   *
   * @throws Exception
   */
