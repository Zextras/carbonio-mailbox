...
public class TestContacts {
  ...
  public void testMaxContacts() throws Exception {
    ...
    TestUtil.setAccountAttr(
        USER_NAME, Provisioning.A_zimbraContactMaxNumEntries, Integer.toString(max));
  ...
  public void testContactAttachments() throws Exception {
    ...
    String attachment1Id =
        mbox.uploadAttachment("attachment.txt", attachment1Text.getBytes(), "text/plain", timeout);
    ...
    String attachment2Id =
        mbox.uploadAttachment("attachment.txt", attachment2Text.getBytes(), "text/plain", timeout);
    attachments.clear();
    ...
    String newAttachment2Id =
        mbox.uploadAttachment(
            "attachment.txt", newAttachment2Text.getBytes(), "text/plain", timeout);
    ...
    String attachment4Id =
        mbox.uploadAttachment("attachment.txt", attachment4Text.getBytes(), "text/plain", timeout);
  ...
  public void testMoveContact() throws Exception {
    ...
    Account acct =
        Provisioning.getInstance().get(Key.AccountBy.name, TestUtil.getAddress(USER_NAME));
    ...
    String attachment1Id =
        zmbx.uploadAttachment("file1.txt", attachment1Text.getBytes(), "text/plain", timeout);
    Map<String, ZAttachmentInfo> attachments = new HashMap<String, ZAttachmentInfo>();
    ZAttachmentInfo info = new ZAttachmentInfo().setAttachmentId(attachment1Id);
    ...
    Account acct2 =
        Provisioning.getInstance().get(Key.AccountBy.name, TestUtil.getAddress(USER2_NAME));
    ...
    String idStr =
        TestUtil.search(remoteZmbx, "in:Contacts testMoveContact", ZSearchParams.TYPE_CONTACT)
            .get(0);
    ...
    remoteZmbx.moveContact(
        String.valueOf(ct.getId()), acct.getId() + ":" + Mailbox.ID_FOLDER_CONTACTS);
    // reset the access
  ...
  public void testImportContacts() throws Exception {
    ...
    ZMailbox mbox = TestUtil.getZMailbox(USER_NAME);
    String csvText =
        "\"email\",\"fullName\"\n\"testImportContacts@example.org\",\""
            + NAME_PREFIX
            + " testImportContacts\"";
    String attachmentId =
        mbox.uploadAttachment("ImportContacts.csv", csvText.getBytes(), "text/plain", timeout);

    ZImportContactsResult res =
        mbox.importContacts(folderId, ZMailbox.CONTACT_IMPORT_TYPE_CSV, attachmentId);
