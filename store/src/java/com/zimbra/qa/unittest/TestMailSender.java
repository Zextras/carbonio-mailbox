...
public class TestMailSender {
  ...
  public void testRejectRecipient() throws Exception {
    ...
    String content =
        TestUtil.getTestMessage(NAME_PREFIX + " testRejectSender", bogusAddress, SENDER_NAME, null);
    MimeMessage msg =
        new ZMimeMessage(
    ...
    try {
      ...
      validateException(
          e, MailServiceException.SEND_ABORTED_ADDRESS_FAILURE, bogusAddress, errorMsg);
      sendFailed = true;
    }
  ...
  public void testRestrictEnvelopeSender() throws Exception {
    ...
    MessageBuilder builder =
        new MessageBuilder()
            .withFrom(from)
            .withToRecipient(RECIPIENT_NAME)
            .withSubject(subject)
            .withBody("Who are you?");
  ...
  private void validateException(
      MailServiceException e, String expectedCode, String invalidRecipient, String errorSubstring) {
    Assert.assertEquals(expectedCode, e.getCode());
    if (errorSubstring != null) {
      Assert.assertTrue(
          "Error did not contain '" + errorSubstring + "': " + e.getMessage(),
