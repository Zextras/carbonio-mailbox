...
public class TestLockoutMailbox extends TestCase {
  ...
  private static final String DELEGATED_ADMIN_NAME = "delegated-admin@" + MY_DOMAIN;
  private static final String MY_USER = "user1@" + MY_DOMAIN;
  private static final String MY_NON_EXISTING_USER = "user2@" + MY_DOMAIN;
  private static final String OFFLIMITS_NON_EXISTING_USER = "user2@" + OFFLIMITS_DOMAIN;
  private static final String OFFLIMITS_USER1 = "user1@" + OFFLIMITS_DOMAIN;
  ...
  public void testLockout() throws Exception {
    ...
    assertFalse(
        "mailbox should not be locked yet",
  ...
  public void testUnlock() throws Exception {
    ...
    assertNotNull("LockoutMailboxRequest return null response", resp);
    assertFalse(
        "mailbox should not be locked any more",
        MailboxManager.getInstance().isMailboxLockedOut(mbox.getAccountId()));
  }
  ...
  public void testLockAccountEnumeration() throws Exception {
    ...
    LockoutMailboxRequest req =
        LockoutMailboxRequest.create(AccountNameSelector.fromName(OFFLIMITS_NON_EXISTING_USER));
    req.setOperation(AdminConstants.A_START);
    try {
      ...
      assertEquals(
          "should be getting 'Permission Denied' response",
  ...
  public void testLockoutSufficientPermissions() throws Exception {
    ...
    try {
      ...
      assertEquals(
          "should be getting 'no such account' response",
  ...
  public void createDelegatedAdmin(List<AdminRight> relatedRights) throws ServiceException {
    ...
    StringUtil.addToMultiMap(
        attrs, Provisioning.A_zimbraIsDelegatedAdminAccount, LdapConstants.LDAP_TRUE);
    domainAdmin =
        adminSoapProv.createAccount(DELEGATED_ADMIN_NAME, TestUtil.DEFAULT_PASSWORD, attrs);
    ...
    for (AdminRight r : relatedRights) {
      ...
      grantRightToAdmin(
          adminSoapProv,
          com.zimbra.soap.type.TargetType.fromString(targetType.toString()),
          target,
    ...
    delegatedSoapProv =
        TestUtil.newDelegatedSoapProvisioning(DELEGATED_ADMIN_NAME, TestUtil.DEFAULT_PASSWORD);
  }

  private static void grantRightToAdmin(
