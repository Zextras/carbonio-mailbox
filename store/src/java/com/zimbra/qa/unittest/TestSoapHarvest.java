...
public class TestSoapHarvest {
  ...
  private String sendReq(
      String userId, String authToken, int expectedCode, boolean useGetInfoReq, boolean byAccountId)
      throws HttpException, IOException {
    HttpClient client = HttpClientBuilder.create().build();
    HttpPost method =
        new HttpPost(TestUtil.getSoapUrl() + (useGetInfoReq ? "GetInfoRequest" : "NoOpRequest"));
    method.setEntity(
        new StringEntity(
            useGetInfoReq
                ? getInfoRequest(userId, authToken, byAccountId)
                : getNoOpRequest(userId, authToken, byAccountId),
  ...
  public void testHarvestDelegated() throws Exception {
    ...
    Assert.assertNotNull(mbox);
    // note this fails if you've shared anything from TARGET_USER_NAME to user1. works fine in clean
    // setup
    response = sendReq(userId, authToken, 500, true);
    Assert.assertTrue(response.indexOf("<Code>service.PERM_DENIED</Code>") > -1);
    Assert.assertTrue(
        response.indexOf("<soap:Text>permission denied: can not access account") > -1);
    // make sure we're not returning account id
    Assert.assertTrue(
        !response.contains(Provisioning.getInstance().get(AccountBy.name, userId).getId()));
  ...
  public void testAdminDelegation() throws Exception {
    ...
    ZMailbox mbox =
        TestUtil.getZMailbox(ADMIN_USER_NAME, adminPasswd, null, TrustedStatus.not_trusted);
    String authToken = mbox.getAuthToken().getValue();
