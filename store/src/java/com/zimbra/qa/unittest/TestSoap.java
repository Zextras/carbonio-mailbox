...
public class TestSoap {
  ...
  public void testSoapRequestMaxSize() throws Exception {
    ...
    for (int i = 1; i <= 100; i++) {
      messageBody.append(
          "Morey Amsterdam was a great man.  Morey Amsterdam was not a sandwich.\r\n");
    }
    ...
    try {
      ...
      Assert.assertTrue(
          ...
          e.toString().contains("bytes set for zimbraSoapRequestMaxSize"));
    }
  }
  ...
  public void testAccountGetInfoRequest() throws Exception {
    ...
    Element request =
        Element.create(transport.getRequestProtocol(), AccountConstants.GET_VERSION_INFO_REQUEST);
  ...
  public void testAdminGetInfoRequest() throws Exception {
    SoapTransport transport =
        SoapTest.authAdmin(LC.zimbra_ldap_user.value(), LC.zimbra_ldap_password.value());
    Element request =
        Element.create(transport.getRequestProtocol(), AdminConstants.GET_VERSION_INFO_REQUEST);
    Element response = transport.invoke(request);
    validateSoapVersionResponse(response);
  }
  ...
  private ZMailbox runAuthTest(ZMailbox.Options options) throws Exception {
    ...
    List<String> prefNames =
        Arrays.asList(
            Provisioning.A_zimbraPrefComposeFormat, Provisioning.A_zimbraPrefAutoSaveDraftInterval);
   ...
   */
  private String doLowLevelRequest(URL url, String request) throws IOException {
  ...
  public void testBadXmlReqWantJSResp() throws IOException {
    ...
    String responseString =
        doLowLevelRequest(new URL(TestUtil.getSoapUrl() + "/WibbleRequest"), req.toString());
    Assert.assertTrue(
        "Response should be a JSON fault",
        responseString.startsWith("{\"Body\":{\"Fault\":{\"Code\":"));
  }
  ...
  public void testBadXmlReqWantXmlResp() throws IOException {
    ...
    String responseString =
        doLowLevelRequest(new URL(TestUtil.getSoapUrl() + "/WibbleRequest"), req.toString());
    Assert.assertTrue(
        "Response should be a SOAP 1.2 fault",
        responseString.startsWith(
            "<soap:Envelope"
                + " xmlns:soap=\"http://www.w3.org/2003/05/soap-envelope\"><soap:Body><soap:Fault>"));
  }
  ...
  public void testBadSoap11XmlReq() throws IOException {
    ...
    Assert.assertTrue(
        String.format(
            "Response [%s] should be a SOAP 1.1 fault with faultstring 'Documentparse failed'",
            responseString),
        responseString.startsWith(
            "<soap:Envelope"
                + " xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\"><soap:Body><soap:Fault><faultcode>soap:Client</faultcode><faultstring>Document"
                + " parse failed"));
  }
