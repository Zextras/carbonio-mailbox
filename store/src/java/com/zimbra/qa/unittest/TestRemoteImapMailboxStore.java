...
public class TestRemoteImapMailboxStore extends TestCase {
  ...
  public void testSaveIMAPSubscriptions() throws Exception {
    ...
    RemoteImapMailboxStore remoteStore =
        new RemoteImapMailboxStore(zmbox, TestUtil.getAccount(USER_NAME).getId());
  ...
  public void testGetCurrentModseq() throws Exception {
    ...
    RemoteImapMailboxStore remoteStore =
        new RemoteImapMailboxStore(zmbox, TestUtil.getAccount(USER_NAME).getId());
    Assert.assertEquals(
        "Before adding a message, remoteStore.getCurrentMODSEQ returns value different from"
            + " folder.getImapMODSEQ",
        remoteStore.getCurrentMODSEQ(folderIdent),
        folder.getImapMODSEQ());
    ...
    Assert.assertEquals(
        "After adding a message, remoteStore.getCurrentMODSEQ returns value different from"
            + " folder.getImapMODSEQ",
        remoteStore.getCurrentMODSEQ(folderIdent),
        folder.getImapMODSEQ());
    Assert.assertFalse(
        "Modseq should have changed after adding a message",
        remoteStore.getCurrentMODSEQ(folderIdent) == oldModSeq);
  }
  ...
  public void testOpenImapFolder() throws Exception {
    ...
    for (int i = 1; i <= 3; i++) {
      Message msg =
          TestUtil.addMessage(
              mbox, folderId, String.format("imap message %s", i), System.currentTimeMillis());
      expected.add(new ImapMessage(msg));
    }
    ZMailbox zmbox = TestUtil.getZMailbox(USER_NAME);
    RemoteImapMailboxStore remoteStore =
        new RemoteImapMailboxStore(zmbox, TestUtil.getAccount(USER_NAME).getId());
    List<ImapMessage> actual =
        remoteStore.openImapFolder(null, new ItemIdentifier(mbox.getAccountId(), folderId));
    Collections.sort(expected);
    Collections.sort(actual);
    assertEquals(
        "expected and actual ImapMessage lists have different lengths",
        expected.size(),
        actual.size());
