...
public class TestSearchJunkTrash {
  ...
  public void setUp() throws Exception {
    ...
    folder =
        remote_mbox.createFolder("1", "my shared folder", ZFolder.View.message, null, null, null);
  ...
  public void testJunk() throws ServiceException {
    ZSearchParams params =
        new ZSearchParams(String.format("(in:junk) (inid:%s OR is:local)", folder.getId()));
  ...
  public void testTrash() throws ServiceException {
    ZSearchParams params =
        new ZSearchParams(String.format("(in:trash) (inid:%s OR is:local)", folder.getId()));
    ZSearchResult results = mbox.search(params);
  ...
  public void testTrashOrJunk() throws ServiceException {
    ZSearchParams params =
        new ZSearchParams(
            String.format("(in:trash OR in:junk) (inid:%s OR is:local)", folder.getId()));
    ZSearchResult results = mbox.search(params);
  ...
  public void testTrashOrNotJunk() throws ServiceException {
    ...
    ZSearchParams params =
        new ZSearchParams(
            String.format("(in:trash OR NOT in:junk) (inid:%s OR is:local)", folder.getId()));
    ZSearchResult results = mbox.search(params);
    Assert.assertEquals(1, results.getHits().size());
  }
  ...
  public void testNotNotTrashOrJunk() throws ServiceException {
    ...
    ZSearchParams params =
        new ZSearchParams(
            String.format("(NOT(in:trash OR NOT in:junk)) (inid:%s OR is:local)", folder.getId()));
    ZSearchResult results = mbox.search(params);
    Assert.assertEquals(2, results.getHits().size());
  }
}
