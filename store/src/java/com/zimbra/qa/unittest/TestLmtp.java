...
public class TestLmtp {
  ...
  public void setUp() throws Exception {
    ...
    originalConfigDiskThreshold =
        TestUtil.getConfigAttr(Provisioning.A_zimbraMailDiskStreamingThreshold);
    ...
    originalDedupeCacheTimeout =
        TestUtil.getConfigAttr(Provisioning.A_zimbraMessageIdDedupeCacheTimeout);
  }

  @AfterEach
  public void tearDown() throws Exception {
    TestUtil.setServerAttr(
        Provisioning.A_zimbraMailDiskStreamingThreshold, originalServerDiskThreshold);
    TestUtil.setConfigAttr(
        Provisioning.A_zimbraMailDiskStreamingThreshold, originalConfigDiskThreshold);
    TestUtil.setConfigAttr(Provisioning.A_zimbraMessageIdDedupeCacheSize, originalDedupeCacheSize);
    TestUtil.setConfigAttr(
        Provisioning.A_zimbraMessageIdDedupeCacheTimeout, originalDedupeCacheTimeout);
    cleanUp();
  }
  ...
  /** Tests reading data with various valid/invalid values for the size hint and disk threshold. */
  @Test
  ...
  public void testDiskStreamingOneRecipient() throws Exception {
    ...
    TestUtil.addMessageLmtp(
        NAME_PREFIX + " testDiskStreamingOneRecipient", recipient, TestUtil.getAddress(USER_NAME));
    ZMailbox mbox = TestUtil.getZMailbox(USER_NAME);
  ...
  /**
   * Another test for bug 25484. Delivers a message to user1 and user2, then confirms that user1 can
   * still read the message after user2 empties the folder that contains the message.
   */
  @Test
  public void testDiskStreamingEmptyFolder() throws Exception {
    ...
    ZFolder folder2 =
        TestUtil.createFolder(mbox2, "/" + NAME_PREFIX + " testDiskStreamingEmptyFolder");
  ...
  public void testSizeHint() throws Exception {
    ...
    lmtp.sendMessage(
        new ByteArrayInputStream(data), recipients, address, "TestLmtp", (long) data.length);
    lmtp.sendMessage(
        new ByteArrayInputStream(data), recipients, address, "TestLmtp", (long) Integer.MAX_VALUE);
    lmtp.close();
  ...
  /**
   * Sends a message with another message attached and confirms that the subject of the attached
   * message is indexed.
   *
  ...
  public void testAttachedMessage() throws Exception {
    ...
    addresses.add(
        new ZEmailAddress(TestUtil.getAddress(USER_NAME), null, null, ZEmailAddress.EMAIL_TYPE_TO));
    msg.setAddresses(addresses);
    msg.setSubject(outerSubject);

    // Assemble body and inner message
    String attachedMessageString =
        TestUtil.getTestMessage(innerSubject, USER_NAME, USER_NAME, null);
  ...
  /**
   * Confirms that delivery succeeds when <tt>zimbraMailDiskStreamingThreshold</tt> isn't set (bug
   * 22536).
   */
  @Test
  ...
  /**
   * Tests the LMTP deduping code. Attempts to deliver multiple copies of the same message to the
   * same message to the same mailbox simultaneously. Confirms that only one copy got delivered. Bug
   * 38898.
   */
  @Test
  ...
  /**
   * Confirms that delivery succeeds after an initial failure. This confirms that the message id was
   * removed from the LMTP dedupe cache when a delivery failure occurs. Bug 38898.
   *
   * @throws Exception
   */
  @Test
  public void testDeliveryAfterFailure() throws Exception {
    ...
    Assert.assertFalse(
        "LMTP should not have succeeded", TestUtil.addMessageLmtp(recipients, USER_NAME, content));
  ...
  public void disableTestDedupeCacheTimeout() throws Exception {
    ...
    String query = "in:inbox subject:\"" + subject + "\"";
    Assert.assertEquals(
        "message should have been delivered", 1, TestUtil.search(mbox, query).size());
    ...
    TestUtil.addMessageLmtp(recipients, USER_NAME, content);
    Assert.assertEquals(
        "dedupe cache entry should have timed out", 2, TestUtil.search(mbox, query).size());
  }

  /**
   * Confirms that we reject messages that have a line that's longer than the limit specified by
   * {@link LC#zimbra_lmtp_max_line_length}. Bug 42214.
   */
  @Test
  public void testValidation() throws Exception {
    ...
    Assert.assertFalse(
        TestUtil.addMessageLmtp(new String[] {USER_NAME}, USER_NAME, buf.toString()));
  }
  ...
  public void testAllowReceiveButNotSendWhenOverQuota() throws Exception {
    TestUtil.setAccountAttr(
        USER_NAME,
        Provisioning.A_zimbraMailAllowReceiveButNotSendWhenOverQuota,
    ...
    try {
      ...
      TestUtil.createDocument(
          mbox,
          Integer.toString(Mailbox.ID_FOLDER_BRIEFCASE),
          NAME_PREFIX + " receivenosend.bin",
    ...
    ZOutgoingMessage outgoingDraft =
        TestUtil.getOutgoingMessage(USER_NAME, draftSubject1, draftSubject1, null);
    ZMessage draftMsg =
        mbox.saveDraft(
            outgoingDraft, null, Integer.toString(Mailbox.ID_FOLDER_DRAFTS)); // Add message
    outgoingDraft = TestUtil.getOutgoingMessage(USER_NAME, draftSubject2, draftSubject2, null);
    mbox.saveDraft(
        outgoingDraft,
        draftMsg.getId(),
        Integer.toString(Mailbox.ID_FOLDER_DRAFTS)); // Set content of existing message
  }

  /**
   * Verifies that duplicate suppression recognizes the {@code Resent-Message-ID} header (bug
   * 36297).
   */
  @Test
  ...
  public void testFinalDotNotSent() throws Exception {
    ...
    Assert.assertTrue(
        "msg got delivered via LMTP even though <CRLF>.<CRLF> was not received", msgs.isEmpty());
  }
  ...
  public void testLhloNotSendByClient() throws Exception {
    ...
    boolean replyOk;
    boolean lhloRequired =
        prov.getLocalServer().getBooleanAttr(Provisioning.A_zimbraLmtpLHLORequired, true);
    for (String command : commands) {
      ...
      Assert.assertTrue(
          "Response :" + lmtpClient.getResponse() + " for command :" + command,
          lhloRequired ? !replyOk : replyOk);
      lmtpClient.abruptClose();
    }
  }
