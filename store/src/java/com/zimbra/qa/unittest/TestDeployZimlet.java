...
/** copy test data from data/unittest to /opt/zextras/unittest before running this test */
public class TestDeployZimlet {
  ...
  public static void init() throws Exception {
    ...
    ADMIN_UPLOAD_URL =
        "https://" + localServer.getServiceHostname() + ":" + port + "/service/upload?fmt=raw";
    ...
    TestUtil.grantRightToAdmin(
        TestUtil.newSoapProvisioning(),
        com.zimbra.soap.type.TargetType.fromString(
            com.zimbra.cs.account.accesscontrol.TargetType.server.toString()),
        localServer.getName(),
        delegatedAdmin.getName(),
        Admin.R_deployZimlet.getName());
    File rogueFile = new File("/opt/zextras/conf/rogue.file");
    if (rogueFile.exists()) {
      rogueFile.delete();
    }
    File legitFile = new File("/opt/zextras/zimlets-deployed/absolute/opt/zextras/conf/rogue.file");
    if (legitFile.exists()) {
      legitFile.delete();
    }
  }
  ...
  public void testEmptyAid() throws Exception {
    ...
    DeployZimletRequest deployReq =
        new DeployZimletRequest(AdminConstants.A_DEPLOYLOCAL, false, true, att);
  ...
  public void testNoAid() throws Exception {
    ...
    transport.setAuthToken(authToken);
    DeployZimletRequest deployReq =
        new DeployZimletRequest(AdminConstants.A_DEPLOYLOCAL, false, true, null);
    Element req = JaxbUtil.jaxbToElement(deployReq);
    try {
      Element res = transport.invoke(req);
      JaxbUtil.elementToJaxb(res);
  ...
  public void testBadZimletName() throws Exception {
    ...
    com.zimbra.soap.admin.message.AuthResponse authResp = JaxbUtil.elementToJaxb(response);
    String authToken = authResp.getAuthToken();
    String aid = adminUpload(authToken, "attack.zip", "/opt/zextras/unittest/zimlets/attack.zip");
  ...
  public void testValidAdminExtension() throws Exception {
    ...
    String aid =
        adminUpload(
            authToken, "adminextension.zip", "/opt/zextras/unittest/zimlets/adminextension.zip");
    assertNotNull("Attachment ID should not be null", aid);
    ...
    assertTrue(
        "should be getting 'pending' or 'succeeded' status",
        status.equals(DeployZimlet.sPENDING) || status.equals(DeployZimlet.sSUCCEEDED));

    UndeployZimletRequest undeployReq =
        new UndeployZimletRequest("adminextension", AdminConstants.A_DEPLOYALL);
  ...
  public void testUndeployBadName() throws Exception {
    ...
    transport.setAuthToken(authToken);
    UndeployZimletRequest undeployReq =
        new UndeployZimletRequest("../data/something", AdminConstants.A_DEPLOYALL);
  ...
  public void testZipWithTraversal() throws Exception {
    ...
    String authToken = authResp.getAuthToken();
    String aid =
        adminUpload(authToken, "attack.zip", "/opt/zextras/unittest/zimlets/com_zimbra_url.zip");
    assertNotNull("Attachment ID should not be null", aid);
  ...
  public void testZipWithInvalidCharacter() throws Exception {
    ...
    com.zimbra.soap.admin.message.AuthResponse authResp = JaxbUtil.elementToJaxb(response);
    String authToken = authResp.getAuthToken();
    String aid = adminUpload(authToken, "jelmer.zip", "/opt/zextras/unittest/zimlets/jelmer.zip");
    assertNotNull("Attachment ID should not be null", aid);
  ...
  public void testZipWithAbsolutePath() throws Exception {
    ...
    String authToken = authResp.getAuthToken();
    String aid =
        adminUpload(authToken, "absolute.zip", "/opt/zextras/unittest/zimlets/absolute.zip");
    assertNotNull("Attachment ID should not be null", aid);
    ...
    File rogueFile = new File("/opt/zextras/conf/rogue.file");
    assertFalse("/opt/zextras/conf/rogue.file should not have been created", rogueFile.exists());
  }
  ...
  public void testPhilsZip() throws Exception {
    ...
    com.zimbra.soap.admin.message.AuthResponse authResp = JaxbUtil.elementToJaxb(response);
    String authToken = authResp.getAuthToken();
    String aid = adminUpload(authToken, "phil.zip", "/opt/zextras/unittest/zimlets/phil.zip");
    assertNotNull("Attachment ID should not be null", aid);
  ...
  public String adminUpload(String authToken, String fileName, String filePath) throws Exception {
    ...
    BasicClientCookie cookie =
        new BasicClientCookie(ZimbraCookie.authTokenCookieName(true), authToken);
