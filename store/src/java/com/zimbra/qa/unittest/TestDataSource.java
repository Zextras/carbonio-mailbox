...
public class TestDataSource {
  ...
  public void setUp() throws Exception {
    NAME_PREFIX =
        String.format("%s-%s", TestDataSource.class.getSimpleName(), testInfo.getMethodName())
            .toLowerCase();
    ...
    mOriginalAccountPollingInterval =
        account.getAttr(Provisioning.A_zimbraDataSourcePollingInterval, false);
    if (mOriginalAccountPollingInterval == null) {
      mOriginalAccountPollingInterval = "";
    }
    mOriginalAccountPop3PollingInterval =
        account.getAttr(Provisioning.A_zimbraDataSourcePop3PollingInterval, false);
    if (mOriginalAccountPop3PollingInterval == null) {
      mOriginalAccountPop3PollingInterval = "";
    }
    mOriginalAccountImapPollingInterval =
        account.getAttr(Provisioning.A_zimbraDataSourceImapPollingInterval, false);
    ...
    mOriginalCosPop3PollingInterval =
        cos.getAttr(Provisioning.A_zimbraDataSourcePop3PollingInterval, "");
    mOriginalCosImapPollingInterval =
        cos.getAttr(Provisioning.A_zimbraDataSourceImapPollingInterval, "");
  }
  ...
  public void testPollingInterval() throws Exception {
    ...
    DataSource ds =
        prov.createDataSource(
            account, DataSourceType.pop3, NAME_PREFIX + " testPollingInterval", attrs);

    // Valid polling interval
    assertNotNull(
        "Min not defined", account.getAttr(Provisioning.A_zimbraDataSourceMinPollingInterval));
  ...
  /**
   * Tests the <tt>lastError</tt> element and <tt>failingSince</tt> attribute for
   * <tt>GetInfoRequest</tt> and <tt>GetDataSourcesRequest</tt>.
   */
  @Test
  public void testErrorStatus() throws Exception {
    ...
    attrs.put(
        ...
        TestUtil.getServerAttr(Provisioning.A_zimbraPop3BindPort));
    ...
    long startTimestamp =
        System.currentTimeMillis() / 1000; // timestamp is returned in seconds, not millis
  ...
  private void confirmErrorStatus(Element ePop3, Long timestampBeforeSync) throws Exception {
    if (timestampBeforeSync != null) {
      ...
      assertTrue(
          failingSince + " is earlier than " + timestampBeforeSync,
          failingSince >= timestampBeforeSync);
      assertTrue(failingSince + " is later than " + now, failingSince < now);
    } else {
      assertNull(
          "Last error was not reset", ePop3.getOptionalElement(MailConstants.E_DS_LAST_ERROR));
      assertNull(
          "Error timestamp was not reset",
          ePop3.getAttribute(MailConstants.A_DS_FAILING_SINCE, null));
    }
  }
  ...
  public static String createPop3DataSource(
      ZMailbox shareeMbox, String sharedAcctName, String dsFolderId) throws ServiceException {
  ...
  public static String createFolderForDataSource(ZMailbox shareeMBox, String datasource)
      throws ServiceException {
    CreateFolderResponse resp =
        shareeMBox.invokeJaxb(
            new CreateFolderRequest(
                NewFolderSpec.createForNameAndParentFolderId(
  ...
  public static String addMessage(ZMailbox mbox, String subject, String body) throws Exception {
    String id =
        mbox.addMessage(
            ...
            new MessageBuilder().withSubject(subject).withBody(body).create(),
            true);
    Thread.sleep(1000);
    return id;
  }

  public static void waitUntilImportsFinish(ZMailbox mbox)
  ...
  public void refreshPop3DatasourceData(ZMailbox mbox, String dsId)
      throws ServiceException, InterruptedException {
    ImportDataRequest req = new ImportDataRequest();
    req.setDataSources(
        Lists.newArrayList((DataSourceNameOrId) ImapDataSourceNameOrId.createForId(dsId)));
    mbox.invokeJaxb(req);
  ...
  public void testPop3() throws Exception {
    ...
    try {
      ...
      fail(
          "SoapFaultException caught when deleting item from Pop3 datasource folder - "
              + sfe.getMessage());
    }
  ...
  /**
   * Creates a folder that syncs to another folder via RSS, and verifies that an RSS data source was
   * implicitly created.
   */
  @Test
  public void testRss() throws Exception {
    ...
    String urlString =
        String.format(
            "https://%s:%s/home/%s%s.rss",
            TestUtil.getServerAttr(Provisioning.A_zimbraServiceHostname),
            TestUtil.getServerAttr(Provisioning.A_zimbraMailSSLPort),
            USER_NAME,
            sourceFolder.getPath());
    urlString = HttpUtil.encodePath(urlString);
    ZFolder rssFolder =
        mbox.createFolder(
            parentId, NAME_PREFIX + " testRss destination", null, null, null, urlString);
     ...
     */
    ConvActionSelector sel =
        ConvActionSelector.createForIdsAndOperation(syncedMsg.getConversationId(), "trash");
    ...
    try {
      ...
      fail(
          "SoapFaultException caught when deleting item from RSS datasource folder - "
              + sfe.getMessage());
    }
  ...
  public void disabledTestCal() throws Exception {
    ...
    String parentId = Integer.toString(Mailbox.ID_FOLDER_USER_ROOT);
    String urlString =
        "http://www.google.com/calendar/ical/k2kh7ncij3s05dog63g0o0n254%40group.calendar.google.com/public/basic.ics";
    ZFolder folder;
    try {
      folder =
          mbox.createFolder(
              parentId, NAME_PREFIX + " testCal", ZFolder.View.appointment, null, null, urlString);
    } catch (ServiceException e) {
      assertEquals(ServiceException.RESOURCE_UNREACHABLE, e.getCode());
      ZimbraLog.test.warn(
          "Unable to test calendar data source for %s: %s", urlString, e.toString());
      return;
    }
