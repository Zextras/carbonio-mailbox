...
public class TestCommunityIntegration extends TestCase {
  ...
  public void setUp() throws Exception {
    ...
    testUser =
        mProv.createAccount(
            USER_NAME.concat("@").concat(DOMAIN_NAME), TestUtil.DEFAULT_PASSWORD, attrs);
  }
  ...
  public void testGetInfo() throws Exception {
    ...
    assertTrue(
        "community URL begin with " + COMMUNITY_BASE_URL,
        communityURL.startsWith(COMMUNITY_BASE_URL));
    assertTrue(
        "community URL should contain username=" + testUser.getUid(),
        communityURL.contains(String.format("username=%s", testUser.getUid())));
    assertTrue(
        "community URL should contain redirect_uri="
            + COMMUNITY_HOME_URL
            + " actual: "
            + communityURL,
        communityURL.contains(
            String.format(
                "redirect_uri=%s",
                URLEncoder.encode(COMMUNITY_BASE_URL.concat(COMMUNITY_HOME_URL), "UTF8"))));
    assertTrue(
        "community URL should contain configured client_id=" + COMMUNITY_CLIENT_ID,
        communityURL.contains(String.format("client_id=%s&", COMMUNITY_CLIENT_ID)));
  }
}
