...
public class TestChangeEphemeralStore {
  ...
  public void testPreviousEphemeralBackendURL() throws Exception {
    ...
    assertEquals(
        "previous URL should be " + testUrl1, config.getPreviousEphemeralBackendURL(), testUrl1);
    config.setEphemeralBackendURL(testUrl3);
    assertEquals(
        "previous URL should be " + testUrl2, config.getPreviousEphemeralBackendURL(), testUrl2);
  }
  ...
  public void testChangeURLDuringMigration() throws Exception {
    ...
    try {
      ...
      assertTrue(
          e.getMessage().contains("attribute migration to ldap://test is currently in progress"));
    } finally {
      info.clearData();
    }
  }
