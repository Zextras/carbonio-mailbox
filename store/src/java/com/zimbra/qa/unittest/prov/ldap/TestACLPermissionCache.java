...
public class TestACLPermissionCache extends LdapTest {
  ...
  private static final String SUBGROUP_OF_GRANTTARGET_USER_GROUP =
      "subgroup-of-granttarget-user-group";
  ...
  private void grantRight(
      TargetType targetType, Entry target, GranteeType granteeType, NamedEntry grantee, Right right)
  ...
  public void testGuestAccount() throws Exception {
    ...
    grantRight(
        TargetType.account,
        grantTarget,
        GranteeType.GT_GUEST,
        grantee,
        GRANTEE_GUEST_ACCT_PASSWORD,
        right);
  ...
  public void testGrantChangeOnIndirectlyInheritedDistributionList() throws Exception {
    ...
    Domain domain = createDomain();
    DistributionList grantTarget = createUserDistributionList(GRANTTARGET_USER_GROUP, domain);
    DistributionList subGroup =
        createUserDistributionList(SUBGROUP_OF_GRANTTARGET_USER_GROUP, domain);
    DistributionList target = createUserDistributionList(TARGET_USER_GROUP, domain);
    Account grantee = createUserAccount(GRANTEE_USER_ACCT, domain);
  ...
  public void testGranteeAdminFlagChanged() throws Exception {
    ...
    try {
      ...
      if (ServiceException.PERM_DENIED.equals(e.getCode())) allow = false;
    }
    assertFalse(allow);
