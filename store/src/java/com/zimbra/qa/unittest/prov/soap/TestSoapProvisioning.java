...
public class TestSoapProvisioning {
  ...
  public void testInvokeJaxbAsAdminWithRetry() throws Exception {
    ...
    assertFalse(
        "SoapProvisioning should have a valid token after authenticating as cn=zimbra",
        soapProv.isExpired());
    ...
    soapProv.soapAdminAuthenticate(ADMIN_NAME, ADMIN_PASS);
    assertFalse(
        "SoapProvisioning should have a valid token after authenticating", soapProv.isExpired());
    req = new GetAllServersRequest(Provisioning.SERVICE_MAILBOX, false);
    resp = soapProv.invokeJaxbAsAdminWithRetry(req);
    assertNotNull(
        "GetAllServersResponse should not be null when executed by an admin account", resp);
