...
public class TestLdapProvCountObjects extends LdapTest {
  ...
  public void countAccount() throws Exception {
    ...
    Account systemResource =
        provUtil.createSystemResource(genAcctNameLocalPart("system-resource"), domain);

    // accounts should NOT be counted as internalUserAccount
    Account externalAccount =
        provUtil.createExternalAccount(genAcctNameLocalPart("external"), domain);

    // archiving account
    Map<String, Object> attrs = Maps.newHashMap();
    attrs.put(Provisioning.A_amavisArchiveQuarantineTo, "test@junk.com");
    Account archivingAcct =
        provUtil.createAccount(genAcctNameLocalPart("archiving"), domain, attrs);
  ...
  public void countDL() throws Exception {
    ...
    Group staticGroup1Sub =
        provUtil.createGroup(genGroupNameLocalPart("static-1"), subDomain, false);
    Group dynamicGroup1Sub =
        provUtil.createGroup(genGroupNameLocalPart("dynamic-1"), subDomain, true);
    Group staticGroup2Sub =
        provUtil.createGroup(genGroupNameLocalPart("static-2"), subDomain, false);
    Group dynamicGroup2Sub =
        provUtil.createGroup(genGroupNameLocalPart("dynamic-2"), subDomain, true);
