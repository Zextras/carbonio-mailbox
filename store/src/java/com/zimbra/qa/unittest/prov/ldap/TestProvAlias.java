  ...
  */
  ...
  public void testRemoveAlias_entryExist_aliasExist_aliasPointToOtherEntry() throws Exception {
    ...
    // the hacked attrs should be removed after the removeAlais call
    {
      ...
      ((LdapProv) prov)
          .getHelper()
          .modifyEntry(ldapAccount.getDN(), attributes, (Entry) ldapAccount, LdapUsage.UNITTEST);
    ...
    // remove the alias, on the "other" account, which is *not* the target for the alias we are
    // removing
  ...
  public void testRemoveAlias_entryExist_aliasExist_aliasPointToNonExistEntry() throws Exception {
    ...
    // now, hack it so the alias points to a non-existing entry
    {
      ...
      ((LdapProv) prov)
          .getHelper()
          .modifyEntry(ldapAlias.getDN(), attributes, (Entry) ldapAlias, LdapUsage.UNITTEST);
    }
  ...
  public void testRemoveAlias_entryNotExist_aliasExist_aliasPointToOtherEntry() throws Exception {
    ...
    // remove the alias, on a "not found" account, and the alias is pointing to another existing
    // target
  ...
  public void testRemoveAlias_aliasNameExistsButIsNotAnAlias() throws Exception {
    ...
    try {
      ...
      if (AccountServiceException.NO_SUCH_ALIAS.equals(e.getCode())) good = true;
    }
    assertTrue(good);

    // make sure the account is not touched
    prov.flushCache(
        CacheEntryType.account, new CacheEntry[] {new CacheEntry(Key.CacheEntryBy.id, acct2Id)});
    ...
    // mProv.flushCache(CacheEntryType.account, new CacheEntry[]{new CacheEntry(CacheEntryBy.id,
    // acct2Id)});
  ...
  public void testCreateAlias_aliasNameExistsButIsNotAnAlias() throws Exception {
    ...
    try {
      ...
      if (AccountServiceException.ACCOUNT_EXISTS.equals(e.getCode())) good = true;
    }
    assertTrue(good);
