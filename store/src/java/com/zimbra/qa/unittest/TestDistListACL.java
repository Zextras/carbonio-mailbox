...
public class TestDistListACL {
  ...
  private void doCheckSentToDistListGuestRight(
      DistributionList targetDl, String email, String guest, boolean expected)
    ...
    if (expected) {
      Assert.assertTrue(
          String.format("%s should be able to send to DL (as guest %s)", email, guest),
          accessMgr.canDo(email, group, User.R_sendToDistList, false));
      Assert.assertTrue(
          String.format("%s should have right to send to DL (as guest %s)", email, guest), result);
      ZimbraLog.test.info("Test for %s against dom %s Via=%s", email, guest, via);
    } else {
      Assert.assertFalse(
          String.format("%s should NOT be able to send to DL (because not guest %s)", email, guest),
          accessMgr.canDo(email, group, User.R_sendToDistList, false));
      Assert.assertFalse(
          String.format(
              "%s should NOT have right to send to DL (because not guest %s)", email, guest),
          result);
    }
  }

  /**
   * "gst" GranteeType testing. Sender must match the configured guest email address. The secret is
   * ignored!
   */
  @Test
  public void testMilterGuestSendToDL() throws Exception {
    ...
    prov.grantRight(
        ...
        TargetBy.name,
        listAddress,
        GranteeType.GT_GUEST.getCode(),
        GranteeBy.name,
        guestName,
        "" /* secret */,
        RightConsts.RT_sendToDistList,
        (RightModifier) null /* rightModifier */);
  ...
  private void doCheckSentToDistListEmailRight(
      DistributionList targetDl, String email, String grantEmail, boolean expected)
      throws ServiceException {
    ZimbraLog.test.info("DL name %s ID %s", targetDl.getName(), targetDl.getId());
    Group group = prov.getGroupBasic(Key.DistributionListBy.name, listAddress);
    Assert.assertNotNull("Unable to find Group object for DL by name", group);
    ...
    if (expected) {
      Assert.assertTrue(
          String.format("%s should be able to send to DL (using email %s)", email, grantEmail),
          accessMgr.canDo(email, group, User.R_sendToDistList, false));
      Assert.assertTrue(
          String.format("%s should have right to send to DL (using email %s)", email, grantEmail),
          result);
      ZimbraLog.test.info("Test for %s against dom %s Via=%s", email, grantEmail, via);
    } else {
      Assert.assertFalse(
          String.format(
              "%s should NOT be able to send to DL (because not email %s)", email, grantEmail),
          accessMgr.canDo(email, group, User.R_sendToDistList, false));
      Assert.assertFalse(
          String.format(
              "%s should NOT have right to send to DL (because not email %s)", email, grantEmail),
          result);
    }
  }

  /**
   * "email" GranteeType testing. Sender must match the configured email address - address can be
   * internal, dl, guest etc
   */
  @Test
  ...
  private void doCheckSentToDistListUserRight(
      DistributionList targetDl, String email, String user, boolean expected)
      throws ServiceException {
    ZimbraLog.test.info("DL name %s ID %s", targetDl.getName(), targetDl.getId());
    Group group = prov.getGroupBasic(Key.DistributionListBy.name, listAddress);
    Assert.assertNotNull("Unable to find Group object for DL by name", group);
    ...
    if (expected) {
      Assert.assertTrue(
          String.format("%s should be able to send to DL (as user %s)", email, user),
          accessMgr.canDo(email, group, User.R_sendToDistList, false));
      Assert.assertTrue(
          String.format("%s should have right to send to DL (as user %s)", email, user), result);
      ZimbraLog.test.info("Test for %s against dom %s Via=%s", email, user, via);
    } else {
      Assert.assertFalse(
          String.format("%s should NOT be able to send to DL (because not user %s)", email, user),
          accessMgr.canDo(email, group, User.R_sendToDistList, false));
      Assert.assertFalse(
          String.format(
              "%s should NOT have right to send to DL (because not user %s)", email, user),
          result);
    }
  }

  /** "usr" GranteeType testing. Sender must match the configured user email address. */
  @Test
  ...
  private void doCheckSentToDistListDomRight(
      DistributionList targetDl, String email, String grantDomain, boolean expected)
      throws ServiceException {
    ZimbraLog.test.info("DL name %s ID %s", targetDl.getName(), targetDl.getId());
    Group group = prov.getGroupBasic(Key.DistributionListBy.name, listAddress);
    Assert.assertNotNull("Unable to find Group object for DL by name", group);
    ...
    if (expected) {
      Assert.assertTrue(
          String.format(
              "%s should be able to send to DL (because in domain %s)", email, grantDomain),
          accessMgr.canDo(email, group, User.R_sendToDistList, false));
      Assert.assertTrue(
          String.format(
              "%s should have right to send to DL (because in domain %s)", email, grantDomain),
          result);
      ZimbraLog.test.info("Test for %s against dom %s Via=%s", email, grantDomain, via);
    } else {
      Assert.assertFalse(
          String.format(
              "%s should NOT be able to send to DL (because not in domain %s)", email, grantDomain),
          accessMgr.canDo(email, group, User.R_sendToDistList, false));
      Assert.assertFalse(
          String.format(
              "%s should NOT have right to send to DL (because not in domain %s)",
              email, grantDomain),
          result);
    }
  }
  ...
  /**
   * "dom" GranteeType testing. Sender must exist and be in the domain that is allowed to send to
   * the DL
   */
  @Test
  public void testMilterDomainSendToDL() throws Exception {
    ...
    prov.grantRight(
        ...
        TargetBy.name,
        listAddress,
        GranteeType.GT_DOMAIN.getCode(),
        GranteeBy.name,
        user1account.getDomainName(),
        null /* secret */,
        RightConsts.RT_sendToDistList,
        (RightModifier) null /* rightModifier */);
    ...
    doCheckSentToDistListDomRight(
        dl, user1email.toUpperCase(Locale.ENGLISH), user1account.getDomainName(), true);
  }

  /**
   * "dom" GranteeType testing. Sender must exist and be in the domain that is allowed to send to
   * the DL Note that currently, an alias address is resolved to the associated account before the
   * domain check is done which might or might not be the best approach.
   */
  @Test
  ...
  /**
   * "dom" GranteeType testing. Note that currently, an alias address is resolved to the associated
   * account before the domain check is done which might or might not be the best approach.
   */
  @Test
  ...
  public void testMilterExternalDomainSendToDL() throws Exception {
    ...
    prov.grantRight(
        ...
        TargetBy.name,
        listAddress,
        GranteeType.GT_EXT_DOMAIN.getCode(),
        GranteeBy.name,
        "example.test",
        null /* secret */,
        RightConsts.RT_sendToDistList,
    ...
    Assert.assertNotNull("Unable to find Group object for DL by name", group);
    Assert.assertTrue(
        "pete@example.test should be able to send to DL (in domain example.test)",
        accessMgr.canDo("pete@example.test", group, User.R_sendToDistList, false));
    Assert.assertFalse(
        String.format("%s should NOT be able to send to DL (in domain example.test)", user1email),
        accessMgr.canDo(user1email, group, User.R_sendToDistList, false));
  }

  /**
   * "edom" GranteeType testing. Addresses for local domains will also match right for "edom"
   * GranteeType (if we decide we don't want this, just testing for a guest account in ZimbraACE
   * won't be sufficient, we will need to make sure that the external domain isn't a local domain.
   */
  @Test
  public void testMilterEdomWithLocalDomain() throws Exception {
    ...
    Assert.assertTrue(
        String.format(
            "%s should be able to send to DL (in domain %s)",
