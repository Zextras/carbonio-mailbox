...
public class TestJaxb {
   ...
   */
  public static InviteComponent createInviteComponentSkeleton(
      String subject, String location, ZMailbox organizerZMbox, ZMailbox... attendeeZMboxes)
      ...
      for (ZMailbox attendeeZMbox : attendeeZMboxes) {
        inviteComp.addAttendee(
            CalendarAttendee.createForAddressDisplaynameRolePartstatRsvp(
                attendeeZMbox.getName(),
                getCN(attendeeZMbox),
                "REQ",
                ZAppointmentHit.PSTATUS_NEEDS_ACTION,
                true));
      }
    }
  ...
  public static MimePartInfo makeTextAndHtmlAlternatives(String text) {
    ...
    mimePart.addMimePart(
        MimePartInfo.createForContentTypeAndContent(
            "text/html", String.format("<html><body><p><b>%s</b></p></body></html>", text)));
    return mimePart;
  }

  public static Msg createMsgForAppointmentRequest(
      String subject, InviteComponent inviteComp, ZMailbox... attendeeZMboxes)
      throws ServiceException {
    Msg msg = new Msg();
    msg.setFolderId("10");
    ...
    if (attendeeZMboxes != null) {
      for (ZMailbox attendeeZMbox : attendeeZMboxes) {
        msg.addEmailAddress(
            EmailAddrInfo.createForAddressPersonalAndAddressType(
  ...
  public static CreateAppointmentResponse sendMeetingRequest(ZMailbox zmbox, Msg msg)
    ...
    Assert.assertNotNull(
        "JAXB CreateAppointmentResponse modified sequence ms", caResp.getModifiedSequence());
  ...
  public static ZMessage waitForInvite(ZMailbox zmbox, String subject) throws Exception {
    ...
    Assert.assertNotNull(
        String.format(
            "ZMessage from waitForInvite(mbox=%s, subject='%s')", zmbox.getName(), subject),
        msg);
    ZInvite invite = msg.getInvite();
    Assert.assertNotNull(
        String.format(
            "ZInvite from waitForInvite(mbox=%s, subject='%s')", zmbox.getName(), subject),
        invite);
    return msg;
  }
  ...
  public static RecurrenceInfo recurrence(String frequency, int interval, String until) {
    SimpleRepeatingRule repeatingRule =
        SimpleRepeatingRule.createFromFrequencyAndInterval(
  ...
  public static void setStartAndEnd(
      InviteComponent inviteComp, String tzName, GregorianCalendar start, GregorianCalendar end) {
  ...
  /**
   * Bug 96748: 1. user1 sends meeting invite to user2 2. user2 proposes new time 3. user1 accepts
   * the proposed new time and new invite is sent to user2 4. user2 accepts the new invite At step
   * 4, no acceptance message was being generated
   */
  @Test
  public void proposeNewTimeWorkflow() throws Exception {
    ...
    InviteComponent inviteComp =
        createInviteComponentSkeleton(subject, "room 101", organizerBox, attendeeBox);
    ...
    ZDateTime start =
        new ZDateTime(startDate.getTime(), false, organizerBox.getPrefs().getTimeZone());
    ...
    ZDateTime newStart =
        new ZDateTime(newStartDate.getTime(), false, organizerBox.getPrefs().getTimeZone());
    Date newEndDate = new Date(newStartDate.getTime() + Constants.MILLIS_PER_HOUR);
    ZDateTime newEnd =
        new ZDateTime(newEndDate.getTime(), false, organizerBox.getPrefs().getTimeZone());
    inviteComp.setDtStart(
        DtTimeInfo.createForDatetimeAndZone(start.getDateTime(), start.getTimeZoneId()));
    inviteComp.setDtEnd(
        DtTimeInfo.createForDatetimeAndZone(end.getDateTime(), end.getTimeZoneId()));
    sendMeetingRequest(
        organizerBox, createMsgForAppointmentRequest(subject, inviteComp, attendeeBox));
    ...
    ZMessage propNewTimeMsg =
        attendeeProposeNewTimeForMeeting(attendeeBox, organizerBox, newStart, newEnd, hit, subject);
    ...
    ZMessage attendee2ndInvite =
        organizerChangeTimeForMeeting(attendeeBox, organizerBox, newStart, newEnd, hit, subject);
  ...
  private static ZMessage sendInviteReplyAndWaitForIt(
      ZMailbox attendeeBox,
      ZMailbox organizerBox,
      String inviteMsgId,
      String subjectSuffix,
      String verb,
      DtTimeInfo exceptionId)
      throws Exception {
    SendInviteReplyRequest sirReq =
        new SendInviteReplyRequest(inviteMsgId, 0 /* componentNum */, verb);
    ...
    EmailAddrInfo attendeeAddr =
        EmailAddrInfo.createForAddressPersonalAndAddressType(
            attendeeBox.getName(), getCN(attendeeBox), "f");
    EmailAddrInfo orgAddr =
        EmailAddrInfo.createForAddressPersonalAndAddressType(
  ...
  private static ZMessage acceptInvite(
      ZMailbox attendeeBox, ZMailbox organizerBox, ZMessage inviteMsg, String subject)
      throws Exception {
    return sendInviteReplyAndWaitForIt(
        attendeeBox, organizerBox, inviteMsg.getId(), subject, "ACCEPT", null);
  }
   ...
   * @throws ServiceException
   */
  private ZMessage organizerChangeTimeForMeeting(
      ZMailbox attendeeBox,
      ZMailbox organizerBox,
      ZDateTime newStart,
      ZDateTime newEnd,
      AppointmentHitInfo hit,
      String subject)
      throws Exception {
    InviteComponent inviteComp =
        createInviteComponentSkeleton(subject, "room 101", organizerBox, attendeeBox);
    inviteComp.setDtStart(
        DtTimeInfo.createForDatetimeAndZone(newStart.getDateTime(), newStart.getTimeZoneId()));
    inviteComp.setDtEnd(
        DtTimeInfo.createForDatetimeAndZone(newEnd.getDateTime(), newEnd.getTimeZoneId()));
    ...
    msg.setSubject(subject);
    msg.setMimePart(
        makeTextAndHtmlAlternatives(
            String.format("The following meeting has been modified:\n\n%s", subject)));
    ModifyAppointmentRequest maReq =
        ModifyAppointmentRequest.createForIdModseqRevCompnumMsg(
            hit.getInvId(),
            hit.getModifiedSequence(),
            hit.getRevision(),
            hit.getComponentNum(),
            msg);
  ...
  private List<AppointmentHitInfo> findMatchingExpandedAppointments(
      ZMailbox mbox, Date startDate, Date endDate, String search, int expected)
      throws ServiceException {
  ...
  private AppointmentHitInfo findMatchingAppointment(
      ZMailbox mbox, Date startDate, Date endDate, String subject) throws ServiceException {
    List<AppointmentHitInfo> hits =
        findMatchingExpandedAppointments(mbox, startDate, endDate, subject, 1);
    AppointmentHitInfo hit = hits.get(0);
    Assert.assertEquals(
        "Matching first instance start time",
        startDate.getTime(),
        hit.getInstances().get(0).getStartTime().longValue());
    return hits.get(0);
  }

  /**
   * @param hit - From search response - represents the attendee's calendar copy
   * @return Message representing the Organizer's intray copy of the new proposal, once it has
   *     arrived
   */
  private ZMessage attendeeProposeNewTimeForMeeting(
      ZMailbox attendeeBox,
      ZMailbox organizerBox,
      ZDateTime newStart,
      ZDateTime newEnd,
    ...
    msg.setInvite(invite);
    CounterAppointmentResponse sirResp =
        attendeeBox.invokeJaxb(
  ...
  private void validateException(
      ZMailbox zmbox,
      InstanceDataInfo instance,
      Map<ZMailbox, String> partstats,
      GregorianCalendar recurIdTime)
      throws ServiceException {
    ...
    List<InviteComponentWithGroupInfo> eInviteComps =
        gmeResp.getMsg().getInvite().getInviteComponents();
    ...
    for (CalendarAttendeeWithGroupInfo eAttendee : eAttendees) {
      ...
      for (Entry<ZMailbox, String> entry : partstats.entrySet()) {
        if (addr.equals(entry.getKey().getName())) {
          Assert.assertEquals(
              String.format("Exception partstat for %s", entry.getKey()), entry.getValue(), ptst);
          found = true;
        }
      }
  ...
  /**
   * Bug 94018. Accepting series, then declining single instance leads to inconsistent display of
   * attendee status for organizer copy for the declined instance.
   *
   * <p>Pseudo exceptions should be created for each reply to an individual instance of the series.
   *
   * <p>At the end of this, check that exceptions have been created. Check that the exceptions
   * register the correct statuses for the users.
   */
  @Test
  public void acceptAndTentativeSeriesDeclineInstanceAcceptInstance() throws Exception {
    ...
    InviteComponent inviteComp =
        createInviteComponentSkeleton(
            subject, "room 101", organizerZMbox, attendee1ZMbox, attendee2ZMbox);
    GregorianCalendar start =
        startTime(londonTZ, year, 1 /* February */, 2 /* day */, 14 /* hour */, 0);
    ...
    List<AppointmentHitInfo> apptHits =
        findMatchingExpandedAppointments(
            organizerZMbox,
            plus(start, Calendar.DAY_OF_MONTH, 1).getTime(),
            plus(start, Calendar.DAY_OF_MONTH, 13).getTime(),
            String.format("in:Calendar and subject:%s", subject),
            1);
    ...
    Assert.assertEquals(
        "Organizer Calendar at end - number of instances in expansion", 6, instances.size());
    ...
    List<InviteComponentWithGroupInfo> sInviteComps =
        gmsResp.getMsg().getInvite().getInviteComponents();
    ...
    for (CalendarAttendeeWithGroupInfo sAttendee : sAttendees) {
      ...
      if (addr.equals(attendee1ZMbox.getName())) {
        ...
        Assert.assertEquals(
            "exception attendee2 partstat", ZAppointmentHit.PSTATUS_TENTATIVE, ptst);
      } else {
  ...
  private static long ymdStringToDate(String ymdString) {
    ...
    try {
      ...
      Assert.fail(
          String.format(
              "Failed to convert string %s to long - exception %s", ymdString, e.getMessage()));
    }
    return 0;
  }
  ...
  private Element sendReqExpectingFail(
      String requestBody, String requestCommand, int expectedRespCode)
      throws HttpException, IOException, ServiceException {
    HttpClient client = HttpClientBuilder.create().build();
    HttpPost post = new HttpPost(TestUtil.getSoapUrl() + requestCommand);
    post.setEntity(new StringEntity(requestBody, "application/soap+xml", "UTF-8"));
  ...
  public BrowseResponse doBrowseRequest(BrowseRequest browseRequest) throws Exception {
    ...
    return (BrowseResponse)
        sendReq(envelope(authToken, doc.getRootElement().asXML()), "BrowseRequest");
  }
  ...
  public void testBrowseRequestAttachments() throws Exception {
    ...
    String raw =
        mb.withSubject(NAME_PREFIX)
            .withBody(bodyWithObject)
            .withContentType(MimeConstants.CT_APPLICATION_PDF)
            .create();
    TestUtil.addRawMessage(zmbox, raw);
    forceIndexing(USER_NAME);
    BrowseRequest browseRequest =
        new BrowseRequest("attachments" /* browseBy */, "" /* regex */, 10);
    BrowseResponse browseResponse = doBrowseRequest(browseRequest);
    Assert.assertNotNull("JAXB BrowseResponse object", browseResponse);
    List<BrowseData> datas = browseResponse.getBrowseDatas();
    Assert.assertNotNull("JAXB BrowseResponse datas", datas);
  ...
  static String bodyWithObject =
      "contains http://www.example.com/fun so treat as containing com_zimbra_url object";
  ...
  public void testBrowseRequestObjects() throws Exception {
    ...
    String raw =
        mb.withSubject(NAME_PREFIX)
            .withBody(bodyWithObject)
            .withContentType(MimeConstants.CT_TEXT_PLAIN)
            .create();
    TestUtil.addRawMessage(zmbox, raw);
    forceIndexing(USER_NAME);
  ...
  public Element doBadBrowseRequest(BrowseRequest browseRequest) throws Exception {
    ...
    Document doc = dr.getDocument();
    ZimbraLog.test.debug(doc.getRootElement().asXML());
    return sendReqExpectingFail(
        envelope(authToken, doc.getRootElement().asXML()), "BrowseRequest", 500);
  }
  ...
  public void testBrowseRequestDomainsBadRegex() throws Exception {
    ...
    forceIndexing(USER_NAME);
    BrowseRequest browseRequest =
        new BrowseRequest("domains" /* browseBy */, BAD_REGEX /* regex */, 10);
    Element envelope = doBadBrowseRequest(browseRequest);
    Assert.assertNotNull("Envelope", envelope);
    Assert.assertTrue(
        "Error contained in SOAP response",
        envelope
            .toString()
            .contains("regular expression match involved more than 100000 accesses for pattern"));
  }
  ...
  public void testBrowseRequestObjectsBadRegex() throws Exception {
    ...
    BrowseResponse browseResponse = doBrowseRequest(browseRequest);
    Assert.assertTrue(
        "BrowseDatas size should be greater than 1", browseResponse.getBrowseDatas().size() >= 1);
  ...
  public void testBrowseRequestAttachmentsBadRegex() throws Exception {
    ...
    forceIndexing(USER_NAME);
    BrowseRequest browseRequest =
        new BrowseRequest("attachments" /* browseBy */, BAD_REGEX /* regex */, 10);
    Element envelope = doBadBrowseRequest(browseRequest);
    Assert.assertNotNull("Envelope", envelope);
    Assert.assertTrue(
        "Error contained in SOAP response",
  ...
  public void testGetFolderWithCacheContainingNullParent() throws Exception {
    ...
    // Bug 100588 f1folder/f2folder will have been retrieved from the folder cache.  Deliberately
    // poison it
    // by setting the parent folder to null, and ensure that can still successfully do a
    // GetFolderRequest
