...
public class TestFolderFilterRules {
  ...
  /** Tests moving to a new parent folder and changing the folder name to upper-case. */
  @Test
  ...
  /**
   * Confirms that when a folder is deleted, any rules that filed into that folder or its subfolders
   * are disabled (bug 17797).
   */
  @Test
  ...
  /**
   * Confirms that when a folder moved to the trash, any rules that filed into that folder or its
   * subfolders are disabled (bug 17797).
   */
  @Test
  ...
  /** Renames the given folder and confirms that filter rules were updated with the new path. */
  private void renameFolder(String folderId, String newName, String newParentId) throws Exception {
    ...
    assertTrue(
        "Could not find path " + oldPath + " in script: " + script, script.contains(oldPath));
    ...
    assertTrue(
        "Folder path '" + newPath + "' does not end with " + newName,
    ...
    assertFalse(
        "Found old path '" + oldPath + " in script: " + script, script.indexOf(oldPath) >= 0);
    assertTrue(
        "Could not find new path '" + newPath + " in script: " + script,
        script.indexOf(newPath) >= 0);
  }
  ...
  private void moveFolder(String folderId, String newParentFolderId) throws Exception {
    ...
    assertTrue(
        "Folder path '" + newPath + "' does not start with " + newParentPath,
