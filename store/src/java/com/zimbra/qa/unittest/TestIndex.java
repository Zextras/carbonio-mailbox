...
public class TestIndex {
  ...
  public void setUp() throws Exception {
    mOriginalTextLimit =
        Integer.parseInt(TestUtil.getServerAttr(Provisioning.A_zimbraAttachmentsIndexedTextLimit));
  ...
  public void testIndexedTextLimit() throws Exception {
    ...
    String msgId =
        sendMessage(
                subject, body.toString().getBytes(), "attachment.txt", MimeConstants.CT_TEXT_PLAIN)
            .getId();
    ...
    msgId =
        sendMessage(subject, htmlBody.getBytes(), "attachment.html", MimeConstants.CT_TEXT_HTML)
            .getId();
    checkQuery("in:inbox subject:\"" + subject + "\" walrus", msgId);
    checkQuery("in:inbox subject:\"" + subject + "\" joob", null);
    ...
    msgId =
        sendMessage(
                subject, attachedMsg.getBytes(), "attachment.msg", MimeConstants.CT_MESSAGE_RFC822)
            .getId();
  ...
  public void testFilenameSearch() throws Exception {
    ...
    TestUtil.createDocument(
        ...
        filename,
        "text/plain",
        "This is the data for testFilenameSearch.".getBytes());
    Assert.assertEquals(
        0, TestUtil.search(mbox, "filename:Blob*", ZSearchParams.TYPE_DOCUMENT).size());
    Assert.assertEquals(
        1,
        TestUtil.search(mbox, "filename:\"" + filename + "\"", ZSearchParams.TYPE_DOCUMENT).size());
  }
  ...
  private void setTextLimit(int numBytes) throws Exception {
    TestUtil.setServerAttr(
        Provisioning.A_zimbraAttachmentsIndexedTextLimit, Integer.toString(numBytes));
  }
