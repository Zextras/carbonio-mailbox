...
public class TestCalDav {
  ...
  public static final String expandPropertyGroupMemberSet =
      "<?xml version=\"1.0\" encoding=\"UTF-8\"?><A:expand-property xmlns:A=\"DAV:\">  <A:property"
          + " name=\"group-member-set\" namespace=\"DAV:\">    <A:property"
          + " name=\"email-address-set\" namespace=\"http://calendarserver.org/ns/\"/>   "
          + " <A:property name=\"calendar-user-address-set\""
          + " namespace=\"urn:ietf:params:xml:ns:caldav\"/>    <A:property name=\"displayname\""
          + " namespace=\"DAV:\"/>  </A:property></A:expand-property>";

  public static final String expandPropertyDelegateFor =
      "<A:expand-property xmlns:A=\"DAV:\">  <A:property name=\"calendar-proxy-write-for\""
          + " namespace=\"http://calendarserver.org/ns/\">    <A:property name=\"displayname\""
          + " namespace=\"DAV:\"/>    <A:property name=\"calendar-user-address-set\""
          + " namespace=\"urn:ietf:params:xml:ns:caldav\"/>    <A:property"
          + " name=\"email-address-set\" namespace=\"http://calendarserver.org/ns/\"/> "
          + " </A:property>  <A:property name=\"calendar-proxy-read-for\""
          + " namespace=\"http://calendarserver.org/ns/\">    <A:property name=\"displayname\""
          + " namespace=\"DAV:\"/>    <A:property name=\"calendar-user-address-set\""
          + " namespace=\"urn:ietf:params:xml:ns:caldav\"/>    <A:property"
          + " name=\"email-address-set\" namespace=\"http://calendarserver.org/ns/\"/> "
  ...
  public static String propFindEtagResType =
  ...
  public static class NamespaceContextForXPath implements javax.xml.namespace.NamespaceContext {
    ...
    public String getNamespaceURI(String prefix) {
      ...
      if (nsURI == null) {
        ...
        ZimbraLog.test.info(
            "NamespaceContextForXPath.getNamespaceURI(prefix) - Unexpected prefix %s", prefix);
      }
      return nsURI;
    }
  ...
  public static ParsedDateTime parsedDateTime(
      int year, int month, int day, int hour, int min, ICalTimeZone icalTimeZone) {
  ...
  public static ZProperty attendee(
      MailTarget mailTarget, ICalTok role, ICalTok cutype, ICalTok partstat) {
  ...
  public String exampleCancelIcal(Account organizer, Account attendee1, Account attendee2)
      throws IOException {
    ...
    vevent.addProperty(
        attendee(attendee1, ICalTok.REQ_PARTICIPANT, ICalTok.INDIVIDUAL, ICalTok.NEEDS_ACTION));
    vevent.addProperty(
        attendee(attendee2, ICalTok.REQ_PARTICIPANT, ICalTok.INDIVIDUAL, ICalTok.NEEDS_ACTION));
  ...
  public static class HttpMethodExecutor {
    ...
    public HttpMethodExecutor(HttpClient client, HttpRequestBase method, int expectedCode)
        throws Exception {
        ...
        try (InputStream responseStream = httpResp.getEntity().getContent()) {
          if (responseStream == null) {
            ...
            ZimbraLog.test.debug(
                "RESPONSE:\n%s\n%s\n%s\n\n", statusLine, hdrsSb, new String(responseBodyBytes));
          }
        }
    ...
    public static HttpMethodExecutor execute(
        HttpClient client, HttpRequestBase method, int expectedCode) throws IOException {
      try {
    ...
    public String getNonNullHeaderValue(String hdrName, String desc) {
      ...
      assertNotNull(
          String.format(
              "%s:response for method '%s' missing header '%s'", desc, methodName, hdrName));
      return val;
    }
  ...
  public void testPostToSchedulingOutbox() throws Exception {
    ...
    method.setEntity(
        new ByteArrayEntity(
            exampleCancelIcal(dav1, dav2, dav3).getBytes(),
  ...
  public static void addBasicAuthHeaderForUser(
      HttpRequestBase method, Account acct, String password) throws UnsupportedEncodingException {
    String basicAuthEncoding =
        DatatypeConverter.printBase64Binary(
            String.format("%s:%s", acct.getName(), password).getBytes("UTF-8"));
    method.addHeader("Authorization", "Basic " + basicAuthEncoding);
  }

  public static void addBasicAuthHeaderForUser(HttpRequestBase method, Account acct)
  ...
  public static Document doMethodYieldingMultiStatus(
      HttpEntityEnclosingRequestBase method, Account acct, String body)
    ...
    method.setEntity(
        new ByteArrayEntity(
            body.getBytes(), org.apache.http.entity.ContentType.create(MimeConstants.CT_TEXT_XML)));
    try {
   ...
   * @throws IOException
   */
  public static String waitForItemInCalendarCollectionByUID(
      String url, Account acct, String UID, boolean expected, int timeout_millis)
          ...
          for (int ndx = 0; ndx < result.getLength(); ndx++) {
            ...
            if ((Strings.isNullOrEmpty(UID)) || (nodeValue.contains(UID))) {
              if (!expected) {
                fail(
                    String.format(
                        "item with UID '%s' unexpectedly arrived in collection '%s' within %d"
                            + " millisecs",
    ...
    if (expected) {
      fail(
          String.format(
              "item with UID '%s' didn't arrive in collection '%s' within %d millisecs",
  ...
  public void testCalendarQueryOnInbox() throws Exception {
    ...
    assertFalse(
        "response when there are no items should have no child elements", rootElem.hasChildNodes());
  ...
  public void testCalendarQueryOnOutbox() throws Exception {
    ...
    assertFalse(
        "response for items in outbox should have no child elements, even though we sent an invite",
  ...
  public void testPropFindSupportedReportSetOnInbox() throws Exception {
    ...
    checkPropFindSupportedReportSet(
        user1,
        getSchedulingInboxUrl(user1, user1),
  ...
  public void testPropFindSupportedReportSetOnOutbox() throws Exception {
    ...
    checkPropFindSupportedReportSet(
        user1,
        getSchedulingOutboxUrl(user1, user1),
  ...
  public void checkPropFindSupportedReportSet(Account user, String fullurl, String shorturl)
    ...
    method.setEntity(
        new ByteArrayEntity(
            propFindSupportedReportSet.getBytes(),
        ...
        if (DavElements.P_HREF.equals(respEntry.getName())) {
          ...
          for (Element psEntry : respEntry.listElements()) {
            if (DavElements.P_STATUS.equals(psEntry.getName())) {
              ...
              for (Element propEntry : psEntry.listElements()) {
                if (DavElements.P_SUPPORTED_REPORT_SET.equals(propEntry.getName())) {
                  for (Element suppRepSetEntry : propEntry.listElements()) {
                    assertEquals(
                        "supported-report-set child",
                        DavElements.P_SUPPORTED_REPORT,
                        suppRepSetEntry.getName());
                    for (Element suppRepEntry : suppRepSetEntry.listElements()) {
                      assertEquals(
                          "supported-report child", DavElements.P_REPORT, suppRepEntry.getName());
              ...
              fail(
                  "Unexpected element '" + psEntry.getName() + "' under " + DavElements.P_PROPSTAT);
            }
          }
        } else {
  ...
  public void checkPropFindSupportedCalendarComponentSet(
      Account user, String fullurl, String shorturl, String[] compNames) throws Exception {
    PropFindMethod method = new PropFindMethod(fullurl);
    addBasicAuthHeaderForUser(method, user);
    HttpClient client = HttpClientBuilder.create().build();
    TestCalDav.HttpMethodExecutor executor;
    ...
    method.setEntity(
        new ByteArrayEntity(
            propFindSupportedCalendarComponentSet.getBytes(),
            org.apache.http.entity.ContentType.create(MimeConstants.CT_TEXT_XML)));
    executor = new TestCalDav.HttpMethodExecutor(client, method, HttpStatus.SC_MULTI_STATUS);
    ...
    xPathExpr =
        xpath.compile(
            "/D:multistatus/D:response/D:propstat/D:prop/C:supported-calendar-component-set/C:comp");
    result = (NodeList) xPathExpr.evaluate(doc, XPathConstants.NODESET);
    assertEquals(
        "Number of comp nodes under supported-calendar-component-set",
        compNames.length,
        result.getLength());
    ...
    for (int ndx = 0; ndx < result.getLength(); ndx++) {
      ...
      assertTrue(
          String.format("comp 'name' attribute '%s' should be one of the expected names", name),
          names.contains(name));
    }
  }
  ...
  public void testPropFindSupportedCalendarComponentSetOnInbox() throws Exception {
    assertNotNull("Test account object", user1);
    checkPropFindSupportedCalendarComponentSet(
        user1,
        getSchedulingInboxUrl(user1, user1),
        UrlNamespace.getSchedulingInboxUrl(user1.getName(), user1.getName()),
  ...
  public void testPropFindSupportedCalendarComponentSetOnCalendar() throws Exception {
    ...
    checkPropFindSupportedCalendarComponentSet(
        user1,
        getFolderUrl(user1, "Calendar"),
        UrlNamespace.getFolderUrl(user1.getName(), "Calendar"),
        eventComponents);
  }
  ...
  public void testPropFindSupportedCalendarComponentSetOnTasks() throws Exception {
    ...
    checkPropFindSupportedCalendarComponentSet(
        user1,
        getFolderUrl(user1, "Tasks"),
        UrlNamespace.getFolderUrl(user1.getName(), "Tasks"),
        todoComponents);
  }
   ...
   * http://pan.local:7070/dav/dav1@pan.local/Calendar/d123f102-42a7-4283-b025-3376dabe53b3.ics at
   * com.zimbra.cs.dav.resource.CalendarCollection.createItem(CalendarCollection.java:431) at
   * com.zimbra.cs.dav.service.method.Put.handle(Put.java:49) at
   * com.zimbra.cs.dav.service.DavServlet.service(DavServlet.java:322)
   */
  @Test
  public void testCreateUsingClientChosenName() throws ServiceException, IOException {
    ...
    propFindMethod.setEntity(
        new ByteArrayEntity(
            propFindEtagResType.getBytes(),
            org.apache.http.entity.ContentType.create(MimeConstants.CT_TEXT_XML)));
    try {
      executor =
          new TestCalDav.HttpMethodExecutor(client, propFindMethod, HttpStatus.SC_MULTI_STATUS);
      respBody = new String(executor.responseBodyBytes, MimeConstants.P_CHARSET_UTF8);
      respElem = Element.XMLElement.parseXML(respBody);
      assertEquals(
          "name of top element in propfind response",
          DavElements.P_MULTISTATUS,
          respElem.getName());
      ...
      assertTrue(
          String.format(
              "PROPFIND RESPONSE should contain href for '%s' - only contained hrefs:%s",
              calFolderUrl, Joiner.on(',').join(hrefs)),
          hasCalendarHref);
      assertTrue(
          String.format(
              "PROPFIND RESPONSE should contain href for '%s' - only contained hrefs:%s",
              url, Joiner.on(',').join(hrefs)),
          hasCalItemHref);
  ...
  public HttpMethodExecutor doIcalPut(String url, Account authAcct, byte[] vcalendar, int expected)
    ...
    putMethod.setEntity(
        new ByteArrayEntity(
            vcalendar, org.apache.http.entity.ContentType.create(MimeConstants.CT_TEXT_CALENDAR)));
    return HttpMethodExecutor.execute(client, putMethod, expected);
  }

  public HttpMethodExecutor doGetMethod(String url, Account authAcct, int expected)
      throws IOException {
    HttpClient client = HttpClientBuilder.create().build();
  ...
  public HttpMethodExecutor doDeleteMethod(String url, Account authAcct, int expected)
      throws IOException {
    HttpClient client = HttpClientBuilder.create().build();
  ...
  /**
   * Mostly checking that if attendees cease to exist (even via DLs) then modification and cancel
   * iTip messages still work to the remaining attendees.
   */
  @Test
  public void testCreateModifyDeleteAttendeeModifyAndCancel() throws ServiceException, IOException {
    ...
    doDeleteMethod(
        getLocalServerRoot().append(inboxhref).toString(), dav2, HttpStatus.SC_NO_CONTENT);

    // attendee via DL
    inboxhref = TestCalDav.waitForNewSchedulingRequestByUID(dav4, uid);
    assertTrue("Found meeting request for newly created item", inboxhref.contains(uid));
    doDeleteMethod(
        getLocalServerRoot().append(inboxhref).toString(), dav4, HttpStatus.SC_NO_CONTENT);
  ...
  public void testAndroidMeetingSeries() throws Exception {
    ...
    String body =
        androidSeriesMeetingTemplate
            .replace("%%ORG%%", dav1.getName())
    ...
    assertTrue(
        "Found meeting request for newly created item",
        inboxhref.contains(androidSeriesMeetingUid));
    ...
    respElem = Element.XMLElement.parseXML(respBody);
    assertEquals(
        "name of top element in propfind response", DavElements.P_MULTISTATUS, respElem.getName());
    assertTrue("propfind response should have child elements", respElem.hasChildren());
    Iterator<Element> iter = respElem.elementIterator();
    boolean hasCalendarHref = false;
    boolean hasCalItemHref = false;
  ...
  public ZVCalendar simpleMeeting(
      Account organizer, List<MailTarget> attendees, String seq, int startHour) throws IOException {
    return simpleMeeting(organizer, attendees, LdapUtil.generateUUID(), seq, startHour);
  }

  public ZVCalendar simpleMeeting(
      Account organizer, List<MailTarget> attendees, String uid, String seq, int startHour)
      throws IOException {
    ZVCalendar vcal = new ZVCalendar();
    vcal.addVersionAndProdId();
    vcal.addProperty(new ZProperty(ICalTok.METHOD, ICalTok.PUBLISH.toString()));
    ...
    for (MailTarget att : attendees) {
      if (att.getId().equals(organizer.getId())) {
        vevent.addProperty(
            attendee(att, ICalTok.REQ_PARTICIPANT, ICalTok.INDIVIDUAL, ICalTok.ACCEPTED));
      } else {
        vevent.addProperty(
  ...
  public String makeFreeBusyRequestIcal(
      Account organizer, List<Account> attendees, Date start, Date end) throws IOException {
    ZVCalendar vcal = new ZVCalendar();
    vcal.addVersionAndProdId();
  ...
  public HttpMethodExecutor doFreeBusyCheck(
      Account organizer, List<Account> attendees, Date start, Date end)
      throws ServiceException, IOException {
    HttpClient client = HttpClientBuilder.create().build();
  ...
  /**
   * http://tools.ietf.org/html/draft-desruisseaux-caldav-sched-03#section-5.3.1 5.3. Scheduling
   * Inbox Properties 5.3.1. CALDAV:calendar-free-busy-set Property Purpose: Identify the calendars
   * that contribute to the free-busy information for the calendar user associated with the
   * scheduling Inbox.
   *
   * <p>If the list is empty - NO calendars affect freebusy. If the list is not empty, each listed
   * calendar affects freebusy. Bug 85275 - Apple Calendar specifies URLs with "@" encoded as %40 -
   * causing us to drop all calendar from FB set
   */
  @Test
  public void testPropPatchCalendarFreeBusySetSettingUsingEscapedUrls() throws Exception {
    ...
    String fbResponse;
    fbResponse =
        doFreeBusyCheck(dav1, Lists.newArrayList(dav1), fbStartDate, fbEndDate)
            .getResponseAsString();
    assertTrue(
        String.format(
            "First FB check Response [%s] should contain [%s]", fbResponse, busyTentativeMarker),
    ...
    assertFalse(
        String.format(
            "2nd FB check after disabling - Response [%s] should NOT contain [%s]",
            fbResponse, busyTentativeMarker),
        fbResponse.contains(busyTentativeMarker));

    String enableWithRawAt =
        String.format(enableFreeBusyTemplateXml, dav1.getName(), dav1.getName());
    ...
    assertTrue(
        String.format(
            "3rd FB check after enabling Response [%s] should contain [%s]",
            fbResponse, busyTentativeMarker),
        fbResponse.contains(busyTentativeMarker));
    ...
    assertFalse(
        String.format(
            "4th FB check after disabling - Response [%s] should NOT contain [%s]",
            fbResponse, busyTentativeMarker),
        fbResponse.contains(busyTentativeMarker));
    ...
    assertTrue(
        String.format(
            "4th FB check after enabling (encoded urls) Response [%s] should contain [%s]",
            fbResponse, busyTentativeMarker),
        fbResponse.contains(busyTentativeMarker));
  }
  ...
  public void testFuzzyTimeZoneMatchGMT06() throws Exception {
    try (ByteArrayInputStream bais = new ByteArrayInputStream(VtimeZoneGMT_0600_0500.getBytes())) {
      ...
      assertEquals(
          "ID of Timezone which fuzzy matches GMT=06.00/-05.00",
          "America/Chicago",
          matchtz.getID());
    }
  }
  ...
  public void testFuzzyTimeZoneMatchGMT08() throws Exception {
    try (ByteArrayInputStream bais = new ByteArrayInputStream(VtimeZoneGMT_0800_0700.getBytes())) {
      ...
      ICalTimeZone matchtz = ICalTimeZone.lookupMatchingWellKnownTZ(tz);
      assertEquals(
          "ID of Timezone which fuzzy matches GMT=08.00/-07.00",
          "America/Los_Angeles",
          matchtz.getID());
    }
  }
  ...
  public void testLondonTimeZoneCalledGMTkeepSameName() throws Exception {
    try (ByteArrayInputStream bais =
        new ByteArrayInputStream(LOTUS_NOTES_WITH_BAD_GMT_TZID.getBytes())) {
      ZVCalendar tzcal = ZCalendar.ZCalendarBuilder.build(bais, MimeConstants.P_CHARSET_UTF8);
      assertNotNull("tzcal", tzcal);
      ZComponent tzcomp = tzcal.getComponent(ICalTok.VTIMEZONE);
      assertNotNull("tzcomp", tzcomp);
      ...
      assertEquals(
          "that London Timezone originally with TZID=GMT maps to this daylightTzname",
          "GMT/BST",
  ...
  public void testLondonTimeZoneCalledGMT() throws Exception {
    try (ByteArrayInputStream bais =
      ...
      assertEquals(
          "ID that London Timezone originally with TZID=GMT maps to", "Europe/London", tz.getID());
      assertEquals(
          "that London Timezone originally with TZID=GMT maps to this daylightTzname",
          "GMT/BST",
          tz.getDaylightTzname());
  ...
  private void attendeeDeleteFromCalendar(boolean suppressReply) throws Exception {
    ...
    String subject =
        String.format(
            ...
            TEST_NAME,
            suppressReply
                ? "testInvite which shouldNOT be replied to"
                : "testInvite to be auto-declined");
    Date startDate = new Date(System.currentTimeMillis() + Constants.MILLIS_PER_DAY);
    Date endDate = new Date(startDate.getTime() + Constants.MILLIS_PER_HOUR);
    TestUtil.createAppointment(organizer, subject, dav1.getName(), startDate, endDate);
    ...
    if (suppressReply) {
      ...
      msgs =
          TestUtil.waitForMessages(
              organizer, "is:invite is:unread inid:2 after:\"-1month\"", 0, 2000);
      if (msgs != null) {
      ...
      msgs =
          TestUtil.waitForMessages(
              organizer, "is:invite is:unread inid:2 after:\"-1month\"", 1, 10000);
  ...
  private void setZimbraPrefAppleIcalDelegationEnabled(ZMailbox mbox, Boolean val)
    ...
    Pref pref =
        Pref.createPrefWithNameAndValue(
            ZAttrProvisioning.A_zimbraPrefAppleIcalDelegationEnabled, val.toString().toUpperCase());
    modPrefsReq.addPref(pref);
    ModifyPrefsResponse modPrefsResp = mbox.invokeJaxb(modPrefsReq);
    assertNotNull(
        "null ModifyPrefs Response for forwarding calendar invites/no auto-add", modPrefsResp);
  }

  /**
   * http://svn.calendarserver.org/repository/calendarserver/CalendarServer/trunk/doc/Extensions/caldav-proxy.txt
   * This is an Apple standard implemented by Apple Mac OSX at least up to Yosemite and offers a
   * fairly simple sharing model for calendars. The model is simpler than Zimbra's native model and
   * there are mismatches, for instance Zimbra requires the proposed delegate to accept shares.
   */
  @Test
  public void testAppleCaldavProxyFunctions() throws ServiceException, IOException {
    ...
    List<ZMessage> msgs =
        TestUtil.waitForMessages(
            mboxSharee1, "in:inbox subject:\"Share Created: Calendar shared by \"", 1, 10000);
    ...
    msgs =
        TestUtil.waitForMessages(
            mboxSharee2, "in:inbox subject:\"Share Created: Calendar shared by \"", 1, 10000);
    ...
    try {
      ...
      NodeList result = (NodeList) xPathExpr.evaluate(doc, XPathConstants.NODESET);
      assertEquals(
          String.format("num XPath nodes for %s for %s", xpathS, sharee1.getName()),
          1,
          result.getLength());
      String text = (String) xPathExpr.evaluate(doc, XPathConstants.STRING);
      assertEquals(
          "HREF for account owner",
          UrlNamespace.getPrincipalUrl(sharee1).replaceAll("@", "%40"),
          text);

      xpathS =
          "/D:multistatus/D:response/D:propstat/D:prop/CS:calendar-proxy-write-for/D:response/D:href/text()";
      xPathExpr = xpath.compile(xpathS);
      result = (NodeList) xPathExpr.evaluate(doc, XPathConstants.NODESET);
      assertEquals(
          String.format("num XPath nodes for %s for %s", xpathS, sharee1.getName()),
      ...
      text = (String) xPathExpr.evaluate(doc, XPathConstants.STRING);
      assertEquals(
          "HREF for sharer", UrlNamespace.getPrincipalUrl(sharer).replaceAll("@", "%40"), text);
    } catch (XPathExpressionException e1) {
      ZimbraLog.test.debug("xpath problem", e1);
    }
  ...
  public static Document groupMemberSetExpandProperty(
      Account acct, Account member, boolean proxyWrite) throws IOException, ServiceException {
    String url =
        proxyWrite
            ? TestCalDav.getCalendarProxyWriteUrl(acct)
            : TestCalDav.getCalendarProxyReadUrl(acct);
    url = url.replaceAll("@", "%40");
    String href =
        proxyWrite
            ? UrlNamespace.getCalendarProxyWriteUrl(acct, acct)
    ...
    method.setEntity(
        new ByteArrayEntity(
            TestCalDav.expandPropertyGroupMemberSet.getBytes(),
            org.apache.http.entity.ContentType.create(MimeConstants.CT_TEXT_XML)));
    try {
      executor = new TestCalDav.HttpMethodExecutor(client, method, HttpStatus.SC_MULTI_STATUS);
      ...
      try {
        ...
        xpathS =
            "/D:multistatus/D:response/D:propstat/D:prop/D:group-member-set/D:response/D:href/text()";
        xPathExpr = xpath.compile(xpathS);
        text = (String) xPathExpr.evaluate(doc, XPathConstants.STRING);
        assertEquals(
            "HREF for sharee", UrlNamespace.getPrincipalUrl(member).replaceAll("@", "%40"), text);
      } catch (XPathExpressionException e1) {
        ZimbraLog.test.debug("xpath problem", e1);
      }
      return doc;
  ...
  public static Document delegateForExpandProperty(Account acct)
    ...
    method.setEntity(
        new ByteArrayEntity(
            expandPropertyDelegateFor.getBytes(),
            org.apache.http.entity.ContentType.create(MimeConstants.CT_TEXT_XML)));
    try {
      executor = new TestCalDav.HttpMethodExecutor(client, method, HttpStatus.SC_MULTI_STATUS);
      return executor.getResponseDoc(DavElements.P_MULTISTATUS);
  ...
  public static CreateMountpointResponse createCalendarMountPoint(
      ZMailbox mboxSharee, Account sharer) throws ServiceException {
    ...
    assertNotNull(
        String.format(
            "CreateMountPointResponse for %s's Calendar on mbox %s",
            sharer.getName(), mboxSharee.getName()),
        createMpResp);
    return createMpResp;
  }
  ...
  public static Document setGroupMemberSet(String url, Account acct, Account memberAcct)
    ...
    method.addHeader("Content-Type", MimeConstants.CT_TEXT_XML);
    String body =
        TestCalDav.propPatchGroupMemberSetTemplate.replace(
            "%%MEMBER%%", UrlNamespace.getPrincipalUrl(memberAcct, memberAcct));
    method.setEntity(
        new ByteArrayEntity(
            body.getBytes(), org.apache.http.entity.ContentType.create(MimeConstants.CT_TEXT_XML)));
    try {
