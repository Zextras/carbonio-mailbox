...
public class TestDocument {
  ...
  public void testNote() throws Exception {
    ...
    ZDocument doc =
        TestUtil.createDocument(
            mbox, folderId, NAME_PREFIX + "-doc.txt", "text/plain", "doc".getBytes());
    ZDocument note =
        TestUtil.createDocument(
            mbox, folderId, NAME_PREFIX + "-note.txt", "text/plain", "note".getBytes(), true);
    ...
    List<String> ids =
        TestUtil.search(mbox, "in:briefcase tag:\\note", ZSearchParams.TYPE_DOCUMENT);
  ...
  public void testMoveNote() throws Exception {
    ...
    String folderId = Integer.toString(Mailbox.ID_FOLDER_BRIEFCASE);
    ZDocument note =
        TestUtil.createDocument(zmbx, folderId, filename, "text/plain", "note".getBytes(), true);
    String flags = Character.toString(ZItem.Flag.NOTE.getFlagChar());
    ...
    String idStr =
        TestUtil.search(remoteZmbx, "in:briefcase " + filename, ZSearchParams.TYPE_DOCUMENT).get(0);
    ...
    remoteZmbx.moveItem(
        String.valueOf(doc.getId()), acct.getId() + ":" + Mailbox.ID_FOLDER_BRIEFCASE, null);
  ...
  public void testContentType() throws Exception {
    ...
    ZDocument doc1 =
        TestUtil.createDocument(
            mbox,
            folderId,
            NAME_PREFIX + "-docOne.doc",
            "application/octet-stream",
            "doc1".getBytes());
    ZDocument doc2 =
        TestUtil.createDocument(
            mbox, folderId, NAME_PREFIX + "-docTwo.xls", "application/ms-tnef", "doc2".getBytes());
