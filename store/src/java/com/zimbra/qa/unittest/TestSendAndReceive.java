...
public class TestSendAndReceive {
  ...
  /** Verifies that we set the Return-Path and Received headers for incoming messages. */
  @Test
  ...
  /**
   * Confirms that the message received date is set to the value of the <tt>X-Zimbra-Received</tt>
   * header.
   */
  @Test
  public void testZimbraReceivedHeader() throws Exception {
    ...
    String msgContent =
        new String(
            ByteUtil.getContent(
                new File(LC.zimbra_home.value() + "/unittest/testZimbraReceivedHeader.msg")));
  ...
  public void testAuthenticatedUserHeader() throws Exception {
    ...
    TestUtil.setConfigAttr(
        Provisioning.A_zimbraSmtpSendAddAuthenticatedUser, LdapConstants.LDAP_FALSE);
    ...
    TestUtil.setConfigAttr(
        Provisioning.A_zimbraSmtpSendAddAuthenticatedUser, LdapConstants.LDAP_TRUE);
    subject = NAME_PREFIX + " testAuthenticatedUserHeader true";
    TestUtil.sendMessage(mbox, USER_NAME, subject);
    msg = TestUtil.waitForMessage(mbox, "in:inbox subject:\"" + subject + "\"");
    Assert.assertEquals(
        mbox.getName(), TestUtil.getHeaderValue(mbox, msg, MailSender.X_AUTHENTICATED_USER));
  }
  ...
  public void testMalformedContentType() throws Exception {
    ...
    MessageBuilder builder =
        new MessageBuilder()
            ...
            .withSubject(subject)
            .withAttachment("This is an attachment", "test.txt", MimeConstants.CT_TEXT_PLAIN);
    ...
    outgoing.setMessagePartsToAttach(
        Arrays.asList(new AttachedMessagePart(srcMsg.getId(), srcAttachPart.getPartName(), null)));
  ...
  public void testInlineAttachment() throws Exception {
    ...
    String attachId =
        mbox.uploadAttachment(imgName, new byte[1024], MimeConstants.CT_IMAGE_JPEG, 5000);
    ...
    addresses.add(
        new ZEmailAddress(
            TestUtil.addDomainIfNecessary(USER_NAME), null, null, ZEmailAddress.EMAIL_TYPE_TO));
    ...
    Assert.assertNotNull(
        "this message should have two subparts", incoming.getMimeStructure().getChildren());
    ...
    Assert.assertTrue(
        "one of the parts should have a content id",
        parts.get(0).getContentId() != null || parts.get(1).getContentId() != null);
    Assert.assertTrue(
        "one of the parts should be " + imgName,
        imgName.equalsIgnoreCase(parts.get(0).getFileName())
  ...
  public void testSendDraftWithData() throws Exception {
    ...
    Assert.assertEquals(
        "listener should ahve been triggered only once", 1, listener.getTriggerCounter());
    ...
    Assert.assertTrue(
        String.format("wrong value in custom header: %s", listenersData[0]),
        customHeaderValue.equalsIgnoreCase(listenersData[0].replace("\"", "")));
  }
  ...
  public void testSendDraftWithCustomDataNoSave() throws Exception {
    ...
    TestUtil.createAccount(REMOTE_USER_NAME);
    MailSender.registerPreSendMailListener(listener);
    MailSender.registerPreSendMailListener(
        listener); // register second time to test that it will be triggered once
    ZMailbox zmbox = TestUtil.getZMailbox(USER_NAME);
    String subj = "Dwalin";
    ...
    request
        .addUniqueElement(MailConstants.E_MSG)
        .addAttribute(MailConstants.A_DRAFT_ID, Integer.parseInt(draft.getId()))
        .addAttribute(MailConstants.A_SEND_FROM_DRAFT, true)
  ...
  public void testMultipleHeaders() throws Exception {
    ...
    Assert.assertTrue(
        customHeaderValue1 + " is missing",
        valueList.contains(String.format("\"%s\"", customHeaderValue1)));
    Assert.assertTrue(
        customHeaderValue2 + " is missing",
        valueList.contains(String.format("\"%s\"", customHeaderValue2)));
    Assert.assertTrue(
        customHeaderValue3 + " is missing",
        valueList.contains(String.format("\"%s\"", customHeaderValue3)));
  }
