...
public abstract class SharedImapTests extends ImapTestBase {
  ...
  public void imapPath() throws IOException, ServiceException, MessagingException {
    ...
    assertNotNull(
        "Account object for user", userAcct); // Shuts up PMD complaint about missing asserts
    ...
    checkImapPath(
        ...
        "INBOX",
        false /* usingReferent */,
        Integer.parseInt(ZFolder.ID_INBOX),
        shareeAcct.getId());
    checkImapPath(
        mp,
        creds,
        mp,
        true /* usingReferent */,
        zfolder.getFolderItemIdentifier().id,
        userAcct.getId());
    checkImapPath(
        subMp,
        creds,
        subMp,
        true /* usingReferent */,
        underZfolder.getFolderItemIdentifier().id,
        userAcct.getId());
    checkImapPath(
        otherUserFolder,
        creds,
  ...
  private void checkImapPath(
      String mboxName,
      ImapCredentials creds,
      String expectedPathToString,
      boolean usingReferent,
      int expectedReferentFolderId,
      String expectedReferentFolderAcct)
      throws ServiceException {
    ImapPath path = new ImapPath(mboxName, creds);
    path.canonicalize();
    ImapPath referent = path.getReferent();
    assertEquals(
        String.format("toString() for ImapPath for mailbox '%s'", mboxName),
        expectedPathToString,
        path.toString());
    if (usingReferent) {
      assertNotSame(
    ...
    assertEquals(
        String.format("Folder ID for path.getReferent().getFolder() for mailbox '%s'", mboxName),
        expectedReferentFolderId,
        folderIdForFolder(folderForPath));
    assertEquals(
        String.format("Account ID for path.getReferent().getFolder() for mailbox '%s'", mboxName),
  ...
  private int folderIdForFolder(FolderStore folder) {
    ZimbraLog.test.debug(
        "Folder ID = %s for folder %s", folder.getFolderIdInOwnerMailbox(), folder);
  ...
  public void testListFolderContents() throws IOException, ServiceException, MessagingException {
    ...
    Map<Long, MessageData> mdMap =
        connection.fetch("1:*", "(ENVELOPE INTERNALDATE BODY BODY.PEEK[])");
  ...
  @Test
  @Timeout(100000)
  public void testListFolderContentsEnvelope()
  ...
  private void doIdleNotificationCheck(
      ImapConnection conn1, ImapConnection conn2, String folderName) throws IOException {
    ...
    assertEquals(
        "Connection was not notified of correct number of existing items", 1, mboxInfo.getExists());
    assertEquals(
        "Connection was not notified of correct number of recent items", 1, mboxInfo.getRecent());
  }
  ...
  public void testBadOrSearch() throws Exception {
    ...
    try {
      ...
      assertTrue(
          String.format("Exception '%s' should contain string '%s'", cfe.getMessage(), es),
  ...
  /**
   * Noted that when running from RunUnitTests where InterruptableRegex did NOT use an
   * InterruptibleCharSequence this would leave a dangling thread consuming resources long after
   * RunUnitTests had completed.
   */
  ...
  public void testList93114DOSRegex() throws ServiceException, InterruptedException {
    ...
    checkRegex(
        ...
        false,
        5000000,
        true /* expecting regex to take too long */);
  }
  ...
  @Test
  @Timeout(100000)
  public void testList93114MatchingEmailedContactsRegex()
  ...
  public void testFoldersList() throws Exception {
    ...
    assertNotNull("list result should not be null", listResult);
    assertTrue(
        "List result should have at least 8 entries. Got " + listResult.size(),
        listResult.size() >= 8);
  ...
  public void testListContacts() throws Exception {
    ...
    assertTrue(
        "List result should have at least 2 entries. Got " + listResult.size(),
        listResult.size() >= 2);
    for (ListData le : listResult) {
      assertTrue(
          String.format("mailbox '%s' contains 'Contacts'", le.getMailbox()),
  ...
  public void testAppendAndCount() throws Exception {
    ...
    try {
      ...
      mi =
          new StatusExecutor(connection)
              .execShouldSucceed(
                  "Sent", "MESSAGES", "UIDNEXT", "UIDVALIDITY", "UNSEEN", "HIGHESTMODSEQ");
  ...
  public void testStoreTags() throws Exception {
    ...
    mbox.addMessage(
        folder.getId(), "u", "", System.currentTimeMillis(), simpleMessage("bar"), true);
    info = connection.select(folderName);
    assertFalse(
        folderName + " contains unexpected flag " + tagName, info.getFlags().isSet(tagName));

    data = connection.fetch("*", "FLAGS");
    assertEquals(1, data.size());
    seq = data.keySet().iterator().next();
    assertFalse(
        "flag unexpectedly set on message in " + folderName,
        data.get(seq).getFlags().isSet(tagName));
    ...
    seq = data.keySet().iterator().next();
    assertTrue(
        "flag not set after STORE on message in " + folderName,
        data.get(seq).getFlags().isSet(tagName));
    ...
    assertTrue(
        "flag not set after STORE on message in " + folderName,
        data.get(seq).getFlags().isSet(tagName2));
    ...
    seq = data.keySet().iterator().next();
    assertFalse(
        "flag unexpectedly set after STORE on message in " + folderName,
        data.get(seq).getFlags().isSet(tagName));
    assertFalse(
        "flag unexpectedly set after STORE on message in " + folderName,
        data.get(seq).getFlags().isSet(tagName2));
    assertTrue(
        "flag not set after STORE on message in " + folderName,
        data.get(seq).getFlags().isSet(tagName3));
  ...
  public void testTagRemovalNotificationsToMultipleConnections() throws Exception {
    ...
    assertEquals(
        "Flags should be equal on the two connections", connectionOneFlags, connectionTwoFlags);
  }
  ...
  public void testListSubfolders() throws Exception {
    ...
    assertEquals(
        String.format("Expecting to find %s folder in list response", testSubFolderPath),
        testSubFolderPath,
        foundFolder);
  }
  ...
  public void testFolderDeletedByOtherConnectionSelected() throws Exception {
    ...
    // Wait for BYE from IMAP server. Zimbra IMAP client does not detect when connection is dropped
    // by server
    otherConnection.idle(
        new ResponseHandler() {
          @Override
          public void handleResponse(ImapResponse res) {
            if (res.isBYE()) {
  ...
  public void testFolderDeletedByOtherConnectionNotSelected() throws Exception {
    ...
    assertEquals(
        String.format(
            "UIDVALIDITY for the old folder should be the same for both connections. C1: %d C2: %d",
            oldUIDValidityC1, oldUIDValidityC2),
        oldUIDValidityC2,
        oldUIDValidityC1);
    ...
    assertEquals(
        String.format(
            "UIDVALIDITY for the new folder should be the same for both connections. C1: %d C2: %d",
            newUIDValidityC1, newUIDValidityC2),
        newUIDValidityC1,
        newUIDValidityC2);
    assertTrue("Second connection should be in SELECTED state", otherConnection.isSelected());
  ...
  public void testNonExistentFolder() throws IOException, ServiceException, MessagingException {
    ...
    try {
      ...
      assertTrue(
          String.format(
              "CommandFailedException error should contain '%s', was '%s'", err, expected),
  ...
  @Test
  @Timeout(100000)
  public void testRenameNonExistentFolder()
      throws IOException, ServiceException, MessagingException {
    ...
    try {
      ...
      fail(
          String.format(
              "'RENAME %s %s succeeded when it shouldn't have'",
              nonExistentFolderName, newFolderName));
    } catch (CommandFailedException cfe) {
      String err = cfe.getError();
  ...
  public void testSessionFlags() throws Exception {
    ...
    try {
      ...
      assertFalse("Should not be getting 'JUNK' session flag", data.getFlags().isSet(junkFlag));
      assertFalse(
          "Should not be getting 'JUNKRECORDED' session flag",
          data.getFlags().isSet(junkRecordedFlag));
    } finally {
      msg.dispose();
    }
  }
  ...
  public void testAppendTags() throws Exception {
    ...
    // now create a visible tag, add it to a message in inbox then try append to message in
    // different folder
  ...
  public void testSubscribe() throws IOException, ServiceException {
    ...
    assertTrue(
        "Should be subscribed to " + folderName + ". Instead got " + listResult.get(0).getMailbox(),
        folderName.equalsIgnoreCase(listResult.get(0).getMailbox()));
  }
  ...
  public void testUidCopy() throws IOException, ServiceException {
    ...
    connection = connectAndSelectInbox();
    AppendMessage msg1 =
        new AppendMessage(null, null, literal(testInfo.getMethodName() + " msg 1"));
    AppendMessage msg2 =
        new AppendMessage(null, null, literal(testInfo.getMethodName() + " msg 2"));
  ...
  public void testSubscribeNested() throws IOException, ServiceException {
    ...
    doLSubShouldSucceed(
        connection, "", "*", Lists.newArrayListWithExpectedSize(0), "before subscribe");
  ...
  public void testUnSubscribe() throws IOException, ServiceException {
    ...
    doLSubShouldSucceed(
        connection, "", "*", Lists.newArrayListWithExpectedSize(0), "after unsubscribe");
  }
  ...
  public void homeNameSpaceSubscribe() throws ServiceException, IOException, MessagingException {
    ...
    otherConnection = connectAndLogin(SHAREE);
    doLSubShouldSucceed(
        otherConnection, "", "*", Lists.newArrayListWithExpectedSize(0), "before 1st subscribe");
    ...
    doLSubShouldSucceed(
        ...
        "",
        homePatt,
        Lists.newArrayList(remFolder, underRemFolder),
    ...
    doLSubShouldSucceed(
        ...
        "",
        homePatt,
        Lists.newArrayList(underRemFolder),
  ...
  public void mountpointSubscribe() throws ServiceException, IOException {
    ...
    doLSubShouldSucceed(
        ...
        "",
        "*",
        Lists.newArrayList("INBOX", mp, subMp),
    ...
    doLSubShouldSucceed(
        ...
        "",
        "*",
        Lists.newArrayList("INBOX", subMp),
  ...
  public void testCopy() throws IOException {
    ...
    Map<Long, MessageData> mdMap =
        connection.fetch("1:3", "(ENVELOPE INTERNALDATE BODY BODY.PEEK[])");
  ...
  public void testXlistThrottle() throws IOException {
    ...
    for (int i = 0; i < LOOP_LIMIT; i++) {
      connection
          .newRequest(CAtom.XLIST, new MailboxName(""), new MailboxName("*"))
          .sendCheckStatus();
    }
  ...
  private String url(String mbox, AppendResult res) {
    ...
    return String.format(
        "/%s;UIDVALIDITY=%d/;UID=%d", mboxname, res.getUidValidity(), res.getUid());
  }
  ...
  public void listSharedFolderViaHome() throws ServiceException, IOException {
    ...
    doListShouldFail(
        otherConnection, "/home", "*", "LIST failed: wildcards not permitted in username");
    doListShouldFail(
        otherConnection, "", "/home/*", "LIST failed: wildcards not permitted in username");
    doListShouldFail(
        otherConnection, "", "/home/fred*", "LIST failed: wildcards not permitted in username");
    doListShouldFail(
        otherConnection, "", "/home/*fred", "LIST failed: wildcards not permitted in username");
    ...
    doListShouldFail(
        otherConnection, "", "/home/pete*fred", "LIST failed: wildcards not permitted in username");
    doListShouldFail(
        otherConnection, "", "/home/*/", "LIST failed: wildcards not permitted in username");
    doListShouldFail(
        otherConnection, "", "/home/pete*/", "LIST failed: wildcards not permitted in username");
    doListShouldFail(
        otherConnection,
        "",
        "/home/pete*fred/",
        "LIST failed: wildcards not permitted in username");
    // reset zimbraImapMaxConsecutiveError counter
    doListShouldSucceed(otherConnection, "", "INBOX", Lists.newArrayList("INBOX"), "JUST INBOX #2");
    doListShouldFail(
        otherConnection, "", "/home/*/INBOX", "LIST failed: wildcards not permitted in username");
    doListShouldFail(
        otherConnection,
        "",
        "/home/pete*/INBOX",
    ...
    doListShouldFail(
        otherConnection,
        "",
        "/home/pete*fred/INBOX",
        "LIST failed: wildcards not permitted in username");

    //  LIST "" "/home/user/sharedFolderName"
    doListShouldSucceed(
        otherConnection, "", remFolder, Lists.newArrayList(remFolder), "JUST remFolder");
    ...
    doListShouldSucceed(
        ...
        "/home",
        USER,
        Lists.newArrayList(String.format("/home/%s", USER)),
        "JUST /home/user");
    ...
    doListShouldSucceed(
        ...
        "*",
        Lists.newArrayList(remFolder, underRemFolder),
        "all folders for user  - 2");

    // 'LIST "/home" "user/INBOX"'
    doListShouldSucceed(
        otherConnection, "/home", USER + "/INBOX", Lists.newArrayList(), "unshared INBOX for user");
    ...
    doListShouldSucceed(
        ...
        sharedFolderName,
        Lists.newArrayList(remFolder),
        "shared folder for user");

    //  LIST "/home/user" "sharedFolderName/subFolder"
    doListShouldSucceed(
        otherConnection,
        homeFilter,
        underSharedFolderName,
        Lists.newArrayList(underRemFolder),
        "shared folder for user");
  ...
  public void listMountpoint() throws ServiceException, IOException {
    ...
    //  LIST "" "mountpointName"
    doListShouldSucceed(
        otherConnection, "", mountpointName, Lists.newArrayList(mountpointName), "mountpoint");
    ...
    assertTrue(
        String.format("'%s' mountpoint not in result of 'list \"\" \"*\"'", mountpointName),
        seenIt);
    otherConnection.logout();
    otherConnection = null;
  }
  ...
  /**
   * Simulate how LIST handles the situation where a user has shared their whole mailbox with a
   * sharee and the sharee has used ZWC to add a shared folder for "All applications".
   * "CreateMountpointRequest": [{ "link": { "l": 1, "name": "owner's", "view": "unknown", "zid":
   * "028fdbd8-d5de-483a-a545-11e421b8bd12", "rid": 1 }, "_jsns": "urn:zimbraMail" }], "_jsns":
   * "urn:zimbra"
   */
  @Test
  @Timeout(100000)
  public void listMountpointForAllApplicationsShare() throws ServiceException, IOException {
    ...
    assertNotNull(
        "list result 'list \"\" \"*\"' should not be null (before share)", beforeListResult);
    mbox.modifyFolderGrant(folderUserRoot, GranteeType.all, null, "rwidx", null);
    shareeZmbox.createMountpoint(
        folderUserRoot,
        mountpointName,
        null,
        null,
    ...
    assertTrue(
        String.format(
            "'list \"\" \"*\"' response before share had %s entries, after %s entries (diff %s)"
                + " expected >= 10 new",
            beforeListResult.size(), listResult.size(), extras),
        extras >= 10);
    otherConnection.logout();
    otherConnection = null;
  }
  ...
  public void mountpointWithSubFolder() throws ServiceException, IOException, MessagingException {
    ...
    doListShouldSucceed(
        ...
        "",
        mountpointName + "*",
        Lists.newArrayList(mountpointName, subMountpoint),
        "wildcard under MP");

    /* exact match shouldn't pick up sub-folder */
    doListShouldSucceed(
        otherConnection, "", mountpointName, Lists.newArrayList(mountpointName), "JUST MP");
    ...
    doListShouldSucceed(
        ...
        "",
        subMountpoint,
        Lists.newArrayList(subMountpoint),
        "JUST subfolder of MP");
    ...
    doListShouldSucceed(
        otherConnection, "", "*", expectedMboxNames, "List ALL including mountpoints");
    ...
    doFetchShouldSucceed(
        otherConnection, "1:*", "(FLAGS ENVELOPE)", subFolderEnv.subFolderSubjects);
    // recent should have been set to 0 when closing the folder to select another one
    new StatusExecutor(otherConnection)
        .setExists(2)
        .setRecent(0)
        .execShouldSucceed(
            mountpointName,
  ...
  @Test
  @Timeout(100000)
  public void homeNameSpaceWithSubFolder()
      throws ServiceException, IOException, MessagingException {
    String sharedFolderName = String.format("INBOX/%s-shared", testId);
    String subFolder = sharedFolderName + "/subFolder";
    Account shareeAcct = TestUtil.createAccount(SHAREE);
    ...
    otherConnection = connectAndLogin(SHAREE);
    doListShouldSucceed(
        otherConnection, "", remFolder, Lists.newArrayList(remFolder), "shared folder");
    doListShouldSucceed(
        otherConnection,
        "",
        underRemFolder,
    ...
    new StatusExecutor(otherConnection)
        ...
        .setRecent(0)
        .execShouldSucceed(
            remFolder, "MESSAGES", "UIDNEXT", "UIDVALIDITY", "UNSEEN", "RECENT", "HIGHESTMODSEQ");
    // recent should not have changed whilst this folder is still selected
    new StatusExecutor(otherConnection)
        .setExists(3)
        .setRecent(3)
   ...
   * do not appear in the main list of folders. They should however, be available under the /home
   * hierarchy.
   */
  @SuppressWarnings("PMD.JUnitTestsShouldIncludeAssert") // checking done in called methods
  @Test
  @Timeout(100000)
  public void copyFromMountpointUsingHomeNaming()
      throws IOException, ServiceException, MessagingException {
    TestUtil.createAccount(SHAREE);
    ZMailbox shareeZmbox = TestUtil.getZMailbox(SHAREE);
    ZMailbox mbox = TestUtil.getZMailbox(USER);
  ...
  @Test
  @Timeout(100000)
  public void copyFromMountpointUsingMountpointNaming()
      throws IOException, ServiceException, MessagingException {
    TestUtil.createAccount(SHAREE);
    ZMailbox shareeZmbox = TestUtil.getZMailbox(SHAREE);
    ZMailbox mbox = TestUtil.getZMailbox(USER);
  ...
  @Test
  @Timeout(100000)
  public void copyFromSharedFolderViaHome()
      throws IOException, ServiceException, MessagingException {
    TestUtil.createAccount(SHAREE);
    ZMailbox shareeZmbox = TestUtil.getZMailbox(SHAREE);
    connection = connectAndSelectInbox();
  ...
  private void doCopy(
      ImapConnection imapConn,
      ZMailbox shareeZmbox,
      String fromFolderName,
      String toFolderName,
      String srcMsgSubject)
      throws IOException, ServiceException, MessagingException {
    imapConn.list("", "*");
    ...
    assertEquals(
        String.format("Number of exists for folder=%s after copy", toFolderName),
        1,
        selectMboxInfo.getExists());
    Map<Long, MessageData> mdMap =
        this.doFetchShouldSucceed(imapConn, "1:*", "(ENVELOPE)", Lists.newArrayList(srcMsgSubject));
    MessageData md = mdMap.values().iterator().next();
    assertNull("Internal date was NOT requested and should be NULL", md.getInternalDate());
    BodyStructure bs = md.getBodyStructure();
    assertNull("Body Structure was not requested and should be NULL", bs);
  ...
  public void savedSearch() throws ServiceException, IOException, MessagingException {
    ...
    ZSearchFolder srchFolder =
        mbox.createSearchFolder(
            ZFolder.ID_USER_ROOT,
            folderName,
            "in:drafts or in:inbox",
            ZSearchParams.TYPE_CONVERSATION,
            SearchSortBy.nameAsc,
            ZFolder.Color.ORANGE);
    assertNotNull(
        "SearchFolder in Response to CreateSearchFolderRequest should not be null", srchFolder);
    connection = this.connectAndLogin(USER);
    List<ListData> listResult;
    //  LIST "" "mountpointName"
    doListShouldSucceed(
        connection, "", folderName, Lists.newArrayList(folderName), "Just search folder");
    listResult = connection.list("", "*");
    assertNotNull("list result 'list \"\" \"*\"' should not be null", listResult);
    boolean seenIt = false;
    for (ListData listEnt : listResult) {
    ...
    assertTrue(
        String.format("'%s' mailbox not in result of 'list \"\" \"*\"'", folderName), seenIt);
    connection.select(folderName);
    Map<Long, MessageData> mdMap = connection.fetch("1:*", "(ENVELOPE)");
    ...
    while (iter.hasNext()) {
      ...
      assertTrue(
          String.format(
              "Message subject was '%s' expected to contain '%s'", env.getSubject(), subjectPrefix),
          env.getSubject().contains(subjectPrefix));
    }
    connection.logout();
    connection = null;
  }

  private void createFolderAndShareWithSharee(String user, String sharedFolderName)
      throws IOException {
  ...
  public void clashingHomeSubFolders() throws ServiceException, IOException, MessagingException {
    ...
    doListShouldSucceed(
        otherConnection, "", remFolder1, Lists.newArrayList(remFolder1), "shared 1");
    doListShouldSucceed(
        otherConnection, "", remFolder2, Lists.newArrayList(remFolder2), "shared 2");
    otherConnection = connectAndLogin(USER);
  ...
  public void copyToMountpoint() throws Exception {
    ...
    assertEquals(
        String.format("Number of exists for folder=%s after copy", mountpoint),
        1,
        selectMboxInfo.getExists());
    Map<Long, MessageData> mdMap =
        this.doFetchShouldSucceed(connection, "1:*", "(ENVELOPE)", Lists.newArrayList(subject));
    MessageData md = mdMap.values().iterator().next();
    assertNull("Internal date was NOT requested and should be NULL", md.getInternalDate());
    BodyStructure bs = md.getBodyStructure();
    assertNull("Body Structure was not requested and should be NULL", bs);
  ...
  public void recentWithSelectAndExamine() throws Exception {
    ...
    assertEquals(
        "number of 'SEARCH NOT NOT RECENT' hits after first SELECT", 7, searchResult.size());
    ...
    selectInfo = doSelectShouldSucceed(otherConnection, folderName);
    assertEquals(
        "SELECT when no other session has selected + append since last select - RECENT count",
        1,
        selectInfo.getRecent());
    selectInfo = doSelectShouldSucceed(otherConnection, "INBOX");
    selectInfo = doSelectShouldSucceed(otherConnection, folderName);
    assertEquals("SELECT when switched folder and back - RECENT count", 0, selectInfo.getRecent());
    selectInfo = doSelectShouldSucceed(otherConnection, "INBOX");
    doAppend(
        connection,
        folderName,
    ...
    assertEquals(
        "EXAMINE folder not selected, recent append elsewhere - RECENT count",
        1,
        examineInfo.getRecent());
    selectInfo = doSelectShouldSucceed(otherConnection, folderName);
    assertEquals(
        "SELECT after EXAMINE, should remain same - RECENT count", 1, selectInfo.getRecent());
    otherConnection.logout();
    otherConnection = null;
  }
  ...
  private void doRangeSearch(String user, String folderName, int numMessages) throws Exception {
    ...
    doListShouldSucceed(
        connection, "", folderName, Lists.newArrayList(folderName), "List just single folder");
    ...
    SearchInfo ten18twenty23uidRange =
        new SearchInfo(
            folderName, String.format("UID %s,%s", ten18.firstToLast, twenty23.firstToLast), 9 + 4);
     ...
     */
    String assertTemplate =
        "Wrong number of results for 'UID SEARCH %s'. Results are: "
            + String.format(
                "%s, %s, %s, %s, %s, %s, %s, %s, %s",
                ten18,
                twenty,
  ...
  public void uidSearchRangeUndeleted() throws Exception {
    ...
    userMbox.createSearchFolder(
        null,
        Mailbox.ID_FOLDER_USER_ROOT,
        virtualFolderInInboxIsUnread,
        "IN:INBOX IS:UNREAD",
        "message",
        "none",
        0,
  ...
  /**
   * The idea of this test was to have a search folder based on items in 2 mailboxes. As we use the
   * item ID as the UID most of the time, then there is a danger of UID collisions.
   */
  @Test
  @Timeout(160000)
  @Disabled(
      "ZCS-4110 Need to support underlying folders from different mailboxes which have same [U]ID")
  public void cursorOnComplexVirtualFolder() throws Exception {
    ...
    shareeMbox.createSearchFolder(
        null,
        Mailbox.ID_FOLDER_USER_ROOT,
        folderName,
        String.format("(IN:INBOX OR IN:%s) IS:UNREAD", mountpoint),
        "message",
        "none",
        0,
        (byte) 9);
  ...
  private class ImapClientThread implements Runnable {
  ...
  private class SearchInfo {
    ...
    private void assertPassed(String template) {
      assertEquals(
          "Folder=" + folder + ":" + String.format(template, search + " UNDELETED"),
          expected,
          numHits);
    }
