...
public class TestServiceServlet {
  ...
  public static void before() throws Exception {
    ...
    delegatedAdminWithoutRights =
        TestUtil.createAccount(
            TestUtil.addDomainIfNecessary(DELEGATED_ADMIN_WITHOUT_RIGHTS), attrs);
    ...
    attrs.put(Provisioning.A_zimbraAdminConsoleUIComponents, "downloadsView");
    attrs.put(Provisioning.A_zimbraAdminConsoleUIComponents, "DLListView");
    delegatedAdminWithRights =
        TestUtil.createAccount(TestUtil.addDomainIfNecessary(DELEGATED_ADMIN_WITH_RIGHTS), attrs);

    SoapProvisioning adminSoapProv = TestUtil.newSoapProvisioning();
    TestUtil.grantRightToAdmin(
        adminSoapProv,
        com.zimbra.soap.type.TargetType.fromString(
            com.zimbra.cs.account.accesscontrol.TargetType.server.toString()),
        localServer.getName(),
        delegatedAdminWithRights.getName(),
        Admin.R_deployZimlet.getName());
    TestUtil.grantRightToAdmin(
        adminSoapProv,
        com.zimbra.soap.type.TargetType.fromString(
            com.zimbra.cs.account.accesscontrol.TargetType.server.toString()),
    ...
    if (HttpStatus.SC_OK != code) {
      Assert.fail(
          String.format("Failed to flush all cache in /zimbra web app. Response code: %d", code));
    }
  }
  ...
  private String getAdminAuthToken(String adminName)
      throws ServiceException, IOException, HttpException {
    SoapHttpTransport transport = new SoapHttpTransport(TestUtil.getAdminSoapUrl());
    com.zimbra.soap.admin.message.AuthRequest authReq =
        new com.zimbra.soap.admin.message.AuthRequest(adminName, TestUtil.DEFAULT_PASSWORD);
    authReq.setCsrfSupported(false);
    Element response =
        transport.invoke(JaxbUtil.jaxbToElement(authReq, SoapProtocol.SoapJS.getFactory()));
    com.zimbra.soap.admin.message.AuthResponse authResp = JaxbUtil.elementToJaxb(response);
    return authResp.getAuthToken();
  }

  private void verifyAdminGET(String url)
      throws ServiceException, AuthTokenException, HttpException, IOException {
    HttpClient client = HttpClientBuilder.create().build();
    ...
    Assert.assertEquals(
        "Should be getting error code without an auth token", HttpStatus.SC_UNAUTHORIZED, respCode);
    ...
    Assert.assertEquals(
        "Should be getting error code with user's auth token",
    ...
    respCode = httpResp.getStatusLine().getStatusCode();
    Assert.assertEquals(
        "Should be getting code 200 with super admin's auth token", HttpStatus.SC_OK, respCode);
    ...
    respCode = httpResp.getStatusLine().getStatusCode();
    Assert.assertEquals(
        "Should be getting code 200 with permitted delegated admin's auth token",
        HttpStatus.SC_OK,
        respCode);
    ...
    respCode = httpResp.getStatusLine().getStatusCode();
    Assert.assertEquals(
        "Should be getting code 401 with unpermitted delegated admin's auth token",
        HttpStatus.SC_UNAUTHORIZED,
        respCode);
  }

  private void verifyNonAdminGET(String url)
      throws ServiceException, AuthTokenException, HttpException, IOException {
    HttpClient client = HttpClientBuilder.create().build();
    ZMailbox mbox = TestUtil.getZMailbox(USER_NAME);
    HttpGet method = new HttpGet(url);
    ...
    respCode = httpResp.getStatusLine().getStatusCode();
    Assert.assertEquals(
        "Should be getting code 200 with user's auth token", HttpStatus.SC_OK, respCode);
    ...
    respCode = httpResp.getStatusLine().getStatusCode();
    Assert.assertEquals(
        "Should be getting code 200 with unpermitted delegated admin's auth token",
        HttpStatus.SC_OK,
        respCode);
  }

  private void deployAdminVersionCheck()
      throws AuthTokenException, ServiceException, HttpException, IOException {
    HttpClient client = HttpClientBuilder.create().build();
    ...
    File zimletFile =
        new File(
            "/opt/zextras/zimlets/com_zimbra_adminversioncheck.zip"); // standard admin extension,
    // should always be there. No
    // harm in redeploying it
    ...
    int respCode = httpResp.getStatusLine().getStatusCode();
    Assert.assertEquals(
        "Super admin should be able to deploy com_zimbra_adminversioncheck admin extension",
        HttpStatus.SC_OK,
        respCode);
  }
  ...
  public void testDeployBadZimletName() throws Exception {
    ...
    respCode = httpResp.getStatusLine().getStatusCode();
    Assert.assertEquals(
        "Should be getting code 400 with super admin's auth token w/o upload",
  ...
  public void testUnDeployBadZimletname() throws Exception {
    ...
    respCode = httpResp.getStatusLine().getStatusCode();
    Assert.assertEquals(
        "Should be getting code 400 with super admin's auth token",
        HttpStatus.SC_BAD_REQUEST,
        respCode);
  }
  ...
  public void testDeployAdminExtension() throws Exception {
    ...
    File zimletFile =
        new File(
            "/opt/zextras/zimlets/com_zimbra_adminversioncheck.zip"); // standard zimlet, should
    // always be there. No harm in
    // redeploying it
    if (zimletFile.exists()) {
      InputStream targetStream = new FileInputStream(zimletFile);
      method.setEntity(new InputStreamEntity(targetStream));
    ...
    respCode = httpResp.getStatusLine().getStatusCode();
    Assert.assertEquals(
        "Should be getting code 401 with permitted delegated admin's auth token",
        HttpStatus.SC_UNAUTHORIZED,
        respCode);
  ...
  public void testUnDeployAdminExtension() throws Exception {
    ...
    respCode = httpResp.getStatusLine().getStatusCode();
    Assert.assertEquals(
        "Delegated admin should not be allowed to undeploy admin extensions even with deployZimlet"
            + " right",
        HttpStatus.SC_UNAUTHORIZED,
        respCode);
    ...
    respCode = httpResp.getStatusLine().getStatusCode();
    Assert.assertEquals(
        "Delegated admin should not be allowed to undeploy admin extensions with or without"
            + " deployZimlet right",
        HttpStatus.SC_UNAUTHORIZED,
        respCode);
    ...
    respCode = httpResp.getStatusLine().getStatusCode();
    Assert.assertEquals(
        "Super admin should be able to undeploy com_zimbra_adminversioncheck admin extension",
        HttpStatus.SC_OK,
        respCode);
  }
}
