...
public class TestUtil extends Assert {
  ...
  public static Message addMessage(
      Mailbox mbox, String recipient, String sender, String subject, String body, long timestamp)
      throws Exception {
  ...
  public static String getTestMessage(
      String subject, String recipient, String sender, String body, Date date)
      throws ServiceException, MessagingException, IOException {
    return new MessageBuilder()
        .withSubject(subject)
        .withToRecipient(recipient)
  ...
  public static Contact createContact(Mailbox mbox, int folderId, String emailAddr)
      throws ServiceException {
    return mbox.createContact(
        null,
        new ParsedContact(Collections.singletonMap(ContactConstants.A_email, emailAddr)),
        folderId,
        null);
  }

  public static Contact createContactInDefaultFolder(Mailbox mbox, String emailAddr)
      throws ServiceException {
  ...
  public static boolean addMessageLmtp(
      String subject, String[] recipients, String sender, String body) throws Exception {
  ...
  public static boolean addMessageLmtp(String[] recipients, String sender, String message)
    ...
    LmtpClient lmtp =
        new LmtpClient(
            "localhost", prov.getLocalServer().getIntAttr(Provisioning.A_zimbraLmtpBindPort, 7025));
  ...
  public static String addRawMessage(ZMailbox mbox, String rawMessage) throws ServiceException {
    return mbox.addMessage(
        Integer.toString(Mailbox.ID_FOLDER_INBOX), null, null, 0, rawMessage, true);
  }
  ...
  public static void sendMessage(
      ZMailbox senderMbox, String recipientName, String subject, String body) throws Exception {
  ...
  public static void saveDraftAndSendMessage(
      ZMailbox senderMbox, String recipient, String subject, String body, String attachmentUploadId)
      throws ServiceException {
    ZOutgoingMessage outgoingDraft =
        getOutgoingMessage(recipient, subject, body, attachmentUploadId);
    ZMessage draft =
        senderMbox.saveDraft(outgoingDraft, null, Integer.toString(Mailbox.ID_FOLDER_DRAFTS));
  ...
  public static ZOutgoingMessage getOutgoingMessage(
      String recipient, String subject, String body, String attachmentUploadId)
      throws ServiceException {
    ZOutgoingMessage msg = new ZOutgoingMessage();
    List<ZEmailAddress> addresses = new ArrayList<ZEmailAddress>();
    addresses.add(
        new ZEmailAddress(
            addDomainIfNecessary(recipient), null, null, ZEmailAddress.EMAIL_TYPE_TO));
  ...
  public static List<Integer> search(Mailbox mbox, String query, Set<MailItem.Type> types)
    ...
    try (ZimbraQueryResults r =
        mbox.index.search(new OperationContext(mbox), query, types, SortBy.DATE_DESC, 100)) {
      while (r.hasNext()) {
  ...
  public static List<ZimbraHit> searchForHits(Mailbox mbox, String query, MailItem.Type type)
      throws Exception {
    return searchForHits(mbox, query, Collections.singleton(type));
  }

  public static List<ZimbraHit> searchForHits(Mailbox mbox, String query, Set<MailItem.Type> types)
      throws Exception {
  ...
  public static byte[] getContent(ZMailbox mbox, String msgId, String name)
    ...
    return ByteUtil.getContent(
        mbox.getRESTResource("?id=" + msgId + "&part=" + part.getPartName()), 1024);
  }
  ...
  private static boolean postfixFlushDeferredMailQueue(Server server, long waitMillis) {
    ...
    try {
      ...
      ZimbraLog.test.info(
          "postfixFlushDeferredMailQueue finished [%s]",
          ZimbraLog.elapsedTime(start, System.currentTimeMillis()));
    } catch (ServiceException e) {
      ZimbraLog.test.error(
          "postfixFlushDeferredMailQueue - Problem getting 'deferred' mail queue", e);
    }
    return true;
  }

  private static List<ZMessage> helpWait4Msgs(
      ZMailbox mbox, String query, int numMsgsExpected, long timeout_millis)
      throws ServiceException {
    long start = System.currentTimeMillis();
    ...
    while (timeout_millis > 0) {
      ...
      if ((numMsgsExpected > 0) && (msgs.size() == numMsgsExpected)) {
        ZimbraLog.test.debug(
            "helpWait4Msgs succeeded - mbox='%s' query='%s' %s",
            mbox.getName(), query, ZimbraLog.elapsedTime(start, System.currentTimeMillis()));
        return msgs;
      }
    ...
    ZimbraLog.test.debug(
        "helpWait4Msgs finished for mbox='%s' query='%s' %s",
        mbox.getName(), query, ZimbraLog.elapsedTime(start, System.currentTimeMillis()));
    return msgs;
  }
   ...
   * @throws ServiceException
   */
  public static List<ZMessage> waitForMessages(
      ZMailbox mbox, String query, int numMsgsExpected, int timeout_millis)
      throws ServiceException {
    long start = System.currentTimeMillis();
    long finishBy = start + timeout_millis;
    List<ZMessage> msgs;
    // Just wait for a third of the time initially.  If hasn't arrived in that time, poke postfix to
    // process
    ...
    if (numMsgsExpected > 0) {
       ...
       *                           user1@example.com
       */
      if (postfixFlushDeferredMailQueue(
          AccountTestUtil.getServer(mbox.getName()), finishBy - System.currentTimeMillis())) {
    ...
    if (numMsgsExpected > 0) {
      Assert.fail(
          String.format(
              "Message%s for query '%s' didn't arrive within %d millisecs.  %s",
              (numMsgsExpected == 1) ? "" : "s",
              query,
  ...
  public static QueryWaitSetResponse waitForSessions(
      int numExpectedSessions,
      int numExpectedFolderInterests,
      int timeout_millis,
      String wsID,
      Server server)
      throws Exception {
  ...
  /**
   * Delete all messages, tags and folders in the user's mailbox whose subjects contain the given
   * substring. For messages, the subject must contain subjectString as a separate word. Tags and
   * folders can have the string anywhere in the name.
   */
  ...
  public static SoapProvisioning newDelegatedSoapProvisioning(String login, String password)
      throws ServiceException {
    SoapProvisioning sp = new SoapProvisioning();
    sp.soapSetURI("https://localhost:7071" + AdminConstants.ADMIN_SERVICE_URI);
  ...
  public static ZMailbox getZMailbox(
      String username, String password, String twoFactorCode, TrustedStatus trusted)
      throws ServiceException {
  ...
  public static Account createAccount(String username) throws ServiceException {
    ...
    attrs.put(
        ...
        Provisioning.getInstance().getLocalServer().getServiceHostname());
  ...
  public static void deleteAccount(String username) throws ServiceException {
    ...
    GetAccountRequest gaReq =
        new GetAccountRequest(
            AccountSelector.fromName(username), false, Lists.newArrayList(Provisioning.A_zimbraId));
    try {
      ...
      if (resp != null) {
        ...
        if (null == id) {
          ZimbraLog.test.error(
              "GetAccountResponse for '%s' did not contain the zimbraId", username);
          return;
        }
  ...
  public static void setAccountAttr(String userName, String attrName, String attrValue)
    ...
    if (null == account) {
      throw ServiceException.FAILURE(
          String.format(
              "Trying to setAccountAttr(%s,%s,%s) on non-existent account %s",
              userName, attrName, attrValue, userName),
          null);
    }
    Map<String, Object> attrs = new HashMap<String, Object>();
    attrs.put(attrName, attrValue);
  ...
  public static void setDataSourceAttr(
      String userName, String dataSourceName, String attrName, String attrValue)
      throws ServiceException {
    Provisioning prov = Provisioning.getInstance();
    Account account = getAccount(userName);
  ...
  public static ZFolder createFolder(ZMailbox mbox, String path, ZFolder.View view)
    ...
    if (idxLastSlash < 0) {
      ...
      if (parent == null) {
        String msg =
            String.format("Creating folder %s: parent %s does not exist", name, parentPath);
  ...
  /**
   * Creates a mountpoint between two mailboxes. The mountpoint gives the "to" user full rights on
   * the folder.
   *
   ...
   * @param mountpointName the name of the mountpoint folder. The folder is created directly under
   *     the user root.
   */
  public static ZMountpoint createMountpoint(
      ZMailbox remoteMbox, String remotePath, ZMailbox localMbox, String mountpointName)
      throws ServiceException {
    ...
    return localMbox.createMountpoint(
  ...
  /**
   * Imports data from the given data source and updates state on both the local and remote
   * mailboxes.
   */
  public static void importDataSource(
      ZDataSource dataSource, ZMailbox localMbox, ZMailbox remoteMbox) throws Exception {
   ...
   */
  public static void importDataSource(
      ZDataSource dataSource, ZMailbox localMbox, ZMailbox remoteMbox, boolean expectedSuccess)
      throws Exception {
  ...
  private static SoapTransport getAdminSoapTransport(
      SoapHttpTransport transport, String adminName, String adminPassword)
    ...
    Element response =
        transport.invoke(
            auth, false /* raw */, true /* noSession value */, null /* requestedAccountId */);
  ...
  public static SoapTransport getAdminSoapTransport()
      throws SoapFaultException, IOException, ServiceException, HttpException {
    return getAdminSoapTransport(
        new SoapHttpTransport(getAdminSoapUrl()),
        LC.zimbra_ldap_user.value(),
  ...
  public static SoapTransport getAdminSoapTransport(Server targetServer)
      throws SoapFaultException, IOException, ServiceException, HttpException {
    return getAdminSoapTransport(
        new SoapHttpTransport(URLUtil.getAdminURL(targetServer)),
        LC.zimbra_ldap_user.value(),
        LC.zimbra_ldap_password.value());
  }
  ...
  public static SoapTransport getAdminSoapTransport(String adminName, String adminPassword)
      throws SoapFaultException, IOException, ServiceException, HttpException {
    return getAdminSoapTransport(
        new SoapHttpTransport(getAdminSoapUrl()), adminName, adminPassword);
  }

  /**
   * Assert the message contains the given sub-message, ignoring newlines. Used for comparing
   * equality of two messages, when one had <tt>Return-Path</tt> or other headers prepended.
   */
  ...
  private static void assertEquals(
      Element expected, Element actual, String expectedDump, String actualDump) {
    ...
    assertEquals(
        context + " children", getElementNames(expectedChildren), getElementNames(actualChildren));
    ...
    assertEquals(
        context + " attributes",
        getAttributesAsString(expectedAttrs),
        getAttributesAsString(actualAttrs));
  }
  ...
  public static ZAppointmentResult createAppointment(
      ZMailbox mailbox, String subject, String attendee, Date startDate, Date endDate)
      throws ServiceException {
  ...
  public static void sendInviteReply(
      ZMailbox mbox,
      String inviteId,
      String organizer,
      String subject,
      ZMailbox.ReplyVerb replyVerb)
      throws ServiceException {
    organizer = addDomainIfNecessary(organizer);
    ZOutgoingMessage msg =
        getOutgoingMessage(organizer, subject, "Reply to appointment " + inviteId, null);
  ...
  public static ZDocument createDocument(
      ZMailbox mbox, String folderId, String name, String contentType, byte[] content)
      throws ServiceException {
  ...
  public static ZAuthResult testAuth(ZMailbox mbox, String account, String password)
      throws ServiceException {
    return testAuth(mbox, account, password, null);
  }

  public static ZAuthResult testAuth(
      ZMailbox mbox, String account, String password, String twoFactorCode)
      throws ServiceException {
  ...
  public static void updateMailItemChangeDateAndFlag(
      Mailbox mbox, int itemId, long changeDate, int flagValue) throws ServiceException {
  ...
  public static void grantRightToAdmin(
      SoapProvisioning adminSoapProv,
    ...
    GranteeSelector grantee =
        new GranteeSelector(com.zimbra.soap.type.GranteeType.usr, GranteeBy.name, granteeName);
    ...
    GrantRightResponse grResp =
        adminSoapProv.invokeJaxb(new GrantRightRequest(target, grantee, right));
  ...
  public static void assumeTrue(String missive, Boolean testVal) {
    try {
      ...
      throw new AssumptionViolatedException(missive, (Throwable) null);
    }
  }
