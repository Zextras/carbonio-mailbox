...
public abstract class SharedImapNotificationTests extends ImapTestBase {
  ...
  protected abstract void runOp(MailboxOperation op, ZMailbox mbox, ZFolder folder)
      throws Exception;
  ...
  public void testNotificationsActiveFolder() throws Exception {
    ...
    MailboxOperation addMessage =
        new MailboxOperation() {
          @Override
          ...
          protected String checkResult() throws Exception {
            ...
            if (mdMap.size() != 2) {
              return String.format(
                  "Size of map returned by fetch should be 2. Getting %d", mdMap.size());
            }
            return null;
          }
        };
        ...
        new MailboxOperation() {
          ...
          protected String checkResult() throws Exception {
            try {
              ...
              if (mdMap.size() != 1) {
                return String.format(
                    "Size of map returned by fetch should be 1. Getting %d", mdMap.size());
              }
    ...
    MailboxOperation addMessage =
        new MailboxOperation() {
          ...
          protected String checkResult() throws Exception {
            ...
            if (mdMap.size() != 2) {
              return String.format(
                  "Size of map returned by fetch afer reselecting cached folder should be 2."
                      + " Getting %d",
                  mdMap.size());
            }
            return null;
  ...
  public void testDeleteMessageNotificationActiveFolder() throws Exception {
    ...
    MailboxOperation deleteMessage =
        new MailboxOperation() {
          @Override
          protected void run(ZMailbox zmbox) throws Exception {
          ...
          protected String checkResult() throws Exception {
            ...
            if (!subject2.equals(md.getEnvelope().getSubject())) {
              return String.format(
                  "Subject should be %s. Getting %d", subject2, md.getEnvelope().getSubject());
            }
            connection.expunge();
  ...
  public void testDeleteMessageBySubjectNotifications() throws Exception {
    ...
    MailboxOperation deleteBySubject =
        new MailboxOperation() {
          @Override
          protected void run(ZMailbox zmbox) throws Exception {
          ...
          protected String checkResult() throws Exception {
            ...
            mdMap
                ...
                .removeIf(
                    e ->
                        e.getValue().getEnvelope() == null
                            || e.getValue().getEnvelope().getSubject() == null);
            if (mdMap.size() != 1) {
              return String.format(
                  "Size of map returned by fetch should be 1. Getting %d", mdMap.size());
            }
    ...
    MailboxOperation tagMessage =
        new MailboxOperation() {
          ...
          protected String checkResult() throws Exception {
            ...
            if (mdMap.size() != 1) {
              return String.format(
                  "Size of map returned by fetch should be 1. Found: %d", mdMap.size());
            }
            Flags flags = mdMap.get(1L).getFlags();
            if (!flags.contains(new Atom(tag.getName()))) {
              return String.format("Flags should contain %s", tag.getName());
  ...
  /**
   * This class encapsulates some mailbox operation run from an IMAP server. It is used to test IMAP
   * notifications via response headers and waitsets.
   */
  protected abstract static class MailboxOperation {
