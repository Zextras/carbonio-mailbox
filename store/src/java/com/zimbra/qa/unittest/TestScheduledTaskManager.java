...
public class TestScheduledTaskManager {
  ...
  public void setUp() throws Exception {
    USER_NAME =
        String.format(
                "%s-%s-user1",
                TestScheduledTaskManager.class.getSimpleName(), testInfo.getMethodName())
            .toLowerCase();
    tearDown();
  ...
  /**
   * Confirms that a single task is persisted to the database, runs, and is then removed from the
  ...
  /**
   * Confirms that a recurring task is persisted to the database, runs multiple times, and is then
   * removed from the database when cancelled.
   */
  @Test
  ...
  private void checkNumPersistedTasks(int expected) throws Exception {
    DbResults results =
        DbUtil.executeQuery(
            "SELECT COUNT(*) FROM "
                + DbScheduledTask.TABLE_SCHEDULED_TASK
                + " WHERE class_name = '"
