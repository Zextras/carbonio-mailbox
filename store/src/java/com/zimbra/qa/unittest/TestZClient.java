...
public class TestZClient {
  ...
  public void setUp() throws Exception {
    NAME_PREFIX =
        String.format("%s-%d", testInfo.getMethodName(), (int) Math.abs(Math.random() * 100));
  ...
  /**
   * Test for fix to ZCS-4341 code similar to ZJspSession.getRestMailbox Prior to fix, fails with:
  ...
  public void simulateGetRestMailbox() throws ServiceException, AuthTokenException {
    ...
    ZMailbox.Options options =
        new ZMailbox.Options(authToken.getEncoded(), AccountUtil.getSoapUri(acct));
  ...
  /**
   * Confirms that the {@code List} of signatures returned by {@link ZMailbox#getSignatures} is
   * modifiable (see bug 51842).
   */
  @Test
  ...
  public void testCopyItemAction() throws Exception {
    ...
    String content =
        new MessageBuilder()
            .withSubject(subject)
            .withFrom(sender)
            .withToRecipient(recipient)
            .create();

    // add a msg flagged as sent; filterSent=TRUE
    mbox.addMessage(
        Integer.toString(Mailbox.ID_FOLDER_DRAFTS),
        null,
        null,
    ...
    ItemActionResponse resp =
        mbox.copyItemAction(new ItemIdentifier(null, Mailbox.ID_FOLDER_SENT), ids);
  ...
  public void testImapUIDforMsg() throws Exception {
    ...
    assertEquals(
        String.format(
            "IMAP UID '%s' should be same as ID '%s' even though didn't request IMAP UID in"
                + " original SOAP request",
            zmsg2.getImapUid(), zmsg2.getIdInMailbox()),
        zmsg2.getImapUid(),
        zmsg2.getIdInMailbox());
  }

  @Test
  public void testImapUIDForGalContact() throws Exception {
    ZMailbox mbox = TestUtil.getZMailbox(USER_NAME);
    ZSearchGalResult zsgr =
        mbox.autoCompleteGal(USER_NAME /* query */, GalEntryType.all, 20 /* limit */);
    ...
    assertTrue(
        String.format("Contacts size %s should be greater than 0", contacts.size()),
        contacts.size() > 0);
    for (ZContact contact : contacts) {
      assertEquals("IMAP UID should be zero for Gal contact", 0, contact.getImapUid());
      assertEquals(
          "ModifiedSequence should be zero for Gal contact", 0, contact.getModifiedSequence());
    }
  }
  ...
  public void testImapUIDForNonStandardContact() throws ServiceException {
    ...
    ItemIdentifier contactId =
        ItemIdentifier.fromAccountIdAndItemId(zmbox.getAccountId(), contact.getId());
    Element reqEl = zmbox.newRequestElement(MailConstants.GET_CONTACTS_REQUEST);
    // Specifically do NOT do reqEl.addAttribute(MailConstants.A_WANT_IMAP_UID, true);
    reqEl
        .addNonUniqueElement(MailConstants.E_CONTACT)
        .addAttribute(MailConstants.A_ID, contactId.id);
    Element respEl = zmbox.invoke(reqEl);
    ZContact zcontact = new ZContact(respEl.getElement(MailConstants.E_CONTACT), zmbox);
    assertNotNull("ZContact object", zcontact);
    // The IMAP UID should be gotten via a refetch of the contact - asking for the IMAP UID
    assertEquals(
        "Expected=ZContact ID Actual=ZContact IMAP UID",
        zcontact.getIdInMailbox(),
        zcontact.getImapUid());
    assertEquals(
        "IMAP UID Expected=Contact Actual=ZContact", contact.getImapUid(), zcontact.getImapUid());
    assertEquals(
        "ModifiedSequence Expected=Contact Actual=ZContact",
        contact.getModifiedSequence(),
        zcontact.getModifiedSequence());
  }
  ...
  public void testZMailboxGetContactsForFolder() throws ServiceException {
    ...
    ZMailbox zmbox = TestUtil.getZMailbox(USER_NAME);
    Contact contact = TestUtil.createContactInDefaultFolder(mbox, "testzclient2@example.com");
    List<ZContact> allZContacts =
        zmbox.getContactsForFolder(
            ZFolder.ID_CONTACTS,
            null /* ids */,
            (ContactSortBy) null,
            true /* sync */,
    ...
    assertEquals(
        "GetAllContacts:Expected=Contact IMAP UID Actual=ZContact IMAP UID",
        contact.getImapUid(),
        zcontact.getImapUid());
    assertEquals(
        "GetAllContacts:Expected=Contact ModifiedSequence Actual=ZContact ModifiedSequence",
        contact.getModifiedSequence(),
        zcontact.getModifiedSequence());
  }
  ...
  public void testCreateAndModifyZContact() throws ServiceException {
    ...
    assertNotNull(
        "Contact object gotten by ID on newly created zcontact should not be null", zcontact);
    assertEquals(
        "IMAP UID Expected=Contact Actual=ZContact", contact.getImapUid(), zcontact.getImapUid());
    assertEquals(
        "ModifiedSequence Expected=Contact Actual=ZContact",
    ...
    assertEquals(
        "After Modify:Expected=ZContact ID Actual=ZContact IMAP UID",
        zcontact.getIdInMailbox(),
        zcontact.getImapUid());
    contact = mbox.getContactById(null, zcontact.getIdInMailbox());
    assertNotNull(
        "After Modify:Contact object gotten by ID on newly modified zcontact should not be null",
        contact);
    assertEquals(
        "After Modify:IMAP UID:Expected=Contact Actual=ZContact",
        contact.getImapUid(),
        zcontact.getImapUid());
    assertEquals(
        "After Modify:ModifiedSequence Expected=Contact Actual=ZContact",
        contact.getModifiedSequence(),
        zcontact.getModifiedSequence());
    assertTrue(
        String.format(
            "After Contact modify, new modSeq=%s should be greater than old modSeq=%s",
            zcontact.getModifiedSequence(), origModSeq),
        zcontact.getModifiedSequence() > origModSeq);
    List<ZContact> allZContacts =
        zmbox.getAllContacts(
            ZFolder.ID_CONTACTS, (ContactSortBy) null, true /* sync */, null /* attrs */);
    assertNotNull("zmbox.getAllContacts result should not be null", allZContacts);
    assertEquals("zmbox.getAllContacts result should have 1 entry", 1, allZContacts.size());
    zcontact = allZContacts.get(0);
    assertEquals(
        "GetAllContacts: IMAP UID Expected=Contact Actual=ZContact",
        contact.getImapUid(),
        zcontact.getImapUid());
    assertEquals(
        "GetAllContacts ModifiedSequence:Expected=Contact Actual=ZContact",
        contact.getModifiedSequence(),
        zcontact.getModifiedSequence());
  }
  ...
  public void testResetImapUID() throws Exception {
    ...
    RemoteImapMailboxStore store =
        new RemoteImapMailboxStore(mbox, TestUtil.getAccount(USER_NAME).getId());
  ...
  public void testZMailboxGetItemById() throws Exception {
    ...
    ZimbraMailItem mItemAsMsg =
        zmbox.getItemById((OpContext) null, msgItemId, MailItemType.MESSAGE);
    assertNotNull("getItemById returned null when got with type MESSAGE", mItemAsMsg);
    assertEquals(
        "Different ID when got with type MESSAGE",
        id,
        Integer.valueOf(mItemAsMsg.getIdInMailbox()));
    assertTrue(
        String.format(
            "%s Not a ZMessage when got with type MESSAGE", mItemAsMsg.getClass().getName()),
    ...
    assertEquals(
        "Different ID when got with type UNKNOWN",
        id,
        Integer.valueOf(mItemAsMsg.getIdInMailbox()));
    assertTrue(
        String.format(
            "%s Not a ZMessage when got with type UNKNOWN", mItemAsMsg.getClass().getName()),
        mItemAsMsg instanceof ZMessage);

    /* getting contact using id of contact */
    ZimbraMailItem mItemAsContact =
        zmbox.getItemById((OpContext) null, contactId, MailItemType.CONTACT);
    ...
    assertTrue(
        String.format(
            "%s Not a ZContact when got with type CONTACT", mItemAsContact.getClass().getName()),
        mItemAsContact instanceof ZContact);
    ZContact zContact = (ZContact) mItemAsContact;
    assertEquals(
        "Imap UID of ZContact should be same as Contact",
        contact.getImapUid(),
        zContact.getImapUid());
    assertTrue(
        String.format("IMAP UID %s of ZContact not 0", zContact.getImapUid()),
        0 != zContact.getImapUid());
  ...
  public void testImapMODSEQ() throws Exception {
    ...
    Folder folder =
        mbox.createFolder(
            null, NAME_PREFIX, new Folder.FolderOptions().setDefaultView(MailItem.Type.MESSAGE));
    ...
    assertEquals(
        "Before adding a message, ZFolder modseq is not the same as folder modseq",
        zmodSeq,
        folder.getImapMODSEQ());

    // add a message to the folder (there is a test in FolderTest which verifies that adding a
    // message modifies imapmodseq)
    DeliveryOptions dopt =
        new DeliveryOptions().setFolderId(folderId).setFlags(Flag.BITMASK_UNREAD);
    String message =
        TestUtil.getTestMessage(
            NAME_PREFIX,
    ...
    assertEquals(
        "After adding a message, ZFolder modseq is not the same as folder modseq",
        zfolder.getImapMODSEQ(),
        folder.getImapMODSEQ());
    assertFalse(
        "ZFolder modseq did not change after adding a message", zmodSeq == zfolder.getImapMODSEQ());
  }
  ...
  public void testRecentMessageCount() throws Exception {
    ...
    ZMailbox zmbox = TestUtil.getZMailbox(USER_NAME);
    assertEquals(
        "Mailbox::getRecentMessageCount should return 0 before adding a message",
        0,
    ...
    mbox.addMessage(null, pm, dopt, null);
    assertEquals(
        "Mailbox::getRecentMessageCount should return 1 after adding a message",
        1,
        mbox.getRecentMessageCount());
    zmbox.resetRecentMessageCount(null);
    assertEquals(
        "Mailbox::getRecentMessageCount should return 0 after reset",
        0,
        mbox.getRecentMessageCount());
    message =
        TestUtil.getTestMessage(
    ...
    mbox.addMessage(null, pm, dopt, null);
    assertEquals(
        "Mailbox::getRecentMessageCount should return 2 after adding two messages",
        2,
        mbox.getRecentMessageCount());
    zmbox.resetRecentMessageCount(null);
    assertEquals(
        "Mailbox::getRecentMessageCount should return 0 after the second reset",
        0,
        mbox.getRecentMessageCount());
  }
  ...
  public void testOpenImapFolder() throws Exception {
    ...
    for (int i = 1; i <= 10; i++) {
      Message msg =
          TestUtil.addMessage(
              mbox, folderId.id, String.format("imap message %s", i), System.currentTimeMillis());
    ...
    Integer cursorId =
        expected
            .get(2)
            .getMsgId(); // test fetching 5 items starting at 3rd item, (more results left)
    ...
    cursorId =
        expected
            .get(6)
            .getMsgId(); // test fetching 5 items starting at 7th item, exhausting all results
    params.setCursorId(String.valueOf(cursorId));
    result = zmbox.fetchImapFolderChunk(params);
    messages = result.getImapMessageInfo();
    ...
    assertEquals(
        "expected and actual lists have different lengths", expected.size(), actual.size());
  ...
  public void testGetIdsOfModifiedItemsInFolder() throws Exception {
    ...
    Folder folder =
        mbox.createFolder(
            null,
            "testGetIdsOfModifiedItemsInFolder",
            new Folder.FolderOptions().setDefaultView(MailItem.Type.MESSAGE));
    int folderId = folder.getId();
    ...
    assertTrue(
        "getIdsOfModifiedItemsInFolder should return an empty list given the last change number",
        modifiedIds.isEmpty());
    ...
    for (int i = 1; i <= 10; i++) {
      Message msg =
          TestUtil.addMessage(
              mbox,
              folderId,
              String.format("testGetIdsOfModifiedItemsInFolder message %s", i),
              System.currentTimeMillis());
    ...
    assertEquals(
        String.format(
            "should return the same number of IDs as the number of added messages. Added %d"
                + " messages, but returned %d IDs",
            expected.size(), modifiedIds.size()),
        expected.size(),
        modifiedIds.size());
    ...
    Message msg =
        TestUtil.addMessage(
            mbox,
            folderId,
            "testGetIdsOfModifiedItemsInFolder last message",
            System.currentTimeMillis());
    modifiedIds = zmbox.getIdsOfModifiedItemsInFolder(null, lastChange, folderId);
  ...
  public void testGetMessageNotRaw() throws Exception {
    ...
    Message msg =
        TestUtil.addMessage(
            mbox,
            folderId,
            String.format("%s message %s", testInfo.getMethodName(), "hello"),
            System.currentTimeMillis());
    ZMessage zmsg =
        zmbox.getMessageById(
            ItemIdentifier.fromAccountIdAndItemId(mbox.getAccountId(), msg.getId()));
  ...
  public void testAlterTag() throws Exception {
    ...
    Message msg =
        TestUtil.addMessage(
            mbox, Mailbox.ID_FOLDER_INBOX, "testAlterTag message", System.currentTimeMillis());
    int msgId = msg.getId();
    ...
    assertTrue(
        "Message.isUnread should return TRUE", waitForFlag(msgId, mbox, "isUnread", true, 3000));
    msg = mbox.getMessageById(null, msgId);
    assertTrue("Message should be unread", msg.isUnread());
    zmbox.alterTag(null, ids, "\\Unread", false);
    assertTrue(
        "Message.isUnread should return FALSE", waitForFlag(msgId, mbox, "isUnread", false, 3000));
    msg = mbox.getMessageById(null, msgId);
  ...
  public void testSetTags() throws Exception {
    ...
    assertTrue(
        "Message should be tagged with " + tag1Name, waitForTag(msgId, mbox, tag1Name, true, 3000));
    ...
    assertTrue(
        "Message should NOT be tagged with " + tag1Name,
        waitForTag(msgId, mbox, tag1Name, false, 3000));
    assertTrue(
        "Message should be tagged with " + tag2Name, waitForTag(msgId, mbox, tag2Name, true, 3000));
    assertTrue(
        "Message should be tagged with " + tag3Name, waitForTag(msgId, mbox, tag3Name, true, 3000));
    msg = mbox.getMessageById(null, msgId);
  ...
  public void testFlags() throws Exception {
    ...
    assertTrue(
        "Message.isUnread should return TRUE",
        waitForFlag(numId, recipMbox, "isUnread", true, 3000));
    ZMessage zmsg = recipZMbox.getMessageById(id);
    assertTrue(zmsg.isUnread());
    Message msg = recipMbox.getMessageById(null, numId);
    assertEquals(
        "ZMessage bitmask does not match Message bitmask",
        msg.getFlagBitmask(),
        zmsg.getFlagBitmask());
    Collection<ItemIdentifier> ids = new ArrayList<ItemIdentifier>(1);
    ...
    assertTrue(
        "Message.isUnread should return FALSE",
        waitForFlag(numId, recipMbox, "isUnread", false, 3000));
    ...
    // HIGH_PRIORITY
    ZOutgoingMessage importantMsg =
        TestUtil.getOutgoingMessage(
            RECIPIENT_USER_NAME, "This is an important message", "about something", null);
    ...
    assertTrue(
        String.format(
            "SavedDraft modifiedSequence=%s should be > 0", savedDraft.getModifiedSequence()),
    ...
    assertEquals(
        ...
        zmsg.getFlagBitmask());
    assertEquals(
        "Expected=savedDraft ModSeq, Actual=MBOX getMessageById ModSeq",
        savedDraft.getModifiedSequence(),
        msg.getModifiedSequence());
    assertEquals(
        "Expected=savedDraft ModSeq, Actual=ZMBOX getMessageById ModSeq",
        savedDraft.getModifiedSequence(),
        zmsg.getModifiedSequence());

    // LOW_PRIORITY
    ZOutgoingMessage nonImportantMsg =
        TestUtil.getOutgoingMessage(
            RECIPIENT_USER_NAME, "This is an important message", "about something", null);
  ...
  public void testLastChangeId() throws Exception {
    ...
    assertEquals(
        String.format(
            "wrong change ID before adding message. Expecting %d. Got %d",
            mbox.getLastChangeID(), firstChangeId),
        mbox.getLastChangeID(),
        firstChangeId);
    ...
    assertEquals(
        String.format(
            "wrong change ID after adding message. Expecting %d. Got %d",
            mbox.getLastChangeID(), secondChangeId),
        mbox.getLastChangeID(),
        secondChangeId);
  }

  public boolean waitForFlag(
      int msgId, Mailbox mbox, String getterName, boolean expected, int maxtimeout)
      throws Exception {
  ...
  public boolean waitForTag(
      int msgId, Mailbox mbox, String tagName, boolean expected, int maxtimeout) throws Exception {
    int remaining = maxtimeout;
    while (remaining > 0) {
      Message msg = mbox.getMessageById(null, msgId);
  ...
  public void testZSearchParamsIncludeTagDeleted() throws Exception {
    ...
    mbox.alterTag(
        null, Integer.valueOf(msgId), MailItem.Type.MESSAGE, FlagInfo.DELETED, true, null);
  ...
  public void testZSearchParamsMailItemTypes() throws Exception {
    ...
    ZAppointmentResult appt =
        TestUtil.createAppointment(
            zmbox,
            "testZSearchParamsMailItemTypes appointment",
            USER_NAME,
            new Date(),
            calendar.getTime());
  ...
  public void testImapSearch() throws Exception {
    ...
    Message msg = mbox.getMessageById(null, msgId);
    verifyImapSearchResults(
        results,
        msgId,
        msg.getImapUid(),
        msg.getParentId(),
        msg.getModifiedSequence(),
        MailItemType.MESSAGE,
        msg.getFlagBitmask(),
  ...
  public void testImapSearchContact() throws Exception {
    ...
    Contact contact =
        TestUtil.createContact(
            mbox, folder.getFolderIdInOwnerMailbox(), "testImapSearch@test.local");
    ...
    verifyImapSearchResults(
        ...
        contact.getParentId(),
        contact.getModifiedSequence(),
        MailItemType.CONTACT,
        contact.getFlagBitmask(),
        new String[] {tag.getId()},
        true);
    params.setZimbraFetchMode(ZimbraFetchMode.MODSEQ);
    results = zmbox.searchImap(null, params);
  ...
  private void verifyImapSearchResults(
      ZimbraQueryHitResults results,
      int id,
      int imapUid,
      int parentId,
      int modSeq,
    ...
    assertEquals(
        "Expected=modSeq Actual=hit.getModifiedSequence()", modSeq, hit.getModifiedSequence());
  ...
  private void compareMsgAndZMsg(Message msg, ZMessage zmsg) throws IOException, ServiceException {
    ...
    if (null != zmsgContent) {
      if (zmsgContent.equals(zmsgContent)) {
        ZimbraLog.test.info(
            "%s Pleasant surprise.  ZMessage.getContent() agrees with msg.getContentStream()",
            testInfo.getMethodName());
      } else {
        ZimbraLog.test.debug(
            "%s ZMessage.getContent() differs from contents of msg.getContentStream()"
    ...
    assertEquals(
        "Imap UID of ZMessage should be same as Message", zmsg.getImapUid(), msg.getImapUid());
  }
  ...
  public void testDeleteAllPresent() throws Exception {
    ...
    Message msg =
        TestUtil.addMessage(
            mbox, Mailbox.ID_FOLDER_INBOX, "testDelete message", System.currentTimeMillis());
  ...
  public void testDeletableFolder() throws Exception {
    ...
    ZFolder zf =
        zmbox.createFolder(
            Integer.toString(Mailbox.ID_FOLDER_USER_ROOT),
            "deleteme",
            ZFolder.View.message,
            ZFolder.Color.DEFAULTCOLOR,
            null,
            null);
  ...
  public void testImmutableFolder() throws Exception {
    ...
    assertFalse(
        "folder::isDeletable should return false for 'immutable' folder", testFolder.isDeletable());
    int folderId = testFolder.getId();
    assertTrue(
        "folderId should be higher than default system IDs", folderId >= Mailbox.FIRST_USER_ID);
    ...
    assertFalse(
        "zFolder::isDeletable should return false for 'immutable' folder", zf.isDeletable());
  ...
  public void testHiddenFolder() throws Exception {
    ...
    int folderId = testFolder.getId();
    assertTrue(
        "folderId for /rootNonDeletable should be higher than default system IDs",
        folderId >= Mailbox.FIRST_USER_ID);
    ...
    zf = zmbox.getFolderRequestById(szFolderId);
    assertNotNull(
        "ZMailbox::getFolderRequestById should be able to get /rootNonDeletable folder by ID", zf);
    assertFalse(
        "zFolder::isDeletable should return false for 'immutable' folder", zf.isDeletable());
    assertEquals("unexpected zfolder ID", szFolderId, zf.getId());
  ...
  public void testVisibleFolders() throws Exception {
    ...
    for (ZFolder zf : folders) {
      ...
      assertFalse(
          String.format("Folder %s should not be deletable", zf.getPath()), zf.isDeletable());
    ...
    assertTrue(
        "did not find /Emailed Contacts", folderIds.contains(Mailbox.ID_FOLDER_AUTO_CONTACTS));
  ...
  public void testRenameFolder() throws Exception {
    ...
    Folder testFolder =
        mbox.createFolder(null, "testRenameFolderOriginal", Mailbox.ID_FOLDER_USER_ROOT, fopt);
    ...
    assertEquals("unexpected renamed zfolder ID", szFolderId, zf.getId());
    assertEquals(
        "zfolder should not be aware of new name yet", "testFolderRenamedOnce", zf.getName());
    assertEquals(
        "zfolder should not be aware of new path yet", "/testFolderRenamedOnce", zf.getPath());
    ...
    assertEquals("unexpected renamed zfolder ID", szFolderId, zf.getId());
    assertEquals(
        "zfolder should pick up new name for renamed folder after NoOp",
        "testFolderRenamedTwice",
        zf.getName());
    assertEquals(
        "zfolder should pick up new path for renamed folder after NoOp",
        "/testFolderRenamedTwice",
        zf.getPath());
  }

  private ZSearchFolder doCreateSearchFolder(
      ZMailbox zmbox,
      String parentId,
      String name,
      String query,
    ...
    try {
      ...
      assertNotNull(
          String.format("createSearchFolder failed for '%s' returned null ZSearchFolder", name),
          sf);
  ...
  public void createSearchFolder() throws ServiceException {
    ZMailbox zmbox = TestUtil.getZMailbox(USER_NAME);
    doCreateSearchFolder(
        zmbox,
        ZFolder.ID_USER_ROOT,
        "isReadSearch-msg-SortAsc-green",
        "is:read",
        MailItemType.MESSAGE.name(),
        SearchSortBy.nameAsc,
        ZFolder.Color.GREEN);
    doCreateSearchFolder(
        zmbox,
        ZFolder.ID_USER_ROOT,
        "isReadSearch-msg-SortAsc-NO-COLOR",
        "is:read",
        MailItemType.MESSAGE.name(),
        SearchSortBy.nameAsc,
        (ZFolder.Color) null);
    doCreateSearchFolder(
        zmbox,
        ZFolder.ID_USER_ROOT,
        "isReadSearch-msg-NO-SORT-green",
        "is:read",
        MailItemType.MESSAGE.name(),
        (SearchSortBy) null,
        ZFolder.Color.GREEN);
    doCreateSearchFolder(
        zmbox,
        ZFolder.ID_USER_ROOT,
        "isReadSearch-NO-TYPE-SortAsc-green",
        "is:read",
        (String) null,
        SearchSortBy.nameAsc,
        ZFolder.Color.GREEN);
  ...
  public void copyMsgToMountpoint() throws ServiceException, IOException, MessagingException {
    ...
    ZMountpoint mp =
        TestUtil.createMountpoint(sharerZmbox, "/" + sharedFolder, shareeZmbox, mountpoint);
    String msgId =
        TestUtil.addMessage(
            shareeZmbox, String.format("test message for %s", testInfo.getMethodName()));
    ...
    assertEquals(
        "ZActionResult Ids array length for move message", 1, result.getIdsAsArray().length);
  }
  ...
  public void searchImapWithCursor() throws Exception {
    ...
    ZimbraSearchParams params =
        zmbox.createSearchParams("in:\"INBOX\" (item:{266--399} -tag:\\Deleted)");
    params.setIncludeTagDeleted(true);
