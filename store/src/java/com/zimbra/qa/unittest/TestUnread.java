...
public class TestUnread {
  ...
  private Message getMessage1() throws Exception {
    return mMbox.getMessageById(null, mMessage1Id);
  }

  private Message getMessage2() throws Exception {
    return mMbox.getMessageById(null, mMessage2Id);
  }

  private Message getMessage3() throws Exception {
    return mMbox.getMessageById(null, mMessage3Id);
  }

  private Conversation getConv() throws Exception {
    return mMbox.getConversationById(null, mConvId);
  }

  private Folder getFolder1() throws Exception {
    return mMbox.getFolderById(null, mFolder1Id);
  }

  private Folder getFolder2() throws Exception {
    return mMbox.getFolderById(null, mFolder2Id);
  }

  private Tag getTag1() throws Exception {
    return mMbox.getTagById(null, mTag1Id);
  }

  private Tag getTag2() throws Exception {
    return mMbox.getTagById(null, mTag2Id);
  }

  private Tag getTag3() throws Exception {
    return mMbox.getTagById(null, mTag3Id);
  }
   ...
   *
   * <ul>
   *   <li>F1 contains F2 and M1
   *   <li>F2 contains M2 and M3
   *   <li>T1 is assigned to M1
   *   <li>T2 is assigned to M1 and M2
   * </ul>
   */
  @BeforeEach
  public void setUp() throws Exception {
    ...
    Assert.assertNotNull(
        String.format("Unable to create account for user '%s'", USER_NAME), mAccount);
    ...
    Folder folder =
        mMbox.createFolder(null, FOLDER1_NAME, Mailbox.ID_FOLDER_INBOX, new Folder.FolderOptions());
  ...
  public void testSearch() throws Exception {
    ...
    ZimbraQueryResults results =
        mMbox.index.search(
            ...
            "is:unread",
            EnumSet.of(MailItem.Type.MESSAGE),
            SortBy.DATE_DESC,
            100);
  ...
  private void verifyAllUnreadFlags() throws Exception {
    ...
    DbResults results =
        DbUtil.executeQuery(
            "SELECT COUNT(*) "
                + "FROM "
                + DbMailItem.getMailItemTableName(mMbox)
                + " WHERE mailbox_id = "
                + mMbox.getId()
                + " AND flags & "
                + Flag.BITMASK_UNREAD
  ...
  private void verifyUnreadFlag(MailItem item) throws Exception {
    ...
    if (item.isUnread()) {
      Assert.assertTrue(
          "unread bit test: " + item.getFlagBitmask(),
          (item.getFlagBitmask() & Flag.BITMASK_UNREAD) > 0);
      Assert.assertTrue(
          "unread flag string: " + flagString,
          flagString.indexOf(Flag.toChar(Flag.ID_UNREAD)) >= 0);
    } else {
      Assert.assertTrue(
          "read bit test: " + item.getFlagBitmask(),
          (item.getFlagBitmask() & Flag.BITMASK_UNREAD) == 0);
      Assert.assertTrue(
          "read flag string: " + flagString, flagString.indexOf(Flag.toChar(Flag.ID_UNREAD)) == -1);
    }
    ...
    if (item.getType() == MailItem.Type.MESSAGE) {
      DbResults results =
          DbUtil.executeQuery(
              "SELECT unread "
                  + "FROM "
                  + DbMailItem.getMailItemTableName(item)
                  + " WHERE mailbox_id = "
                  + item.getMailboxId()
                  + " AND id = "
                  + item.getId());
      Assert.assertEquals(
          "Verify unread flag in the database", item.isUnread(), results.getBoolean(1));
    }
  }
