...
public class TestBug89152 extends TestCase {
  ...
  private static final String TEST_MESSAGE_FILE =
      "/Users/gsolovyev/p4/main/ZimbraQA/data/TestMailRaw/bug89152/testmessage.eml";
  ...
  public void testMessagesWithDifferentTimestamps() {
    try {
        ...
        for (ZMessage m : resultSet) {
          assertFalse(
              String.format(
                  ...
                  recCount, resCount, m.getId(), seenIds.indexOf(m.getId())),
              seenIds.contains(m.getId()));
          seenIds.add(m.getId());
          resCount++;
          lastHit = m;
        }
      ...
      assertEquals(
          "expecting " + numMessages + " messages. Got " + seenIds.size(),
          numMessages,
          seenIds.size());
