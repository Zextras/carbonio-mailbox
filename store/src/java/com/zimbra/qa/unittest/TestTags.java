...
public class TestTags {
  ...
  public void testAddTag() throws Exception {
    ...
    ActionSelector msgAction =
        ActionSelector.createForIdsAndOperation(
            Integer.toString(mMessage1.getId()), MailConstants.OP_UPDATE);
    ...
    msgAction =
        ActionSelector.createForIdsAndOperation(
            Integer.toString(mMessage3.getId()), MailConstants.OP_TAG);
  ...
  public void testTagSearch() throws Exception {
    ...
    ids =
        search(
            "tag:" + mTags[0].getName() + " or tag:" + mTags[1].getName(), MailItem.Type.MESSAGE);
    ...
    ids =
        search(
            "not tag:" + mTags[1].getName() + " not tag:" + mTags[2].getName(),
    ...
    ids =
        search(
            "tag:" + mTags[1].getName() + " not tag:" + mTags[2].getName(), MailItem.Type.MESSAGE);
  ...
  public void testRemoteTagSearch() throws Exception {
    ...
    remoteMbox.grantAccess(
        null,
        Mailbox.ID_FOLDER_INBOX,
        mAccount.getId(),
        ACL.GRANTEE_USER,
        (short) (ACL.RIGHT_READ | ACL.RIGHT_WRITE | ACL.RIGHT_INSERT),
        null);
    mMbox.createMountpoint(
        ...
        false);
    Message remoteMsg1 =
        TestUtil.addMessage(remoteMbox, MSG_SUBJECT + " in shared inbox tagged with shared TAG");
    Message remoteMsg2 =
        TestUtil.addMessage(remoteMbox, MSG_SUBJECT + " in shared inbox tagged remOnly");
    ...
    refresh();
    remoteMbox.alterTag(
        null, remoteMsg1.getId(), remoteMsg1.getType(), remoteTags[0].getName(), true, null);
    remoteMbox.alterTag(
        null, remoteMsg2.getId(), remoteMsg2.getType(), remoteTags[1].getName(), true, null);
    mMbox.alterTag(null, mMessage2.getId(), mMessage2.getType(), mTags[0].getName(), true, null);
    mMbox.alterTag(null, mMessage3.getId(), mMessage3.getType(), mTags[1].getName(), true, null);
    Folder sharedFolder = mMbox.getFolderByName(null, Mailbox.ID_FOLDER_USER_ROOT, "remoteInbox");
    sharedFolder.getId();
    List<ZimbraHit> hits =
        TestUtil.searchForHits(
            mMbox,
            String.format(
                "tag:\"%s\" (inid:%d OR is:local)", mTags[0].getName(), sharedFolder.getId()),
            MailItem.Type.MESSAGE);
    assertEquals(
        "Search for tag present in both local and remote mboxes.  Number of hits returned",
        2,
        hits.size());
    ...
    Set<Integer> ids =
        search(
            String.format(
                "tag:\"%s\" (inid:%d OR is:local)", mTags[1].getName(), sharedFolder.getId()),
            MailItem.Type.MESSAGE);
    assertEquals(
        "Search for tag not present in remote mbox. Number of ids returned", 1, ids.size());
  ...
  public void testFlagSearch() throws Exception {
    ...
    mMbox.alterTag(
        null, mMessage2.getId(), mMessage2.getType(), Flag.FlagInfo.REPLIED, false, null);
    mMbox.alterTag(
        null, mMessage3.getId(), mMessage3.getType(), Flag.FlagInfo.REPLIED, false, null);
    mMbox.alterTag(
        null, mMessage4.getId(), mMessage4.getType(), Flag.FlagInfo.REPLIED, false, null);
    ...
    mMbox.alterTag(
        null, mMessage3.getId(), mMessage3.getType(), Flag.FlagInfo.FORWARDED, true, null);
    refresh();
  ...
  public void testSearchUnreadAsTag() throws Exception {
    ...
    if (!(isUnreadIds.containsAll(tagUnreadIds))) {
      fail(
          "Mismatch in search results.  is:unread returned ("
              + StringUtil.join(",", isUnreadIds)
              + "), tag:\\Unread returned ("
