...
public class TestCountObjects extends TestCase {
  ...
  public void setUp() throws Exception {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      Account acct =
          mProv.createAccount("count-test-acc" + i + "@" + DOMAIN_NAME, PASSWORD, acctAttrs);
      assertNotNull(acct);
    ...
    for (int i = 0; i < 10; i++) {
      Map<String, Object> acctAttrs = new HashMap<String, Object>();
      DistributionList dl =
          mProv.createDistributionList("count-test-DL" + i + "@" + DOMAIN_NAME, acctAttrs);
      assertNotNull(dl);
    ...
    mProv.grantRight(
        ...
        TargetBy.name,
        DOMAIN_NAME,
        com.zimbra.cs.account.accesscontrol.GranteeType.GT_USER.getCode(),
        GranteeBy.name,
        DOMAIN_ADMIN_USER_EMAIL,
        null,
        RightConsts.RT_domainAdminRights,
        null);
    testAccountIDs.add(acct.getId());
    ...
    attrs = new HashMap<String, Object>();
    attrs.put(Provisioning.A_zimbraIsDelegatedAdminAccount, ProvisioningConstants.TRUE);
    acct = mProv.createAccount(DELEGATED_ADMIN_USER_EMAIL, PASSWORD, attrs);
    assertNotNull(acct);
    mProv.grantRight(
        TargetType.domain.getCode(),
        TargetBy.name,
    ...
    mProv.grantRight(
        ...
        com.zimbra.cs.account.accesscontrol.GranteeType.GT_USER.getCode(),
        GranteeBy.name,
        ROGUE_ADMIN_USER_EMAIL,
        null,
        RightConsts.RT_countDistributionList,
        null);
    testAccountIDs.add(acct.getId());
  }
  ...
  public void testDomainAdmin() throws Exception {
    try {
      ...
      try {
        ...
        assertEquals(
            "should not be able to count domains", ServiceException.PERM_DENIED, e.getCode());
      }
      ...
      try {
        ...
        assertEquals(
            "should not be able to count COSes", ServiceException.PERM_DENIED, e.getCode());
      }
      ...
      assertEquals(
          "object type in response should be 'userAccount'", "userAccount", resp.getType());
      ...
      req =
          new CountObjectsRequest(
              CountObjectsType.dl, new DomainSelector(DomainBy.name, DOMAIN_NAME));
      resp = SoapTest.invokeJaxb(transport, req);
      assertTrue("should have at least one distribution list", resp.getNum() > 0);
      assertEquals("object type in response should be 'dl'", "dl", resp.getType());
