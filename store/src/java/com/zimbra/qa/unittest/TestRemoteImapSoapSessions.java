...
public class TestRemoteImapSoapSessions extends ImapTestBase {
  ...
  public void setUp() throws ServiceException, IOException, DocumentException, ConfigException {
    ...
    boolean canUseLocalImap =
        imapServer.isImapServerEnabled() && imapServer.isImapCleartextLoginEnabled();
    if (canUseLocalImap) {
      TestUtil.setLCValue(LC.imap_always_use_remote_store, String.valueOf(true));
    } else {
      canUseRemoteImap =
          imapServer.isRemoteImapServerEnabled()
              && imapServer.isImapCleartextLoginEnabled()
              && Arrays.asList(imapServer.getReverseProxyUpstreamImapServers())
                  .contains(imapServer.getServiceHostname());
    }
    TestUtil.assumeTrue(
        "neither embeded remote, nor standalone imapd are available",
        canUseRemoteImap || canUseLocalImap);
  }
  ...
  public void testLogout() throws Exception {
    Collection<Session> sessionsBeforeLogin =
        SessionCache.getSoapSessions(TestUtil.getAccount(USER).getId());
    connection = connect();
    connection.login(PASS);
    connection.select("INBOX");
    Collection<Session> sessionsAfterLogin =
        SessionCache.getSoapSessions(TestUtil.getAccount(USER).getId());
    ...
    assertEquals(
        "Should have one more session after login",
        numSessionsBeforeLogin,
        numSessionsAfterLogin - 1);
    connection.logout();
    Thread.sleep(500);
    Collection<Session> sessionsAfterLogout =
        SessionCache.getSoapSessions(TestUtil.getAccount(USER).getId());
    int numSessionsAfterLogout = sessionsAfterLogout == null ? 0 : sessionsAfterLogout.size();
    assertEquals(
        "Should have as many sessions after logout as before login",
        numSessionsBeforeLogin,
        numSessionsAfterLogout);
  }
  ...
  public void testOpenCloseFolders() throws Exception {
    ...
    Collection<Session> sessionsAfterSelect =
        SessionCache.getSoapSessions(TestUtil.getAccount(USER).getId());
    int numSessionsAfterSelect = sessionsAfterSelect == null ? 0 : sessionsAfterSelect.size();
    assertEquals(
        "Should have as many sessions after selecting DRAFTS as after login",
        numSessionsAfterSelect,
        numSessionsAfterLogin);
    connection.close_mailbox();
    Collection<Session> sessionsAfterClose =
        SessionCache.getSoapSessions(TestUtil.getAccount(USER).getId());
    int numSessionsAfterClose = sessionsAfterClose == null ? 0 : sessionsAfterClose.size();
    assertEquals(
        "Should have as many sessions after closing DRAFTS as after login",
        numSessionsAfterClose,
        numSessionsAfterLogin);
    connection.create("testOpenCloseFolders");
    folderInfo = connection.select("testOpenCloseFolders");
    assertNotNull(
        String.format("return MailboxInfo for 'SELECT %s'", "testOpenCloseFolders"), folderInfo);
    ...
    assertEquals(
        "Should have as many sessions after selecting 'testOpenCloseFolders' as after login",
        numSessionsAfterSelect,
        numSessionsAfterLogin);
  }
  ...
  public void testTerminate() throws Exception {
    ...
    int numSessionsAfterClose = sessionsAfterClose == null ? 0 : sessionsAfterClose.size();
    assertEquals(
        "SOAP session should be dropped when IMAP client drops without logging out",
        numSessionsBeforeLogin,
        numSessionsAfterClose);
  }
