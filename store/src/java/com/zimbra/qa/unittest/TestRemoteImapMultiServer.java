...
public class TestRemoteImapMultiServer {
  ...
  public static void beforeClass() throws Exception {
    ...
    sp.soapSetURI(
        LC.zimbra_admin_service_scheme.value()
            + servers.get(0).getServiceHostname()
            + ":"
            + servers.get(0).getAdminPort()
  ...
  public void setUp() throws ServiceException, IOException {
    TestUtil.assumeTrue(
        String.format("Number of servers=%d needs to be > 1", servers.size()), servers.size() > 1);
    cleanup();
  ...
  public void testAUTHENTICATEToHomeHost() throws IOException {
    ...
    try {
      ...
      assertNotNull(e);
      assertTrue(
          "Should toget 'LOGIN failed' error from home server",
          e.getMessage().indexOf("LOGIN failed") > -1);
    }
    assertFalse("IMAP connection should not be authenticated", connection.isAuthenticated());
  }

  @Test
  public void testAuthenticateAsProxyToIMAPHost()
      throws IOException, LoginException, AuthProviderException, AuthTokenException {
  ...
  @Test
  public void testAuthenticateAsProxyToHomeHost()
      throws IOException, LoginException, AuthProviderException, AuthTokenException {
  ...
  @Disabled("requires SELECT to work")
  ...
  public void testSubscribeNested() throws IOException, ServiceException {
    ...
    Assert.assertTrue(
        "Should be subscribed to "
            + folder.getPath().substring(1)
            + ". Instead got "
            + listResult.get(0).getMailbox(),
        folder.getPath().substring(1).equalsIgnoreCase(listResult.get(0).getMailbox()));
  }
  ...
  public void testLOGOUTNotLoggedIn() throws IOException {
    ...
    connection.logout();
    assertFalse(
        "IMAP connection should not be authenticated after sending LOGOUT",
  ...
  public void testLOGOUT() throws IOException {
    ...
    connection.login(PASS);
    Assert.assertTrue(
        "IMAP should be authenticated after logging in", connection.isAuthenticated());
    connection.logout();
    assertFalse(
        "IMAP connection should not be authenticated after sending LOGOUT",
        connection.isAuthenticated());
  ...
  public void testListFolderContents() throws IOException, ServiceException, MessagingException {
    ...
    Map<Long, MessageData> mdMap =
        connection.fetch("1:*", "(ENVELOPE INTERNALDATE BODY BODY[TEXT])");
  ...
  public void testMultipleSelect() throws Exception {
    ...
    for (int i = 0; i < 3; i++) {
      Future<MailboxInfo> future =
          executor.submit(
              new Callable<MailboxInfo>() {
