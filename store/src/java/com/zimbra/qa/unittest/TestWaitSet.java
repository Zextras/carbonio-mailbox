  ...
  private void runMeFirst() throws Exception {
    ...
    try {
      ...
      { // waitset should pick up added user
        ...
        synchronized (cb) {
          assertEquals(
              "Callback should not be completed before it is triggered by the second account",
              false,
              cb.completed);
        }
        ...
        synchronized (cb) {
          assertEquals(
              "Callback should be completed after it is triggered by the second account",
              true,
              cb.completed);
        }
        curSeqNo = cb.seqNo;
  ...
  public void runMeSecond() throws Exception {
    ...
    try {
      ...
      { // waitset shouldn't signal until message added to a mailbox
        ...
        synchronized (cb) {
          assertFalse(
              "callback for all accounts should not be completed before it is triggered",
              cb.completed);
        }
        ...
        synchronized (cb) {
          assertTrue(
              "callback for all accounts should be completed after it is triggered by "
                  + USER_1_NAME,
              cb.completed);
        }
        curSeqNo = cb.seqNo;
      ...
      { // waitset should remain signalled until sequence number is increased
        ...
        synchronized (cb) {
          assertTrue(
              "call back for all accounts should remain completed until sequence number is"
                  + " increased",
              cb.completed);
        }
        curSeqNo = cb.seqNo;
      ...
      { // waitset shouldn't signal until a document is added
        ...
        synchronized (cb) {
          assertFalse(
              "call back for all accounts should switch to not completed after sequence number is"
                  + " increased",
              cb.completed);
        }
        ...
        TestUtil.createDocument(
            TestUtil.getZMailbox(USER_2_NAME),
            ZFolder.ID_BRIEFCASE,
            subject,
            "text/plain",
      ...
      { // part 2: waitset for "all" should pick up new account added
        ...
        synchronized (cb) {
          assertTrue(
              "Callback should be completed after adding a message to " + WS_USER_NAME,
              cb.completed);
        }
        curSeqNo = cb.seqNo;
      }
