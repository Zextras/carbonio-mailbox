...
public class MimeHeaderTest {
  ...
  public void encode() {
    ...
    src = "Pru\u00ee";
    Assert.assertEquals(
        "=?iso-8859-1?Q?Pru=EE?=", MimeHeader.escape(src, CharsetUtil.ISO_8859_1, true));
    Assert.assertEquals(
        "=?iso-8859-1?Q?Pru=EE?=", MimeHeader.escape(src, CharsetUtil.ISO_8859_1, false));
    ...
    src =
        "lskdhf lkshfl aksjhlfi ahslkfu Pru\u00ee uey liufhlasuifh haskjhf lkajshf"
            + " lkajshflkajhslkfj hals\u00e4kjhf laskjhdflaksjh ksjfh ka";
    Assert.assertEquals(
        "lskdhf lkshfl aksjhlfi ahslkfu"
            + " =?utf-8?Q?Pru=C3=AE_uey_liufhlasuifh_haskjhf_lkajshf_lkajshflkajhslkfj_hals=C3=A4kjhf?="
            + " laskjhdflaksjh ksjfh ka",
    ...
    src = "\u00eb\u00ec\u00ed\u00ee";
    Assert.assertEquals(
        "=?iso-8859-1?B?6+zt7g==?=", MimeHeader.escape(src, CharsetUtil.ISO_8859_1, true));
    Assert.assertEquals(
        "=?iso-8859-1?B?6+zt7g==?=", MimeHeader.escape(src, CharsetUtil.ISO_8859_1, false));
  }
  ...
  public void decode() {
    String src =
        "RE: [Bug"
            + " 30944]=?UTF-8?Q?=20Meeting=20invitation=20that=E2=80=99s=20created=20within=20exchange=20containing=20=C3=A5=C3=A4=C3=B6=20will=20show=20within=20the=20calendar=20and=20acceptance=20notification=20as=20?=?????";
    Assert.assertEquals(
        "RE: [Bug 30944] Meeting invitation that\u2019s created within exchange containing"
            + " \u00e5\u00e4\u00f6 will show within the calendar and acceptance notification as"
            + " ?????",
  ...
  public void charsetSuperset() {
    ...
    Assert.assertEquals(
        "=?UTF-8?Q?=E2=82=AC200000?=",
        MimeHeader.escape("\u20ac200000", CharsetUtil.ISO_8859_1, true).toUpperCase());
  }
  ...
  public void semiColonSeparatedGroup() {
    for (int i = 0; i < 2; i++) {
      ...
      String src =
          "mine:=?us-ascii?Q?Bob_?=\t=?us-ascii?Q?the_Builder_1?= <bob@example.com>, Group User 2"
              + " <user2@group.com>;=?us-ascii?Q?Bob the Builder 2?= <bob@example.com>";
      MimeAddressHeader hdr = new MimeAddressHeader("To", src);
      List<InternetAddress> iaddrs = hdr.expandAddresses();
  ...
  public void semiColonSeparatedGroupMixed() {
    String src =
        "mine:=?us-ascii?Q?Bob_?=\t=?us-ascii?Q?the_Builder_1?= <bob@example.com>, Group User 2"
            + " <user2@group.com>;=?us-ascii?Q?Bob the Builder 2?="
            + " <bob@example.com>;=?us-ascii?Q?Bob the Builder 3?= <bob3@example.com>";
    MimeAddressHeader hdr = new MimeAddressHeader("To", src);
    List<InternetAddress> iaddrs = hdr.expandAddresses();
