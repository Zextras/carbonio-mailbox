...
public class MimeCompoundHeaderTest {
  private void test(
      boolean isContentType, String description, String raw, String value, String[] params) {
    MimeCompoundHeader mch = isContentType ? new ContentType(raw) : new ContentDisposition(raw);

    String primary =
        isContentType
            ? ((ContentType) mch).getContentType()
            : ((ContentDisposition) mch).getDisposition();
  ...
  public void testContentType() {
    test(
        ...
        "text/plain",
        new String[] {"charset", "US-ASCII", "format", "Flowed"});
    test(
        true,
        ...
        "application/x-stuff",
        new String[] {"title", "This is even more ***fun*** isn't it!"});
    test(
        true,
        "downcasing value, implicit end-of-value at eol",
        "multipart/MIXED; charset=us-ascii;\n foo=\n  boundary=\"---\" \n",
        "multipart/mixed",
        new String[] {"charset", "us-ascii", "foo", "boundary=\"---\""});
    test(
        true,
    ...
    test(
        ...
        new String[] {
          "name",
          "TriplePlay_Converged_Network_v5.pdf",
          "x-mac-creator",
          "70727677",
          "x-mac-type",
          "50444620"
    ...
    test(
        true,
        "'text' as value, backslashes in quoted-string, missing equals, missing param name, blank"
            + " param, comments before param name, nested comments",
        "text;\n"
            + " pflaum;=foo; name=\"spam\\\"bag\\\\wall\" \n"
            + "\t((plain; text=missing); (pissed=off); where=myrtle);;a=b;c;=d;\n"
            + " (a)foo=bar",
        "text/plain",
        new String[] {"pflaum", "", "name", "spam\"bag\\wall", "a", "b", "c", "", "foo", "bar"});
    test(true, "null input", null, "text/plain", new String[] {});
    test(
        true,
        ...
        "text/plain",
        new String[] {"format", "floo", "zoom", "whop"});
    test(
        true,
        "unquoted encoded-words, bad encoded-words in non-2231 values",
        "text/plain; filename==?us-ascii?q?boo_bah.pdf?=; note=\"   ?==?\";"
            + " bloop=\"=?x-unknown?a?text?=\" ",
        "text/plain",
        new String[] {
          "filename", "boo bah.pdf", "note", "   ?==?", "bloop", "=?x-unknown?a?text?="
        });
    test(
        true,
        "tspecials in primary value",
        "audio/x-mpeg-3,name=VM_2012-01-24_11-12-54.mp3",
        "audio/x-mpeg-3",
        new String[] {});
    test(
        true,
        "extra slash in primary value",
        "text/plain/html; foo=bar",
        "text/plain",
        new String[] {"foo", "bar"});
    test(
        true,
        "trailing slash in primary value",
        "text/; foo=bar",
        "text/plain",
        new String[] {"foo", "bar"});
    test(
        true,
        "trailing double slash in primary value",
        "text//; foo=bar",
        "text/plain",
        new String[] {"foo", "bar"});
  }
  ...
  public void testContentDisposition() {
    test(
        ...
        "   \n"
            ...
            + " ",
        "inline",
        new String[] {"filename", "\u30bd\u30eb\u30c6\u30a3\u30ec\u30a4.rtf"});
    test(
        false,
    ...
    test(
        ...
        "attachment",
        new String[] {"filename", "\uff12\u6708\u53f7.jpg"});
    test(
        false,
        "encoded continuations",
        "attachment; filename*0*=ISO-8859-1''BASE%20INICIAL%20CAMPANHA%20PROVIS%C3O%20ABAIXO;"
            + " filename*1*=%20DE%20ZERO%2009_10_06%20SUCHY.xls",
        "attachment",
        new String[] {
          "filename", "BASE INICIAL CAMPANHA PROVIS\u00c3O ABAIXO DE ZERO 09_10_06 SUCHY.xls"
        });
    test(
        false,
    ...
    test(
        ...
        "inline",
        new String[] {"filename", "1565 ST\u00c5ENDE CAD Netic SKI.pdf"});
    test(
        false,
        "misordered continuations, continuations overriding standard value",
        "inline;\n"
        ...
        "inline",
        new String[] {"filename", "1565 H\u00c5MBURGER.pdf"});
    test(
        false,
        "leading CFWS, missing semicolon after value, missing semicolon after quoted-string,"
            + " trailing comment",
        "  \n inline\n foo=\"bar\"\n baz=whop\n (as)\n",
        "inline",
        new String[] {"foo", "bar", "baz", "whop"});
    test(
        false,
        "missing charset on first continuation",
        "attachment; foo*0=big; foo*1*=%20dog",
        "attachment",
        new String[] {"foo", "big dog"});
    test(
        false,
    ...
    test(
        ...
        "attachment",
        new String[] {"foo", "iso-8859-1'en'big dog"});
    test(
        false,
        ...
        "inline",
        new String[] {"filename", "\u3053\u3093\u306b\u3061\u306f\u3001\u4e16\u754c\uff01.pdf"});
    test(
        false,
        "bare 8-bit chars in filename",
        "attachment; filename=\"About \u00E5 \u00E4 \u00F6 Downloads.pdf\"",
        "attachment",
        new String[] {"filename", "About \u00E5 \u00E4 \u00F6 Downloads.pdf"});
    test(
        false,
        "missing 2231 charset/language",
        "Content-Disposition: attachment; \r\n"
            + "filename*=Jarrod_Stiles%7a_resume_5579615_10344909272010946044.pdf",
        "attachment",
        new String[] {"filename", "Jarrod_Stilesz_resume_5579615_10344909272010946044.pdf"});
    test(
        false,
        "missing 2231 charset/language with subsequent param",
        "Content-Disposition: attachment; \r\n"
            + "filename*=Jarrod_Stiles%7a_resume_5579615_10344909272010946044.pdf; foo=bar",
        "attachment",
        new String[] {
          "filename", "Jarrod_Stilesz_resume_5579615_10344909272010946044.pdf", "foo", "bar"
        });
  }
