...
public final class HttpUtilTest {
  ...
  public void encodeFilename() {
    ...
    filename = "\u65e5\u672c\u8a9e.pdf";
    Assert.assertEquals(
        "%E6%97%A5%E6%9C%AC%E8%AA%9E.pdf", HttpUtil.encodeFilename(Browser.IE, filename));
    Assert.assertEquals(
        "\"=?utf-8?B?5pel5pys6KqeLnBkZg==?=\"", HttpUtil.encodeFilename(Browser.FIREFOX, filename));
    Assert.assertEquals("", HttpUtil.encodeFilename(Browser.SAFARI, filename));
    Assert.assertEquals(
        "\"=?utf-8?B?5pel5pys6KqeLnBkZg==?=\"", HttpUtil.encodeFilename(Browser.UNKNOWN, filename));

    filename = "\u65e5 \u672c \u8a9e.pdf";
    Assert.assertEquals(
        "%E6%97%A5%20%E6%9C%AC%20%E8%AA%9E.pdf", HttpUtil.encodeFilename(Browser.IE, filename));
    Assert.assertEquals(
        "\"=?utf-8?B?5pelIOacrCDoqp4ucGRm?=\"", HttpUtil.encodeFilename(Browser.FIREFOX, filename));
    Assert.assertEquals("", HttpUtil.encodeFilename(Browser.SAFARI, filename));
    Assert.assertEquals(
        "\"=?utf-8?B?5pelIOacrCDoqp4ucGRm?=\"", HttpUtil.encodeFilename(Browser.UNKNOWN, filename));
  }
  ...
  public void testIEDetection() {
    // 10.6
    String IE_10_6 =
        "Mozilla/5.0 (compatible; MSIE 10.6; Windows NT 6.1; Trident/5.0; InfoPath.2; SLCC1; .NET"
            + " CLR 3.0.4506.2152; .NET CLR 3.5.30729; .NET CLR 2.0.50727) 3gpp-gba UNTRUSTED/1.0";
    ...
    String IE_9_0 =
        "Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; Trident/5.0; SLCC2; .NET CLR 2.0.50727;"
            + " .NET CLR 3.5.30729; .NET CLR 3.0.30729; Media Center PC 6.0; InfoPath.2; .NET CLR"
            + " 1.1.4322; .NET4.0C; Tablet PC 2.0)";
    ...
    String IE_7_0 =
        "Mozilla/5.0 (compatible; MSIE 7.0; Windows NT 6.0; SLCC1; .NET CLR 2.0.50727; Media Center"
            + " PC 5.0; c .NET CLR 3.0.04506; .NET CLR 3.5.30707; InfoPath.1; el-GR)";
  ...
  public void testFirefoxDetection() {
    ...
    String FF_10_0_a4 =
        "Mozilla/6.0 (Macintosh; I; Intel Mac OS X 11_7_9; de-LI; rv:1.9b4) Gecko/2012010317"
            + " Firefox/10.0a4";
  ...
  public void testChromeDetection() {
    ...
    String CH_17_0 =
        "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_7_2) AppleWebKit/535.11 (KHTML, like Gecko)"
    ...
    String CH_10_0 =
        "Mozilla/5.0 (X11; U; Linux i686; en-US) AppleWebKit/534.15 (KHTML, like Gecko)"
            + " Ubuntu/10.10 Chromium/10.0.611.0 Chrome/10.0.611.0 Safari/534.15";
    ...
    String CH =
        "Mozilla/5.0 (Macintosh; U; Mac OS X 10_6_1; en-US) AppleWebKit/530.5 (KHTML, like Gecko)"
            + " Chrome/ Safari/530.5";
  ...
  public void testSafariDetection() {
    String SF_5_0_5 =
        "Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10_6_8; de-at) AppleWebKit/533.21.1 (KHTML, like"
            + " Gecko) Version/5.0.5 Safari/533.21.1";
    ...
    String SF_4_1 =
        "Mozilla/5.0 (Windows; U; Windows NT 5.0; en-en) AppleWebKit/533.16 (KHTML, like Gecko)"
            + " Version/4.1 Safari/533.16";
    browser = HttpUtil.guessBrowser(SF_4_1);
    Assert.assertEquals(Browser.SAFARI, browser);
    Assert.assertEquals(4, HttpUtil.guessBrowserMajorVersion(SF_4_1));

    String SF_3_2_3 =
        "Mozilla/5.0 (Windows; U; Windows NT 5.1; cs-CZ) AppleWebKit/525.28.3 (KHTML, like Gecko)"
            + " Version/3.2.3 Safari/525.29";
    browser = HttpUtil.guessBrowser(SF_3_2_3);
    Assert.assertEquals(Browser.SAFARI, browser);
    Assert.assertEquals(3, HttpUtil.guessBrowserMajorVersion(SF_3_2_3));

    String SF_2_0_2 =
        "Mozilla/5.0 (Macintosh; U; PPC Mac OS X; nl-nl) AppleWebKit/416.12 (KHTML, like Gecko)"
            + " Safari/416.13";
    ...
    // browsers apart we can do it by the build numbers, but they are probably pretty old and not
    // worth
    // the code complication to figure that out.
    String SF_1_3_2 =
        "Mozilla/5.0 (Macintosh; U; PPC Mac OS X; sv-se) AppleWebKit/312.8 (KHTML, like Gecko)"
            + " Safari/312.5";
  ...
  public void testIEDisposition() {
    ...
    Assert.assertEquals(
        "attachement; filename=\"ascii.txt\"",
        HttpUtil.createContentDisposition(request, "attachement", asciiFilename));
    Assert.assertEquals(
        "attachement; filename*=UTF-8''space%22%20space.txt",
        HttpUtil.createContentDisposition(request, "attachement", asciiQuote));
    Assert.assertEquals(
        "attachement; filename*=UTF-8''Wikip%C3%A9dia.txt",
        HttpUtil.createContentDisposition(request, "attachement", iso8859Filename));
    Assert.assertEquals(
        "attachement;"
            + " filename*=UTF-8''%ED%95%9C%EA%B5%AD%EC%96%B4%2520%EC%88%98%EC%8B%A0%EC%9E%90.pdf",
        HttpUtil.createContentDisposition(request, "attachement", unicodeFilename));
  }
  ...
  public void testChrome10minusDispositionNoPath() {
    ...
    String asciiFilename = "ascii.txt";
    String asciiQuote =
        "space\" space.txt"; // note, we can't upload a file with this name in chrome.
    String iso8859Filename = "Wikip\u00E9dia.txt";
    String unicodeFilename = "\uD55C\uAD6D\uC5B4%20\uC218\uC2E0\uC790.pdf";
    ...
    Assert.assertEquals(
        ...
        HttpUtil.createContentDisposition(request, "attachement", asciiFilename));
    Assert.assertEquals(
        "attachement; filename=space\" space.txt",
        HttpUtil.createContentDisposition(request, "attachement", asciiQuote));
    // Note, this should work as the \u00E9 is just extended ascii (iso8859) which appears to be
    // valid for a web header (but not a mail header)
    Assert.assertEquals(
        "attachement; filename=Wikip\u00E9dia.txt",
        HttpUtil.createContentDisposition(request, "attachement", iso8859Filename));
    Assert.assertEquals(
        "attachement; filename=???%20???.pdf",
        HttpUtil.createContentDisposition(request, "attachement", unicodeFilename));
  }
  ...
  public void testChrome10minusDispositionWithPath() {
    ...
    Assert.assertEquals(
        "attachement; filename=space\" space.txt",
        HttpUtil.createContentDisposition(request, "attachement", asciiQuote));
    // Note: Here's where the main difference is between with and without a path that matches. If
    // there is a non-ascii filename
    // that happens to match what we got in the pathInfo of the request, we'll leave the name out so
    // the browser
  ...
  public void testSanitizeUrl() {
    Assert.assertEquals(
        "http://example.com/", HttpUtil.sanitizeURL("http://username:password@example.com/"));
  }
}
