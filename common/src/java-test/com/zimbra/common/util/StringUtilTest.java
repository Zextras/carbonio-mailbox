...
public class StringUtilTest {
  ...
  public void testFillTemplateWithNewlineValue() {
    String template =
        "New message received at ${RECIPIENT_ADDRESS}."
    ...
    String expected =
        "New message received at recipient@example.zimbra.com."
  ...
  public void testStripControlCharacters() {
    ...
    Assert.assertEquals(
        "just strippable chars", StringUtil.stripControlCharacters("\u0000\u0002"), "");
    Assert.assertEquals(
        "char between strippable chars", StringUtil.stripControlCharacters("\u0000v\u0002"), "v");
    Assert.assertEquals(
        "char, strip, char, strip", StringUtil.stripControlCharacters("c\u0000v\u0002"), "cv");
    Assert.assertEquals(
        "strip, char, strip, char", StringUtil.stripControlCharacters("\u0000v\u0002x"), "vx");
    Assert.assertEquals(
        "misordered surrogates at start",
        StringUtil.stripControlCharacters("\uDC00\uDBFFv\u0002x"),
        "vx");
    Assert.assertEquals(
        "misordered surrogates at end",
        StringUtil.stripControlCharacters("v\u0002x\uDC00\uDBFF"),
        "vx");
    Assert.assertEquals(
        "surrogates and char, strip, char",
        StringUtil.stripControlCharacters("\uDBFF\uDC00v\u0002x"),
        "\uDBFF\uDC00vx");
    Assert.assertEquals(
        "surrogates and BOM",
        StringUtil.stripControlCharacters("\uDBFF\uDC00\uFFFFvx"),
        "\uDBFF\uDC00vx");
  }
  ...
  public void testReplaceSurrogates() {
    ...
    Assert.assertEquals(
        "leading surrogate", StringUtil.removeSurrogates("\uDBFF\uDC00\uFFFFvx"), "?\uFFFFvx");
    Assert.assertEquals(
        "trailing surrogate", StringUtil.removeSurrogates("\uFFFFvx\uDBFF\uDC00"), "\uFFFFvx?");
    Assert.assertEquals(
        "consecutive surrogates",
        StringUtil.removeSurrogates("\uFFFFvx\uDBFF\uDC00\uDBFF\uDC00"),
        "\uFFFFvx??");
  }
  ...
  public void testReplaceAll() {
    Assert.assertEquals(
        "abc456def456ghi", StringUtil.replaceAll("abc123def12223ghi", "1\\d+3", "456"));
  }
