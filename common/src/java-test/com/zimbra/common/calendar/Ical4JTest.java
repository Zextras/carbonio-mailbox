...
/**
 * Intended to test features that Zimbra has chosen to add to ical4j in the past, to ensure that
 * future integrations still function correctly.
 */
public class Ical4JTest {
  ...
  /**
   * Uri-encoding was causing problems with java.net.URL building (specifically we would end up with
   * URL's where the SchemeSpecificPart was "MAILTO%3Afoo%40bar.com")
   */
  @Test
  public void testUrisEncodeDecode() {
    ...
    /**
     * This was failing on baseline ical4j-0.9.16 with: junit.framework.ComparisonFailure: Result of
     * Encode expected:<mailto[:foobar@]example.net> but was:<mailto[%3Afoobar%40]example.net>
     */
  ...
  /**
   * Bug 50398 - handling of empty CN parameter An empty CN parameter should not affect the value of
   * a property. With unpatched ical4j-0.9.16 it did. It is acceptable to either drop the CN
   * parameter or leave it as the empty string
   */
  @Test
