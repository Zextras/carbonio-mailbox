...
public class ZMimeBodyPartTest {
  ...
  public void setStringContent() throws Exception {
    ...
    Assert.assertArrayEquals(
        content.getBytes("utf-8"), ByteUtil.getContent(mbp.getInputStream(), -1));
  ...
  public void shareContent() throws Exception {
    ...
    mbp.writeTo(System.out);
    Assert.assertEquals(
        ZTransferEncoding.BASE64.toString(), mbp.getHeader("Content-Transfer-Encoding", null));
    Assert.assertArrayEquals(
        content.getBytes("ISO-2022-JP"), ByteUtil.getContent(mbp.getInputStream(), -1));
  }

  private void testEncodingSelection(String msg, String content, ZTransferEncoding cteExpected)
      throws Exception {
    ZMimeBodyPart mbp = new ZMimeBodyPart();
    mbp.setText(content);
    mbp.updateHeaders();
    Assert.assertEquals(
        msg, cteExpected.toString(), mbp.getHeader("Content-Transfer-Encoding", null));
  }
  ...
  public void encoding() throws Exception {
    ...
    testEncodingSelection(
        "single non-ASCII", "cyb\u00c8le illus.", ZTransferEncoding.QUOTED_PRINTABLE);
    ...
    testEncodingSelection(
        "line too long (ascii)",
        StringUtils.leftPad("", 1000, "X"),
        ZTransferEncoding.QUOTED_PRINTABLE);
    testEncodingSelection(
        "line too long (JIS)",
        new String(StringUtils.leftPad("", 1000, "あ").getBytes("ISO-2022-JP")),
        ZTransferEncoding.QUOTED_PRINTABLE);
    testEncodingSelection(
        "line too long (utf-8)",
        new String(StringUtils.leftPad("", 1000, "あ").getBytes("UTF-8")),
        ZTransferEncoding.BASE64);
    testEncodingSelection(
        "line too long (shift-jis)",
        new String(StringUtils.leftPad("", 1000, "あ").getBytes("Shift-JIS")),
        ZTransferEncoding.BASE64);
  }
}
