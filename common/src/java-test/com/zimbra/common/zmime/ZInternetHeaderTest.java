...
public class ZInternetHeaderTest {
  ...
  private static String RAW_HEADER =
      "=?utf-8?B?V1NVUzog5pu05paw44OX44Ot44Kw44Op44Og44Gu54q25oWL?=\r\n "
          + "=?utf-8?B?44Gu5qaC6KaB44KSIEJXU1VTVk1TViDjgYvjgonlj5fkv6HjgZfjgb7j?=\r\n "
          + "=?utf-8?B?gZfjgZ8=?=";
  private static String RAW_HEADER_COMBINED =
      "=?utf-8?B?V1NVUzog5pu05paw44OX44Ot44Kw44Op44Og44Gu54q25oWL"
          + "44Gu5qaC6KaB44KSIEJXU1VTVk1TViDjgYvjgonlj5fkv6HjgZfjgb7j"
          + "gZfjgZ8=?=";
  private static String ZBUG536 =
      "=?iso-8859-1?B?QVBBRCAtIFN0YXRzIEFQQUQgLSBE6WJ1dCBldCBmaW4gZOljaXNpb24gZW50cmUgcG91ciBsZSBt\n"
          + "  b2lzIGRlIEp1aW4gMjAxOA==?=";
  private static String DECODED_HEADER = "WSUS: 更新プログラムの状態の概要を BWSUSVMSV から受信しました";
  private static String RAW_HEADER_FRENCH1 =
      "[FSU] Fwd: XXXXXX] =?UTF-8?Q?r=C3=A9ponse_=C3=A0?= la lettre du"
          + " =?UTF-8?Q?pr=C3=A9sident=2E?=";
  private static String DECODED_FRENCH1 = "[FSU] Fwd: XXXXXX] réponse à la lettre du président.";
  private static String RAW_HEADER_FRENCH2 =
      "Je vais en =?utf-8?Q?v=C3=A9lo_=C3=A0_l'?= =?utf-8?Q?=C3=A9t=C3=A9?=";
  ...
  private static String ZBUG1022 =
      "=?UTF-8?Q?Memo Manager Data Import ATA.SDOA.AMMP512.D042419.T0943.xm?=\n"
          + " =?UTF-8?Q?l APR 24-2019 08:10 AM (GMT - 5:00).?=";

  /**
   * Created this test file using an external Python script. Each line of the file contains the
   * following elements, each separated by a single TAB character: messageId decoded-subject-header
   * encoded-subject-header Both the decoded-subject-header and encoded-subject-header are stored in
   * the file as a base64-encoded chunk. This is because they may span multiple lines and the
  ...
  @Test
  public void testDecodeFromResource() throws IOException {
    ...
    try {
      ...
      while (line != null) {
        ...
        Assert.assertEquals(
            String.format(
                "Decoding failed, messageId=%s, expected=\"%s\", actual=\"%s\"",
                parts[0], decoded, decodedHeader),
            decoded,
            decodedHeader);
  ...
  public void testMultilineZBUG536Subject() {
    ...
    Assert.assertEquals(
        "APAD - Stats APAD - Début et fin décision entre pour le mois de Juin 2018", decodedHeader);
  }
  ...
  public void testMultilineUTF8ZBUG1022Subject() {
    ...
    Assert.assertEquals(
        "Memo Manager Data Import ATA.SDOA.AMMP512.D042419.T0943.xml APR 24-2019 08:10 AM (GMT -"
            + " 5:00).",
        decodedHeader);
  }
}
