...
public class ZMimeParserTest {
  ...
  private void testMultipartWithoutBoundary(ZMimeMultipart mmp) throws Exception {
    Assert.assertEquals(
        "multipart subtype: mixed", "mixed", new ZContentType(mmp.getContentType()).getSubType());
    ...
    Assert.assertEquals(
        ...
        "text/plain",
        new ZContentType(mmp.getBodyPart(0).getContentType()).getBaseType());
    Assert.assertEquals(
        "second part is application/x-unknown",
        "application/x-unknown",
        new ZContentType(mmp.getBodyPart(1).getContentType()).getBaseType());
  }
  ...
  public void detectBoundary() throws Exception {
    ...
    MimeMessage mm =
        new ZMimeMessage(getSession(), new SharedByteArrayInputStream(bb.toByteArray()));
    ...
    Assert.assertEquals(
        ...
        "alternative",
        new ZContentType(mmp.getContentType()).getSubType());
  ...
  public void multipleContentTypes() throws Exception {
    ...
    try {
      ...
      Assert.assertEquals(
          "text/plain", "text/plain", new ZContentType(mm.getContentType()).getBaseType());
  ...
  public void parse() throws Exception {
    ...
    try {
      MimeMessage mm =
          ZMimeParser.parse(getSession(), new SharedByteArrayInputStream(bb.toByteArray()));
      Assert.assertFalse("content isn't multipart", mm.getContent() instanceof MimeMultipart);
      Assert.assertEquals(
          "text/plain", "text/plain", new ZContentType(mm.getContentType()).getBaseType());
    } catch (ClassCastException e) {
  ...
  private void traverseChildren(ZMimeMultipart mp, int targetDepth)
  ...
  public void recursion() throws Exception {
    ...
    try {
      ...
      traverseChildren(
          (ZMimeMultipart) zmp.getBodyPart(0).getContent(), LC.mime_max_recursion.intValue() - 1);
    } catch (ClassCastException e) {
      Assert.fail("mishandled double Content-Type headers");
    }
  }
  ...
  //        MimeMessage jmmm = new com.zimbra.common.mime.shim.JavaMailMimeMessage(s, new
  // javax.mail.util.SharedFileInputStream(file));
  ...
  // java.io.File("/Users/dkarp/Documents/messages").listFiles()) {
  //            if (file.isFile()) {
  //                checkFile(file);
  //            }
  //        }
